...
void test1() {
  ...
  for (double x = -5; x <= 5.0; x += 0.1) {
    ...
    ham->compute(mol);
    std::cout << x << "  " << ham->H(mol, 0, 0).real() << " " << ham->H(mol, 1, 1).real() << "  "
              << ham->H(mol, 0, 1).imag() << "\n";
...
void run_semiclassical() {
  ...
  for (double kstart = 1; kstart <= 50; kstart += 0.25) {
    ...
    Ensemble* ens;
    ens = new Ensemble(ntraj, nstates, Nnucl);

    vector<vector<vector<double> > > FCij(
        ntraj, vector<vector<double> >(nstates, vector<double>(nstates, 1.0)));
          ...
          if (ens->is_active[traj]) {
            if (method == 0) {
              compute_hopping_probabilities(
                  ens->mol[traj], ens->el[traj], ens->ham[traj], Pij, dt, 0, 300.0);
            } else if (method == 1) {
              compute_hopping_probabilities_mssh(
                  ens->mol[traj], ens->el[traj], ens->ham[traj], Pij, dt, 0, 300.0);
            }
            if (method == 2) {
              compute_hopping_probabilities_decoh(
                  ens->mol[traj], ens->el[traj], ens->ham[traj], Pij, FCij[traj], dt, 0, 300.0);
            }
...
*/
            ...
            if (ens->mol[traj]->R[0].x < -20 || ens->mol[traj]->R[0].x > 10.0) {
              ens->is_active[traj] = 0;
            }
  ...
  for (int ie = 0; ie <= 16; ie++) {  // fake parameter loop
    ...
    for (i = 0; i < ntraj; i++) {
      ...
      if (rep == 0) {
        ...
        ens->el[i]->q[0] = f0 * cos(ksi);
        ens->el[i]->p[0] = f0 * sin(ksi);
        ens->el[i]->q[1] = f1 * cos(ksi);
        ens->el[i]->p[1] = f1 * sin(ksi);
...
*/
      ...
      ens->sh_pop1(
          pops);  // specifically for Marcus spin-boson model in adiabatic representation with projection on diabatic states
...
void run_double_slit() {
  ...
  double kstart =
      0.1366 *
      1.0;  // sqrt(2048.0 * 4.5564e-6 * 2.0 * 1.0);   // initial nuclear momentum - corresponds to Ekin = 2048 cm^-1
  ...
  for (i = 0; i < ntraj; i++) {
    ...
    ens->mol[i]->P[0].y =
        0.0 + 0.0 * (0.5 / 279.34) *
                  normal();  // It is essentiall to make some distribution in this direction
    ens->mol[i]->P[0].z = 0.0;
  ...
  vector<vector<double> > flux(
      Nx, vector<double>(Ny, 0.0));  // Total (integrated over time flux density on the grid)
...
*/
      ...
      // Deactivate trajectories:
      for (int traj = 0; traj < ens->size; traj++) {
        if (ens->mol[traj]->R[0].x < -500 || ens->mol[traj]->R[0].x > 500.0) {
          ens->is_active[traj] = 0;
        }
      }
...
*/
    ...
    en << isnap << "  " << Ekin << "   " << Epot << "  " << Etot << "  " << Ecorr << "  "
       << Etot + Ecorr << endl;

    ofstream flux_map("flux_map.txt", ios::out);
    for (int nx = 0; nx < Nx; nx++) {
      for (int ny = 0; ny < Ny; ny++) {
        flux_map << (fl_Xmin + nx * fl_dx) << "  " << (fl_Ymin + ny * fl_dy) << "  "
                 << (flux[nx][ny] / counter) << "\n";
      }
      flux_map << "\n";
    }
  ...
  if (n_hits < 1) {
    cout << "Warning: No trajectory reached the screen\n";
  }
...
void run_double_slit_exact() {
  ...
  cout << "Nx= " << wfc->Nx << "  xmin= " << wfc->xmin << "  xmax= " << wfc->xmax
       << "  dx=  " << wfc->dx << endl;
  cout << "Ny= " << wfc->Ny << "  ymin= " << wfc->ymin << "  ymax= " << wfc->ymax
       << "  dy=  " << wfc->dy << endl;
  //  exit(0);
  ...
  // dy = 279.346
  wfc->init_wfc(
      xstart, ystart, kstart, 0.0, alpha, 1.0 * sqrt(16.0 * V0 / (1.0 * omega * omega)), 0);
  ...
  wfc->update_propagator_K(
      0.5 * dt, wfc->mol->mass[0]);  // !!! NOTE: dt/2 not dt !!! - depends on splitting scheme

...
*/
        ...
        if (ham_indx == 8 || ham_indx == 9) {  // cubic, double_well
          ...
          nnucl = 1;
                 ...
                 ham_indx == 5 || ham_indx == 6 ||
                 ham_indx == 7) {  // SAC, DAC, ECWR, Marcus, Rabi2, sin
          nelec = 2;
          nnucl = 1;
        ...
        *ham_adi = 0.0;
        basis_transform = new MATRIX(nelec, nelec);
        basis_transform->Init_Unit_Matrix(1.0);  // trivial basis
        basis_overlap = new MATRIX(nelec, nelec);
        basis_overlap->Init_Unit_Matrix(1.0);  // identity matrix
        ...
        for (i = 0; i < nnucl; i++) {
          ...
          *d1ham_adi[i] = 0.0;
          d1basis_overlap[i] = new MATRIX(nelec, nelec);
          *d1basis_overlap[i] = 0.0;
        }
...
*/
        ...
        if (d1basis_overlap.size() > 0) {
          d1basis_overlap.clear();
        }
      }
...
*/
    ...
    columns in the input matrix ( "
               << basis_overlap_.n_cols << " ) does not match\
    the number of electronic states used in the present model ( "
               << nelec << ") \nExiting now...\n";
          exit(0);
        }
    ...
    rows in the input matrix ( "
               << basis_overlap_.n_rows << " ) does not match\
    the number of electronic states used in the present model ( "
               << nelec << ") \nExiting now...\n";
          exit(0);
        }
...
*/
    ...
    the number of electronic states used in the present model ( "
      ...
      void Hamiltonian_Model::set_basis_overlap_derivs_by_ref(vector<MATRIX>& basis_overlap_derivs_,
                                                              vector<int>& indxs_) {
        /**
*/
      }

      void Hamiltonian_Model::set_basis_overlap_derivs_by_val(vector<MATRIX>& basis_overlap_derivs_,
                                                              vector<int>& indxs_) {
        /**
*/
      }
...
*/
        ...
        if (status_dia == 0) {  // only compute this if the result is not up to date
          ...
          if (ham_indx == 0) {
            SAC_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // SAC potetnial
          else if (ham_indx == 1) {
            DAC_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // DAC potential
          else if (ham_indx == 2) {
            ECWR_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // ECWR potential
          else if (ham_indx == 3) {
            Marcus_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // Marcus potential
          else if (ham_indx == 4) {
            SEXCH_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // SEXCH potential
          else if (ham_indx == 5) {
            Rabi2_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // Rabi2 potential
          else if (ham_indx == 6) {
            ;
            ;
            ...
            sin_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // sin potential
          else if (ham_indx == 8) {
            cubic_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // cubic potential
          else if (ham_indx == 9) {
            double_well_Ham(x, ham_dia, d1ham_dia[0], d2ham_dia[0], params);
          }  // double_well potential

          //=============== 2D models ========================
          else if (ham_indx == 200) {
            sin_2D_Ham(
                x, y, ham_dia, d1ham_dia[0], d1ham_dia[1], d2ham_dia[0], d2ham_dia[3], params);
          }  // sin potetnial
...
*/
        ...
        if (status_adi == 0) {
          ...
          for (int n = 0; n < nnucl; n++) {
            *d1ham_adi[n] = (*basis_transform).T() * (*d1ham_dia[n] - *ham_adi * *basis_overlap) *
                            (*basis_transform);

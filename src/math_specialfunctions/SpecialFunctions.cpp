...
*/
    ...
    void binomial_expansion(int n1,
                            int n2,
                            double x1,
                            double x2,
                            double* f,
...
**/
          ...
          if (is_derivs) {
            ...
            dfdx2[4] = 0.0;
            dfdx1[0] = f[1] - dfdx2[0];
            dfdx1[1] = dfdx2[1] + 6.0 * x12;
            dfdx1[2] = 3.0 * x2 + 6.0 * x1;
            dfdx1[3] = 3.0;
            dfdx1[4] = 0.0;
          }
...
**/
          ...
          if (is_derivs) {
            ...
            dfdx1[4] = 0.0;
            dfdx2[0] = f[1] - dfdx1[0];
            dfdx2[1] = dfdx1[1] + 6.0 * x12;
            dfdx2[2] = 3.0 * x1 + 6.0 * x2;
            dfdx2[3] = 3.0;
            dfdx2[4] = 0.0;
          }
          /**  Fully expanded version - retain for clarity: 27 mults
...
**/
          ...
          if (is_derivs) {
            dfdx1[0] = 2.0 * x1 * x22;
            dfdx1[1] = 2.0 * x22 + 4.0 * x12;
            dfdx1[2] = 2.0 * x1 + 4.0 * x2;
            dfdx1[3] = 2.0;
            dfdx1[4] = 0.0;
            dfdx2[0] = f[1] - dfdx1[0];
            dfdx2[1] = 4.0 * x12 + 2.0 * x11;
            dfdx2[2] = 2.0 * x2 + 4.0 * x1;
            dfdx2[3] = 2.0;
            dfdx2[4] = 0.0;
          }
          ...
          for (int j = 0; j <= n2; j++) {
            ...
            if (is_derivs) {
              ...
              if (n1 - i == 0) {
                ...
                pw1 = (n1 - i) * FAST_POW(x1, n1 - i - 1);
              }
              if (n2 - j == 0) {
                pw2 = 0.0;
              } else {
                pw2 = (n2 - j) * FAST_POW(x2, n2 - j - 1);
              }
...
*/
      if (x < a) {
        cout << "Error: Chebychev polynomial is not defined for x = " << x << " < a = " << a
             << endl;
        exit(0);
      }
      if (x > b) {
        cout << "Error: Chebychev polynomial is not defined for x = " << x << " > b = " << b
             << endl;
        exit(0);
      }
...
*/
    ...
    double Ellipe(double phi0,
                  double k,
                  int N)  // Elliptic integral of first kind (Jacobi elliptic function)
    {
      double cp, sp, sp2;
      double k2 = k * k;
      double phi = phi0;
    ...
    void Ellipe2(double u, double k, double& sn, double& cn, double& dn) {
      ...
      sn = u * (1.0 - (1.0 + k2) * u2 * f3 + (1.0 + 14.0 * k2 + k4) * u4 * f5 -
                (1.0 + 135.0 * k2 + 135.0 * k4 + k6) * u6 * f7);
    ...
    void Jacobi_Elliptic(
        double u, double m, double tol, double& am, double& sn, double& cn, double& dn) {
...
*/
      ...
      int sz = x.n_cols;
      MATRIX* evec;
      evec = new MATRIX(sz, sz);
      *evec = 0.0;
      MATRIX* eval;
      eval = new MATRIX(sz, sz);
      *eval = 0.0;
      MATRIX res(sz, sz);

      // Find the eigenvalues of the the S matrix
      libmeigen::solve_eigen(
          x, *eval, *evec, 0);  // x * evec = evec * eval  ==>  x = evec * eval * evec.T()

      for (i = 0; i < sz; i++) {
        res.M[i * sz + i] = std::exp(dt * eval->get(i, i));
      }
...
*/
      ...
      int sz = x.n_cols;
      CMATRIX* evec;
      evec = new CMATRIX(sz, sz);
      *evec = complex<double>(0.0, 0.0);
      CMATRIX* eval;
      eval = new CMATRIX(sz, sz);
      *eval = complex<double>(0.0, 0.0);
      CMATRIX res(sz, sz);

      // Find the eigenvalues of the the S matrix
      libmeigen::solve_eigen(
          x, *eval, *evec, 0);  // x * evec = evec * eval  ==>  x = evec * eval * evec.H()
...
*/
      ...
      if (err > max_tol) {
        cout << "Error in exp_2(MATRIX...): The the sum has not converged to the defined tolerance "
                "of "
             << max_tol << " in " << nterms << " iterations.\nExiting now...\n";
        exit(0);
      }
    ...
    MATRIX exp_2(MATRIX& x, double dt, int nterms) { return exp_2(x, dt, nterms, 0.0); }

    MATRIX exp_2(MATRIX& x, double dt) { return exp_2(x, dt, 1000, 0.0); }
...
*/
      ...
      if (err > max_tol) {
        cout << "Error in exp_2(CMATRIX...): The the sum has not converged to the defined "
                "tolerance of "
             << max_tol << " in " << nterms << " iterations.\nExiting now...\n";
        exit(0);
      }
    ...
    CMATRIX exp_2(CMATRIX& x, complex<double> dt, int nterms) { return exp_2(x, dt, nterms, 0.0); }

    CMATRIX exp_2(CMATRIX& x, complex<double> dt) { return exp_2(x, dt, 1000, 0.0); }
    ...
    int merge_sort(vector<pair<int, double> >& in, vector<pair<int, double> >& out) {
      ...
      if (sz == 0) {
        ...
        while ((cl < half) && (cr < sz)) {
          ...
          if (out1[cl].second <= out2[cr - half].second) {
            out.push_back(out1[cl]);
            cl++;
          }  ///< <-- This is good
    ...
    vector<vector<int> > permutations_reiteration(vector<int>& given_list,
                                                  int size,
                                                  int num_elements,
    ...
    */
      ...
      for (i = 0; i < size; i++) {
        list_of_permutations =
            permutations_reiteration(given_list, size - 1, num_elements, list_of_permutations);

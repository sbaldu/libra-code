      ...
      BOOST_FOREACH (ptree::value_type& v, ctx_pt) {
        if (i == 0) {
          ...
          BOOST_FOREACH (ptree::value_type& v1, ctxt.ctx_pt) {
            if (j == 0) {
              v.second.put_child(
                  boost::property_tree::ptree::path_type(ctxt.path, ctxt.path_separator),
                  v1.second);
            }
            j++;
          }
...
*/
      ...
      BOOST_FOREACH (
          ptree::value_type& v1,
          ctx_pt.get_child(boost::property_tree::ptree::path_type(_path, path_separator))) {
    ...
    Context Context::get_child(std::string _path, std::string varname, Context default_val) {
      ...
      BOOST_FOREACH (ptree::value_type& v, x) {
        if (v.first == varname) {
          ...
          res.path = v.first;
          res.ctx_pt.add_child(boost::property_tree::ptree::path_type(v.first, path_separator),
                               v.second);
          return res;
        }
      }
...
*/
      ...
      BOOST_FOREACH (ptree::value_type& v1, x) {
        if (v1.first == varname) {
          ...
          y.path = v1.first;
          y.ctx_pt.add_child(boost::property_tree::ptree::path_type(v1.first, path_separator),
                             v1.second);
          res.push_back(y);
        }
      }

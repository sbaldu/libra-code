...
namespace liblibra {
  ...
  namespace libscripts {
    namespace libstate {
      ...
      public:
        std::string ensemble;
        int is_ensemble;  // ensemble used (NVE, NVT, etc)
        std::string integrator;
        int is_integrator;  // name of MD integrator used

        double dt;
        int is_dt;  // time step in tau (0.02 tau = 1 fs)
        int n_medium;
        int is_n_medium;  // number of medium steps per 1 slow (dt) step - for RESPA
        int n_fast;
        int is_n_fast;  // number of fast steps per 1 medium (dt/n_medium) step - for RESPA
        int n_outer;
        int is_n_outer;  // number of outer steps(barostat,thermostat,etc.) per 1 md step (slow) - for RESPA
        double max_time;
        int is_max_time;  // time of the simulation
        int max_step;
        int is_max_step;  // # of MD step

        double curr_time;
        int is_curr_time;  // current time since start of MD simulation
        double curr_step;
        int is_curr_step;  // current # of MD step

        int terec_exp_size;
        int is_terec_exp_size;  // expansion size for Terec method
        int use_vlist;
        int is_use_vlist;  // Flag turning on/off the Verlet list option
        int vlist_upd_freq;
        int is_vlist_upd_freq;  // Number of steps between updating the Verlet list
        int vlist_time;
        int is_vlist_time;  // Number of steps from the last update of Verlet list
      ...
      public:
        ...
        // State veriables
        double E_kin;
        int is_E_kin;  // Kinetic energy;
        double E_kin_tr;
        int is_E_kin_tr;  // Translational kinetic energy
        double E_kin_rot;
        int is_E_kin_rot;  // Rotational kinetic energy
        double E_pot;
        int is_E_pot;  // Potential energy;
        double E_tot;
        int is_E_tot;  // Total   energy;
        double H;
        int is_H;  // Enthalpy
        double H_NHC;
        int is_H_NHC;  // Nose-Hoover Chains Hamiltonian.
        double H_NP;
        int is_H_NP;  // Nose-Poincare Hamiltonian
        double H0;
        int is_H0;  // Value of the Hamiltonian for time dependent (nonconservative) simulations
        double curr_T;
        int is_curr_T;  // Current Temperature;
        double curr_V;
        int is_curr_V;  // Current system volume
        double curr_P;
        int is_curr_P;  // Current system pressure
        MATRIX3x3 curr_P_tens;
        int is_curr_P_tens;  // Current pressure tensor
        int Nf_t;
        int is_Nf_t;  // Number of translational degrees of freedom
        int Nf_r;
        int is_Nf_r;  // Number of rotational degrees of freedom
        VECTOR L_tot;
        int is_L_tot;  // Total angular momentum
        VECTOR P_tot;
        int is_P_tot;  // Total linear momentum

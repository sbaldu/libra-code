    ...
    namespace libstate {
      ...
      void State::init_md(Nuclear& mol, Electronic& el, Hamiltonian& ham, Random& rnd) {
        ...
        if (md->ensemble == "NVT" || md->ensemble == "NPT") {
          if (!is_thermostat) {
            std::cout << "Error: No Thermostat object were set up. Can not run simulations in "
        ...
        if (md->ensemble == "NPT" || md->ensemble == "NPH") {
          Nf_b = 1;
          if (!is_barostat) {
            std::cout << "Error: No Barostat object were set up. Can not run simulations in "
                      << md->ensemble << " ensemble\n";
            exit(1);
          }
        }
        ...
        if (is_barostat && is_thermostat) {
          if (thermostat->thermostat_type != "Nose-Hoover" && md->ensemble == "NPT") {
            std::cout << "Error: Can not use thermostat other than Nose-Hoover for simulations in "
                         "NPT ensemble\n Exiting now...\n";
            exit(1);
          }
        }
      ...
      void State::run_md(Nuclear& mol, Electronic& el, Hamiltonian& ham) {
        ...
        is_thermostat =
            ((thermostat != NULL) &&
             ((md->ensemble == "NVT") || (md->ensemble == "NPT") || (md->ensemble == "NPT_FLEX")));
        is_barostat =
            ((barostat != NULL) && ((md->ensemble == "NPT") || (md->ensemble == "NPT_FLEX") ||
        ...
        while (md->curr_step < md->max_step) {
          ...
          if (is_barostat) {
            if (md->ensemble == "NPT" || md->ensemble == "NPH") {
              barostat->update_barostat_forces(syst->ekin_tr(), syst->ekin_rot(), curr_V, curr_P);
            } else if (md->ensemble == "NPT_FLEX" || md->ensemble == "NPH_FLEX") {
              barostat->update_barostat_forces(
                  syst->ekin_tr(), syst->ekin_rot(), curr_V, curr_P_tens);
            }
            scl = 0.0;
            if (is_thermostat) {
          ...
          if (md->ensemble == "NVT" || md->ensemble == "NPT" || md->ensemble == "NPT_FLEX" ||
              md->ensemble == "NPH" || md->ensemble == "NPH_FLEX") {
            if (is_thermostat) {
          ...
          if (is_barostat) {
            if (Nf_b == 9) {
              S = barostat->ksi_eps +
                  (barostat->ksi_eps.tr() / (barostat->get_Nf_t() /*+barostat->get_Nf_r()*/)) * I;
            } else if (Nf_b == 1) {
              S = barostat->ksi_eps_iso * I +
                  (3.0 * barostat->ksi_eps_iso / (barostat->get_Nf_t() /*+barostat->get_Nf_r()*/)) *
                      I;
            }
          }
          if (is_thermostat) {
          ...
          for (i = 0; i < syst->Number_of_fragments; i++) {
            RigidBody& top = syst->Fragments[i].Group_RB;
            if (is_thermostat) {
              thermostat->propagate_Ps(0.5 * dt_over_s2 * (top.ekin_rot() + top.ekin_tr()));
            }
            double Ps, s_var;
            s_var = 1.0;
            if (is_thermostat) {
          ...
          if (is_thermostat) {
            thermostat->propagate_Ps(dt * (H0 - Nf * boltzmann * thermostat->Temperature *

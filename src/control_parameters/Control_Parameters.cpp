...
*/
      ...
      DF =
          0;  /// DF = 0  - no extra output by default, use it only for small systems and for benchmarking purposes
      // </calculations>
      ...
      // For EHT
      eht_params_format =
          "eht+0";          /// eht_params_format = "eht+0" default format for EHT parameters
      ...
      eht_fock_opt =
          1;  /// eht_fock_opt    = 1 - need self-consistency correction, if SC-EHT is used
      ...
      nac_dt =
          1.0;  // convention is to compute NACs in units of [Ha/fs], so don't transform nac_dt to a.u. of time
...
*/
        ...
        if (file[i].size() > 0) {
          if (file[i][0] == "<calculation>") {
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "runtype") {
                  prms.runtype = file[i1][2];
                } else if (file[i1][0] == "hamiltonian") {
                  prms.hamiltonian = file[i1][2];
                } else if (file[i1][0] == "spin_method") {
                  prms.spin_method = file[i1][2];
                } else if (file[i1][0] == "DF") {
                  prms.DF = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "guess_type") {
                  prms.guess_type = file[i1][2];
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                // General
                if (file[i1][0] == "parameters") {
                  prms.parameters = file[i1][2];
                }

                // EHT-specific
                else if (file[i1][0] == "eht_params_format") {
                  prms.eht_params_format = file[i1][2];
                } else if (file[i1][0] == "eht_formula") {
                  prms.eht_formula = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "eht_sce_formula") {
                  prms.eht_sce_formula = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "eht_fock_opt") {
                  prms.eht_fock_opt = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "eht_electrostatics") {
                  prms.eht_electrostatics = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "scf_algo") {
                  prms.scf_algo = file[i1][2].c_str();
                } else if (file[i1][0] == "use_disk") {
                  prms.use_disk = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "use_rosh") {
                  prms.use_rosh = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "do_annihilate") {
                  prms.do_annihilate = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "pop_opt") {
                  prms.pop_opt = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "use_diis") {
                  prms.use_diis = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "diis_max") {
                  prms.diis_max = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "diis_start_iter") {
                  prms.diis_start_iter = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "use_level_shift") {
                  prms.use_level_shift = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "shift_magnitude") {
                  prms.shift_magnitude = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "use_damping") {
                  prms.use_damping = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "damping_start") {
                  prms.damping_start = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "damping_const") {
                  prms.damping_const = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "etol") {
                  prms.etol = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "den_tol") {
                  prms.den_tol = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "Niter") {
                  prms.Niter = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "degen_tol") {
                  prms.degen_tol = atof(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "dt") {
                  prms.md_dt = atof(file[i1][2].c_str()) * FS;
                } else if (file[i1][0] == "nsteps") {
                  prms.md_nsteps = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "dt") {
                  prms.opt_dt = atof(file[i1][2].c_str()) * FS;
                } else if (file[i1][0] == "nsteps") {
                  prms.opt_nsteps = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < f_sz; i1++) {
              if (file[i1].size() > 0) {
                if (file[i1][0] == "</multipole_options>") {
                  end_i = i1;
                  break;
                }
              }  // non-empty line
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "compute_dipole") {
                  prms.compute_dipole = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "compute_dos") {
                  prms.compute_dos = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "dos_opt") {
                  prms.dos_opt = file[i1][2];
                } else if (file[i1][0] == "dos_prefix") {
                  prms.dos_prefix = file[i1][2];
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < f_sz; i1++) {
              if (file[i1].size() > 0) {
                if (file[i1][0] == "</charge_density_options>") {
                  end_i = i1;
                  break;
                }
              }  // non-empty line
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "compute_charge_density") {
                  prms.compute_charge_density = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "charge_density_prefix") {
                  prms.charge_density_prefix = file[i1][2];
                } else if (file[i1][0] == "nx_grid") {
                  prms.nx_grid = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "ny_grid") {
                  prms.ny_grid = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "nz_grid") {
                  prms.nz_grid = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "orbs") {
                  prms.orbs = vector<int>(file[i1].size() - 2, 0);
                  for (int i2 = 2; i2 < file[i1].size(); i2++) {
                    prms.orbs[i2 - 2] = atoi(file[i1][i2].c_str());
                  }
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "nac_prefix") {
                  prms.nac_prefix = file[i1][2];
                } else if (file[i1][0] == "nac_md_trajectory_filename") {
                  prms.nac_md_trajectory_filename = file[i1][2];
                } else if (file[i1][0] == "nac_min_frame") {
                  prms.nac_min_frame = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "nac_max_frame") {
                  prms.nac_max_frame = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "nac_min_orbs") {
                  int nfr = atoi(file[i1][1].c_str());
                  prms.nac_min_orbs = vector<int>(nfr, 0);
                  for (int i2 = 0; i2 < nfr; i2++) {
                    prms.nac_min_orbs[i2] = atoi(file[i1][2 + i2].c_str());
                  }
                } else if (file[i1][0] == "nac_max_orbs") {
                  int nfr = atoi(file[i1][1].c_str());
                  prms.nac_max_orbs = vector<int>(nfr, 0);
                  for (int i2 = 0; i2 < nfr; i2++) {
                    prms.nac_max_orbs[i2] = atoi(file[i1][2 + i2].c_str());
                  }
                } else if (file[i1][0] == "nac_dt") {
                  prms.nac_dt = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "nac_opt") {
                  prms.nac_opt = atoi(file[i1][2].c_str());
                }
              }
            }  // for i1
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "scan_mov_at") {
                  prms.scan_mov_at = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "scan_ref_at") {
                  prms.scan_ref_at = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "scan_dxmin") {
                  prms.scan_dxmin = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "scan_dxmax") {
                  prms.scan_dxmax = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "scan_dx") {
                  prms.scan_dx = atof(file[i1][2].c_str());
                }
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() > 2) {
                if (file[i1][0] == "compute_excitations") {
                  prms.compute_excitations = atoi(file[i1][2].c_str());
                } else if (file[i1][0] == "excitations_opt") {
                  prms.excitations_opt = file[i1][2];
                } else if (file[i1][0] == "spectral_width") {
                  prms.spectral_width = atof(file[i1][2].c_str());
                } else if (file[i1][0] == "num_excitations") {
                  // Number of excitations
                  int tmp_sz = atoi(file[i1][2].c_str());

                  prms.num_excitations = 0;
                  if (prms.excitations.size() > 0) {
                    prms.excitations.clear();
                  }
                  ...
                  for (int n = 0; n < tmp_sz; n++) {
                    ...
                    if (ex_size != 1) {
                      cout << "Warning: Only single excitations are currently implemented. "
                              "Skipping this excitation\n";
                    } else {
            ...
            // now analyze all lines in between
            prms.t1 = VECTOR(atof(file[i + 2][0].c_str()),
                             atof(file[i + 2][1].c_str()),
                             atof(file[i + 2][2].c_str()));
            prms.t2 = VECTOR(atof(file[i + 3][0].c_str()),
                             atof(file[i + 3][1].c_str()),
                             atof(file[i + 3][2].c_str()));
            prms.t3 = VECTOR(atof(file[i + 4][0].c_str()),
                             atof(file[i + 4][1].c_str()),
                             atof(file[i + 4][2].c_str()));
            prms.t1 *= Angst;
            prms.t2 *= Angst;
            prms.t3 *= Angst;
            //        cell = Cell(t1,t2,t3);
            ...
            for (int n = 0; n < nfrags; n++) {
              ...
              int sz = atoi(file[i + 3 + n][3].c_str());
              prms.frag_size.push_back(sz);

              vector<int> frag;
              for (int i1 = 0; i1 < sz; i1++) {
                int at_indx = atoi(file[i + 3 + n][4 + i1].c_str()) -
                              1;  // !!! Because input contains atomic numbers (so min is 1, not 0)
      ...
      if (prms.compute_excitations == 1 && prms.compute_dipole != 1) {
        cout << "To compute excitations (spectra) dipole moments must be computed. Set "
                "compute_dipole to value 1\n";
        exit(0);
      }
      ...
      if (prms.hamiltonian == "eht") {
        if (prms.eht_sce_formula == 1) {
          if (prms.eht_params_format == "eht+0") {
            cout << "Error: eht_sce_formula = 1 requires eht_params_format to be more general than "
                    "eht+0\n";
            exit(0);
          }
        }  // SC-EHT

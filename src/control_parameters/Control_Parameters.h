...
*/
    ...
    public:
      ...
      int damping_start;  ///< The number of (standard) iterations before damping is in effect
                          ...
                          ///< Default: 3
      double
          damping_const;  ///< Parameter for damping, if used - this is the magnitude of electronic iteration step
      ...
      int Niter;  ///< The maximal number of SCF iterations before SCF is considered not converged
                  ...
                  ///< Default: 300
      double
          degen_tol;  ///< The amount of population difference between two levels, when one can say the two
      ...
      std::string parameters;  ///< Name of the file that contains parameters for Hamiltonian
                               ///< Default: "none"
      std::string
          eht_params_format;  ///< Format of the file that contains parameters for EHT Hamiltonian
                              ///< Possible options:
      ...
      std::string
          dos_opt;  ///< option for DOS comutation: "dens" - based on density matrix, "wfc" - based on wavefunction
      ...
      std::string
          charge_density_prefix;  ///< Prefix for the files in which CUBE orbitals will be written
      vector<int> orbs;
      // </charge_density_options>

      // <nac_options>
      std::string
          nac_md_trajectory_filename;  ///< Name of the file that contains coordinates of the MD trajectory (in xyz format)
      std::string
          nac_prefix;  ///< Prefix of all files into wich NACs for different time steps will be written
      ...
      vector<int>
          nac_min_orbs;  ///< indexes of minimal orbitals to include in NAC matrix - for each fragment
      vector<int>
          nac_max_orbs;  ///< indexes of maximal orbitals to include in NAC matrix - for each fragment
      ...
      double
          scan_dxmin;  ///< initial displacement along the scan direction w.r.t position of the reference atoms
      double
          scan_dxmax;  ///< final displacement along the scan direction w.r.t position of the reference atoms
      ...
      // <fragments>
      vector<vector<int> >
          fragments;  ///< list of atomic indices for atoms that are included in given fragment
      vector<int> frag_size;          ///< size of the fragments
      vector<std::string> frag_name;  ///< names of the fragments
      vector<double>
          frag_charge;  ///< charges of the fragments, sum must be equal to charge - see <coordinates> sections
                        // </fragments>

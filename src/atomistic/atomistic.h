...
*/
      ...
      System*
          _syst;  ///< reference to the system object (which can be allocated internally, or can be an external object)
      vector<int>
          ham_types;  ///< we can have several types of Hamiltonians. Their status is kept in this array:
    ...
    public:
      // Data members:
      listHamiltonian_MM*
          mm_ham;  ///< mm part: type = 0 This is the list of classical (MM) Hamiltonians for the sub-systems (e.g. bonds, angles, etc.)
      listHamiltonian_QM*
          qm_ham;  ///< qm part: type = 1 This is the list of quantum (QM) Hamiltonians for the sub-systems (e.g. fragments, blocks, etc)
      ...
      void add_excitation(int f_o, int f_s, int t_o, int t_s) {
        qm_ham->add_excitation(f_o, f_s, t_o, t_s);
      }
...
*/
      //  void compute();
      void set_system(System& syst) {
        _syst = &syst;
      }  ///< Making the internal _syst variable to point to the external object (binding)
      void compute_diabatic();
      void compute_adiabatic();
    };

    typedef std::vector<Hamiltonian_Atomistic>
        Hamiltonian_AtomisticList;  ///< data type for keeping a list of atomistic Hamiltonians

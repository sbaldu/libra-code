...
*/
      ...
      *ham_adi = 0.0;
      ...
      ham_types = vector<int>(
          5, 0);  // which Hamiltonians are initialized ham_types[0] = MM, ham_types[1] = QM
    }
...
*/
    ...
    bool Hamiltonian_Atomistic::is_active(Atom& a1, Atom& a2) { return mm_ham->is_active(a1, a2); }
    ...
    void Hamiltonian_Atomistic::set_atom_interactions_for_atoms(System& syst,
                                                                string int_type,
                                                                vector<Atom>& top_elt,
                                                                vector<int>& lst1,
                                                                vector<int>& lst2,
    ...
    void Hamiltonian_Atomistic::set_group_interactions_for_atoms(System& syst,
                                                                 string int_type,
    ...
    void Hamiltonian_Atomistic::set_interactions_for_fragments(System& syst,
                                                               boost::python::list lst1,
                                                               boost::python::list lst2,
                                                               ForceField& ff) {
    ...
    void Hamiltonian_Atomistic::apply_pbc_to_interactions(
...
*/
      ...
      if (q_.size() != nnucl) {
        cout << "Error in Hamiltonian_Atomistic::set_q - the size of input array (" << q_.size()
             << ") does not match";
...
*/
      ...
      if (v_.size() != nnucl) {
        cout << "Error in Hamiltonian_Atomistic::set_v - the size of input array (" << v_.size()
             << ") does not match";
        cout << "the number of nuclear degrees of freedom (" << nnucl << ")\nExiting...\n";
        exit(0);
      }
          ...
          if (method_option == 0) {
            ...
            *C = 0.0;
            ...
            for (int n = 0; n < _syst->Number_of_atoms; n++) {
              ...
              derivative_couplings1(*qm_ham->el,
                                    *_syst,
                                    qm_ham->basis_ao,
                                    qm_ham->prms,
                                    qm_ham->modprms,
                                    qm_ham->atom_to_ao_map,
              ...
              if (qm_ham->basis_ex.size() < nelec) {
                ...
                cout << "The number of excitations (" << qm_ham->basis_ex.size()
                     << ") is less than the dimensionality of the Hamiltonian (" << nelec << ")\n";
                cout << "Use the ham.add_excitation(int,int,int,int) function to add the "
                        "appropriate number of excitations\n";
                cout << "Exiting now...\n";
                exit(0);
              } else if (qm_ham->basis_ex.size() > nelec) {
                cout << "Warning: in Hamiltonian_Atomistic::compute_adiabatic()...\n";
                cout << "The number of excitations (" << qm_ham->basis_ex.size()
                     << ") is larger than the dimensionality of the Hamiltonian (" << nelec
                     << ")\n";
                cout << "Only first " << nelec
                     << " states will be handled. Other excitations are disregarded\n";
              }
            ...
            cout << "Derivative couplings computations for all atoms, time = " << tim1.stop()
                 << endl;
            ...
            for (int i = 1; i < qm_ham->basis_ex.size(); i++) {  // over all excitons in this basis
              ...
              *qm_ham->el->P = *qm_ham->el->P_alp + *qm_ham->el->P_bet;
              ...
              for (int n = 0; n < _syst->Number_of_atoms; n++) {
                _syst->Atoms[n].Atom_RB.rb_force = force(*qm_ham->el,
                                                         *_syst,
                                                         qm_ham->basis_ao,
                                                         qm_ham->prms,
                                                         qm_ham->modprms,

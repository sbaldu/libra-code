...
*/
        ...
        if (Norb != Hao->n_cols) {
          cout << "In Hamiltonian_core_hf: Dimension of input/output matrix is not compatible "
        ...
        for (i = 0; i < Norb; i++) {
          for (j = 0; j < Norb; j++) {
            ...
            for (n = 0; n < syst.Number_of_atoms; n++) {
              Hao->M[i * Norb + j] -=
                  modprms.PT[syst.Atoms[n].Atom_element].Zeff *
                  nuclear_attraction_integral(
                      basis_ao[i], basis_ao[j], syst.Atoms[n].Atom_RB.rb_cm);  // ,n,da,db,dc);
            }  // for n
...
*/
        ...
        if (Norb != el->Hao->n_cols) {
          cout << "In Hamiltonian_Fock_hf: Dimension of input/output matrix is not compatible "
                  "whith the number of the fragment-localized orbitals\n";
          exit(0);
        }
          ...
          for (b = 0; b < Norb; b++) {
            for (c = 0; c < Norb; c++) {
              for (d = 0; d < Norb; d++) {
                ...
                if (prms.use_rosh) {
                  el->Fao_alp->M[a * Norb + b] +=
                      (el->P->M[c * Norb + d] * J_abcd - 0.5 * el->P->M[c * Norb + d] * K_adcb);
                  el->Fao_bet->M[a * Norb + b] +=
                      (el->P->M[c * Norb + d] * J_abcd - 0.5 * el->P->M[c * Norb + d] * K_adcb);
                } else {
                  el->Fao_alp->M[a * Norb + b] +=
                      (el->P->M[c * Norb + d] * J_abcd - el->P_alp->M[c * Norb + d] * K_adcb);
                  el->Fao_bet->M[a * Norb + b] +=
                      (el->P->M[c * Norb + d] * J_abcd - el->P_bet->M[c * Norb + d] * K_adcb);
                }

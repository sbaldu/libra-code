...
*/
        ...
        if (Norb != Hao->n_cols) {
          cout << "In Hamiltonian_core_eht: Dimension of input/output matrix is not compatible "
        ...
        for (i = 0; i < Norb; i++) {  //    old
          Hao->M[i * Norb + i] = Hao->M[i * Norb + i] =
              modprms.orb_params[i].IP;  //modprms.PT[basis_ao[i].element].IP[basis_ao[i].ao_shell];
        }  // for i
        ...
        for (i = 0; i < Norb; i++) {
          ...
          for (j = i + 1; j < Norb; j++) {
            ...
            if (prms.eht_formula == 0) {  // Unweighted formula
              ...
              Hao->M[i * Norb + j] = 0.5 * K_const * (Hao->M[i * Norb + i] + Hao->M[j * Norb + j]) *
                                     Sao->M[i * Norb + j];
              ...
              delt = (Hao->M[i * Norb + i] - Hao->M[j * Norb + j]) /
                     (Hao->M[i * Norb + i] + Hao->M[j * Norb + j]);
              delt2 = delt * delt;
              delt4 = delt2 * delt2;

              Hao->M[i * Norb + j] = 0.5 * (K_const + delt2 + (1.0 - K_const) * delt4) *
                                     (Hao->M[i * Norb + i] + Hao->M[j * Norb + j]) *
                                     Sao->M[i * Norb + j];
              Hao->M[j * Norb + i] = Hao->M[i * Norb + j];
              ...
              // for atom i
              if (nz_i == 1) {
                ri = (n_i / modprms.PT[elt_i].zetas[sh_i][0]);
              } else if (nz_i == 2) {
              ...
              // for atom j
              if (nz_j == 1) {
                rj = (n_j / modprms.PT[elt_j].zetas[sh_j][0]);
              } else if (nz_j == 2) {
...
*/
        ...
        if (Norb != Hao->n_cols) {
          cout << "Hao matrix is not allocated\n Must be allocated before "
                  "Hamiltonian_core_deriv_eht is called\n";
          cout << "In Hamiltonian_core_deriv_eht: Dimension of input/output matrix is not "
            ...
            if (j != i) {
              ...
              if (b == a) {
                ...
                if (prms.eht_formula == 0) {  // Unweighted formula
                  ...
                  double beta_ij = 0.5 * (K_const + delt2 + (1.0 - K_const) * delt4) *
                                   (Hao->M[i * Norb + i] + Hao->M[j * Norb + j]);
                  ...
                  if (nz_i == 1) {
                    ...
                    ri = n_i / (c1 * c1 * z1 + c2 * c2 * z2 +
                                (pow(2.0, 2.0 * n_i) * pow(z1 * z2, n_i + 0.5) /
                                 pow((z1 + z2), 2.0 * n_i)));
                  }
                  ...
                  if (nz_j == 1) {
                    ...
                    rj = n_j / (c1 * c1 * z1 + c2 * c2 * z2 +
                                (pow(2.0, 2.0 * n_j) * pow(z1 * z2, n_j + 0.5) /
                                 pow((z1 + z2), 2.0 * n_j)));
                  }
                  ...
                  if (c == a) {
                    dbeta_ij_dr =
                        0.5 * (Hao->M[i * Norb + i] + Hao->M[j * Norb + j]) *
                        (-delta *
                         (syst.Atoms[a].Atom_RB.rb_cm - syst.Atoms[b].Atom_RB.rb_cm).unit()) *
                        (K_const - 1.0);
                  }
                  if (c == b) {
                    dbeta_ij_dr -=
                  ...
                  dHao_dx->M[i * Norb + j] +=
                      (beta_ij * dSdc.x + dbeta_ij_dr.x * Sao->M[i * Norb + j]);
                  dHao_dy->M[i * Norb + j] +=
                      (beta_ij * dSdc.y + dbeta_ij_dr.y * Sao->M[i * Norb + j]);
                  dHao_dz->M[i * Norb + j] +=
                      (beta_ij * dSdc.z + dbeta_ij_dr.z * Sao->M[i * Norb + j]);
...
*/
        ...
        Hamiltonian_core_deriv_eht(syst,
                                   basis_ao,
                                   prms,
                                   modprms,
                                   atom_to_ao_map,
        ...
        if (prms.eht_sce_formula == 0) {
          ...
          *el->Fao_bet = *el->Hao;
          ...
          for (int i = 0; i < Norb; i++) {
            ...
            double Q =
                syst.Atoms[a]
                    .Atom_mull_charge_gross;  // charge of the atom, on which i-th AO is sitting

            if (Q > 0) {
              Ai = modprms.PT[basis_ao[a].element].J_param1[basis_ao[a].ao_shell];
            } else {
              Ai = modprms.PT[basis_ao[a].element].J_param2[basis_ao[a].ao_shell];
            }
            ...
            for (j = i + 1; j < Norb; j++) {
              ...
              if (prms.eht_formula == 0) {  // Unweighted formula
                ...
                el->Fao_alp->M[i * Norb + j] =
                    0.5 * K_const * (el->Fao_alp->M[i * Norb + i] + el->Fao_alp->M[j * Norb + j]) *
                    el->Sao->M[i * Norb + j];
                ...
                delt = (el->Fao_alp->M[i * Norb + i] - el->Fao_alp->M[j * Norb + j]) /
                       (el->Fao_alp->M[i * Norb + i] + el->Fao_alp->M[j * Norb + j]);
                delt2 = delt * delt;
                delt4 = delt2 * delt2;

                el->Fao_alp->M[i * Norb + j] =
                    0.5 * (K_const + delt2 + (1.0 - K_const) * delt4) *
                    (el->Fao_alp->M[i * Norb + i] + el->Fao_alp->M[j * Norb + j]) *
                    el->Sao->M[i * Norb + j];
                el->Fao_alp->M[j * Norb + i] = el->Fao_alp->M[i * Norb + j];
          ...
          *el->Fao_bet = *el->Fao_alp;
          ...
          cout << "Warning (in Hamiltonian_Fock_eht):  no eht_sce_formula=" << prms.eht_sce_formula
               << " is known. Skipping...\n";
        }

        //========================= Additional QEq-like terms ==========================
        if (prms.eht_electrostatics >= 1) {
          for (a = 0; a < syst.Number_of_atoms;
               a++) {  // over all atoms - effects of electronegativities

            // Now add this to Fock matrix:
            int orb_a = atom_to_ao_map
                [a]
                [0];  // first orbital of the atom a - assume that coefficients do not depend on orbital, only
                      // on atom type
            double xi_a =
                modprms.orb_params[orb_a]
                    .J_param1;  // PT[basis_ao[orb_A].element].J_param1[basis_ao[orb_A].ao_shell];
            double J_aa =
                modprms.orb_params[orb_a]
                    .J_param2;  // PT[basis_ao[orb_A].element].J_param2[basis_ao[orb_A].ao_shell];
            ...
            for (i = 0; i < atom_to_ao_map[a].size(); i++) {    // over all orbitals on a
              for (j = 0; j < atom_to_ao_map[a].size(); j++) {  // over all orbitals on a
                ...
                double eri_ab =
                    modprms.meht_k.get_K_value(1, i, j);  // a.u. of energy - exchange integral
                ...
                el->Fao_alp->M[i * Norb + j] +=
                    (xi_a + J_aa * Qa) * (-el->Sao->M[i * el->Norb + j]);
          ...
          for (a = 0; a < syst.Number_of_atoms;
            ...
            for (i = 0; i < atom_to_ao_map[a].size(); i++) {    // over all orbitals on a
              for (j = 0; j < atom_to_ao_map[a].size(); j++) {  // over all orbitals on a
                ...
                for (b = 0; b < syst.Number_of_atoms;
                     b++) {  // over all atoms - effects of electronegativities

                  if (b != a) {
                    int orb_b = atom_to_ao_map
                        [b]
                        [0];  // first orbital of the atom b - assume that coefficients do not depend on orbital, only
                    // on atom type

                    double Qb = syst.Atoms[b].Atom_mull_charge_gross;
                    double dist =
                        (syst.Atoms[a].Atom_RB.rb_cm - syst.Atoms[b].Atom_RB.rb_cm).length();

                    double K2_const =
                        modprms.meht_k.get_K_value(2, orb_a, orb_b);  // a.u. of energy
                    double K3_const =
                        modprms.meht_k.get_K_value(3, orb_a, orb_b);  // a.u. of length
                    double K4_const =
                        modprms.meht_k.get_K_value(4, orb_a, orb_b);  // a.u. of length
                    if (K4_const < 0.0) {
                      K4_const = 0.0;
                    }
                    ...
                    J_ab =
                        J_ab * f + (1.0 - f) * K2_const;  // add long-range electrostatics smoothly

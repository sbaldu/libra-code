...
namespace liblibra {
  ...
  namespace libatomistic {
    ...
    namespace libhamiltonian_qm {
      ...
      vector<int> compute_sorb_indices(int sz,
                                       vector<AO>& basis_ao,
...
*/
        ...
        for (int a = 0; a < sz; a++) {  // for all atoms
          for (int i = 0; i < atom_to_ao_map[a].size();
               i++) {  // all orbitals on given atom (i-dummy)

            int I = atom_to_ao_map
                [a][i];  // i-th AO on atom a, I - is the global index of this AO in the given basis
      ...
      void compute_indo_core_parameters(System& syst,
                                        vector<AO>& basis_ao,
...
*/
        ...
        eri = electron_repulsion_integral(
            basis_ao[I], basis_ao[I], basis_ao[J], basis_ao[J]);  // eri[a][b]
        ...
        if (opt == 0) {
          V_AB = Zeff * nuclear_attraction_integral(
                            basis_ao[J], basis_ao[J], syst.Atoms[B].Atom_RB.rb_cm);  // V_AB[a][b]
        } else if (opt == 1) {
          V_AB = Zeff * eri;  //[a*sz+b];
        }
      }

      void compute_indo_core_parameters_derivs(System& syst,
                                               vector<AO>& basis_ao,
                                               Model_Parameters& modprms,
                                               vector<vector<int> >& atom_to_ao_map,
                                               vector<int>& ao_to_atom_map,
...
*/
        ...
        VECTOR DA, DB, DC, DD;
        double eri = electron_repulsion_integral(
            &basis_ao[I], &basis_ao[I], &basis_ao[J], &basis_ao[J], 1, 1, DA, DB, DC, DD);
        ...
        if (opt == 0) {
          ...
          double nai = nuclear_attraction_integral(
              basis_ao[J], basis_ao[J], syst.Atoms[B].Atom_RB.rb_cm, 1, 1, DA, DB, DC);  // V_AB[a][b]
...
*/
        ...
        double eri = electron_repulsion_integral(&basis_ao[I],
                                                 &basis_ao[I],
                                                 &basis_ao[J],
                                                 &basis_ao[J],
                                                 1,
...
*/
        ...
        if (deri.size() != syst.Number_of_atoms) {
          deri = vector<VECTOR>(syst.Number_of_atoms, VECTOR(0.0, 0.0, 0.0));
        }
        if (dV_AB.size() != syst.Number_of_atoms) {
      ...
      void compute_all_indo_core_parameters_derivs(System& syst,
                                                   vector<AO>& basis_ao,
                                                   Model_Parameters& modprms,
                                                   vector<vector<int> >& atom_to_ao_map,
                                                   vector<int>& ao_to_atom_map,
...
*/
        ...
        for (int c = 0; c < N; c++) {
          for (int a = 0; a < N; a++) {
            for (int b = 0; b < N; b++) {
              ...
              compute_indo_core_parameters_derivs(syst,
                                                  basis_ao,
                                                  modprms,
                                                  atom_to_ao_map,
                                                  ao_to_atom_map,
      ...
      void compute_all_indo_core_parameters_derivs1(System& syst,
                                                    vector<AO>& basis_ao,
                                                    Model_Parameters& modprms,
                                                    vector<vector<int> >& atom_to_ao_map,
                                                    vector<int>& ao_to_atom_map,
...
*/
      ...
      void indo_core_parameters(System& syst,
                                vector<AO>& basis_ao,
                                Model_Parameters& modprms,
                                vector<vector<int> >& atom_to_ao_map,
                                vector<int>& ao_to_atom_map,
...
*/
        ...
        if (DF) {
          ...
          cout << "sorb_indx[i] - is the global index of s-type orbital (assuming only one) "
                  "centered on atom i\n";
...
*/
        ...
        for (a = 0; a < sz; a++) {
          for (b = 0; b < sz; b++) {
            compute_indo_core_parameters(syst,
                                         basis_ao,
                                         modprms,
                                         atom_to_ao_map,
                                         ao_to_atom_map,
      ...
      void Hamiltonian_core_indo(System& syst,
                                 vector<AO>& basis_ao,
...
*/
        ...
        if (Norb != Hao->n_cols) {
          cout << "Hao matrix is not allocated\n Must be allocated before Hamiltonian_core_indo is "
                  "called\n";
          cout << "In Hamiltonian_core_indo: Dimension of input/output matrix is not compatible "
                  "whith the number of the fragment-localized orbitals\n";
          exit(0);
        }
        int sz = syst.Number_of_atoms;  // number of atoms in this fragment

        if (modprms.eri.size() != sz * sz) {
          cout << "Error in Hamiltonian_core_indo: size of auxiliary eri array is not right\n";
          exit(0);
        }
        if (modprms.V_AB.size() != sz * sz) {
          cout << "Error in Hamiltonian_core_indo: size of auxiliary V_AB array is not right\n";
          exit(0);
        }
        ...
        for (i = 0; i < Norb; i++) {  // global orbital indices
          ...
          if (Z == 1) {
            ...
            if (basis_ao[i].ao_shell_type == "s") {
              ...
              Hao->M[i * Norb + i] -= ((Z_core - 0.5) * modprms.eri[a * sz + a] - (1.0 / 3.0) * G1 -
          ...
          for (j = 0; j < Norb; j++) {
            if (j != i) {
              ...
              if (b == a) {
                ;
                ;
              }  // different orbitals centered on the same atom - give zero (not true for hybrid orbitals)
                ...
                double sao_ij = gaussian_overlap(
                    basis_ao[i], basis_ao[j]);  // 0, dIdA,dIdB), mem->aux, mem->n_aux);
...
*/
        ...
        Hamiltonian_core_indo(
            syst, basis_ao, prms, modprms, atom_to_ao_map, ao_to_atom_map, &Hao, &Sao, DF);
      }

      void Hamiltonian_core_deriv_indo(System& syst,
                                       vector<AO>& basis_ao,
                                       Control_Parameters& prms,
                                       Model_Parameters& modprms,
                                       vector<vector<int> >& atom_to_ao_map,
...
*/
        ...
        if (Norb != Hao->n_cols) {
          ...
          cout << "In Hamiltonian_core_deriv_indo: Dimension of input/output matrix is not "
        ...
        if (modprms.eri.size() != sz * sz) {
          cout
              << "Error in Hamiltonian_core_deriv_indo: size of auxiliary eri array is not right\n";
          exit(0);
        }
        if (modprms.V_AB.size() != sz * sz) {
          cout << "Error in Hamiltonian_core_deriv_indo: size of auxiliary V_AB array is not "
                  "right\n";
          exit(0);
        }
          ...
          for (j = 0; j < Norb; j++) {
            if (j != i) {
              ...
              if (b == a) {
                ...
                double beta_ij =
                    0.5 * (modprms.PT[basis_ao[i].element].beta0[basis_ao[i].ao_shell] +
                           modprms.PT[basis_ao[j].element].beta0[basis_ao[j].ao_shell]);
...
*/
        ...
        Hamiltonian_core_deriv_indo(syst,
                                    basis_ao,
                                    prms,
                                    modprms,
                                    atom_to_ao_map,
...
*/
      ...
      void Hamiltonian_Fock_indo(Electronic_Structure* el,
                                 System& syst,
                                 vector<AO>& basis_ao,
                                 Control_Parameters& prms,
                                 Model_Parameters& modprms,
...
*/
        ...
        if (Norb != el->Hao->n_cols) {
          cout << "In Hamiltonian_Fock_indo: Dimension of input/output matrix is not compatible "
                  "whith the number of the fragment-localized orbitals\n";
          exit(0);
        }
...
*/
                ...
                if (prms.use_rosh) {  // Restricted open-shell
                  el->Fao_alp->M[i * Norb + i] +=
                      (el->P->M[k * Norb + k] * ii_kk - 0.5 * el->P->M[k * Norb + k] * ik_ik);
                  el->Fao_bet->M[i * Norb + i] +=
                      (el->P->M[k * Norb + k] * ii_kk - 0.5 * el->P->M[k * Norb + k] * ik_ik);

                } else {  // unrestricted
                  el->Fao_alp->M[i * Norb + i] +=
                      (el->P->M[k * Norb + k] * ii_kk - el->P_alp->M[k * Norb + k] * ik_ik);
                  el->Fao_bet->M[i * Norb + i] +=
                      (el->P->M[k * Norb + k] * ii_kk - el->P_bet->M[k * Norb + k] * ik_ik);
                }
              ...
              for (b = 0; b < syst.Number_of_atoms; b++) {
                if (b != a) {
                  ...
                  el->Fao_alp->M[i * Norb + i] += (Zeff[b] - syst.Atoms[b].Atom_mull_charge_net) *
                                                  modprms.eri[a * syst.Number_of_atoms + b];
                  el->Fao_bet->M[i * Norb + i] += (Zeff[b] - syst.Atoms[b].Atom_mull_charge_net) *
                                                  modprms.eri[a * syst.Number_of_atoms + b];
                }
              }  // for b
              ...
              if (a == b) {  // different orbitals are on the same atom
                ...
                if (prms.use_rosh) {
                  el->Fao_alp->M[i * Norb + j] +=
                      ((2.0 * el->P->M[i * Norb + j] - 0.5 * el->P->M[i * Norb + j]) * ij_ij -
                  ...
                  el->Fao_alp->M[i * Norb + j] +=
                      ((2.0 * el->P->M[i * Norb + j] - el->P_alp->M[i * Norb + j]) * ij_ij -
                       el->P_alp->M[i * Norb + j] * ii_jj);
                  el->Fao_bet->M[i * Norb + j] +=
                      ((2.0 * el->P->M[i * Norb + j] - el->P_bet->M[i * Norb + j]) * ij_ij -
                ...
                if (prms.use_rosh) {
                  el->Fao_alp->M[i * Norb + j] -=
                      0.5 * el->P->M[i * Norb + j] * modprms.eri[a * syst.Number_of_atoms + b];
                  el->Fao_bet->M[i * Norb + j] -=
                      0.5 * el->P->M[i * Norb + j] * modprms.eri[a * syst.Number_of_atoms + b];
                } else {
                  el->Fao_alp->M[i * Norb + j] -=
                      el->P_alp->M[i * Norb + j] * modprms.eri[a * syst.Number_of_atoms + b];
                  el->Fao_bet->M[i * Norb + j] -=
                      el->P_bet->M[i * Norb + j] * modprms.eri[a * syst.Number_of_atoms + b];
                }
              }
...
*/
      ...
      void Hamiltonian_Fock_derivs_indo(Electronic_Structure* el,
                                        System& syst,
                                        vector<AO>& basis_ao,
                                        Control_Parameters& prms,
                                        Model_Parameters& modprms,
...
*/
        ...
        *dFao_bet_dz = *dHao_dz;
        ...
        vector<int> sorb_indx;
        sorb_indx =
            compute_sorb_indices(syst.Number_of_atoms, basis_ao, atom_to_ao_map, ao_to_atom_map);
...
*/
        ...
        Hamiltonian_Fock_derivs_indo(&el,
                                     syst,
                                     basis_ao,
                                     prms,
                                     modprms,

...
namespace liblibra {
  ...
  namespace libatomistic {
    ...
    namespace libhamiltonian_qm {
      ...
      void Hamiltonian_core(System& syst,
                            vector<AO>& basis_ao,
...
*/
      ...
      void Hamiltonian_Fock(Electronic_Structure* el,
                            System& syst,
                            vector<AO>& basis_ao,
                            Control_Parameters& prms,
                            Model_Parameters& modprms,
...
*/
      ...
      void derivative_couplings(Electronic_Structure& el,
                                System& syst,
                                vector<AO>& basis_ao,
                                Control_Parameters& prms,
                                Model_Parameters& modprms,
...
*/
        ...
        update_derivative_coupling_matrix(x_period,
                                          y_period,
                                          z_period,
                                          t1,
                                          t2,
      ...
      void derivative_couplings1(Electronic_Structure& el,
                                 System& syst,
                                 vector<AO>& basis_ao,
                                 Control_Parameters& prms,
                                 Model_Parameters& modprms,
...
*/
      ...
      VECTOR force(Electronic_Structure& el,
                   System& syst,
                   vector<AO>& basis_ao,
                   Control_Parameters& prms,
                   Model_Parameters& modprms,
...
*/
      ...
      VECTOR force_extended(Electronic_Structure& el,
                            System& syst,
                            vector<AO>& basis_ao,
                            Control_Parameters& prms,
                            Model_Parameters& modprms,
...
*/
  ...
  */
        ...
        *dP_alp_dx = *(el.C_alp) * ((*Dmo_a_x) * (*Oa) - (*Oa) * (*Dmo_a_x)) * (*el.C_alp).T() -
                     (*T + (*T).T());

        *T = (*el.C_alp) * (*el.C_alp).T() * (*Dao_y) * (*el.C_alp);
        *dP_alp_dy = *(el.C_alp) * ((*Dmo_a_y) * (*Oa) - (*Oa) * (*Dmo_a_y)) * (*el.C_alp).T() -
                     (*T + (*T).T());

        *T = (*el.C_alp) * (*el.C_alp).T() * (*Dao_z) * (*el.C_alp);
        *dP_alp_dz = *(el.C_alp) * ((*Dmo_a_z) * (*Oa) - (*Oa) * (*Dmo_a_z)) * (*el.C_alp).T() -
                     (*T + (*T).T());

        *T = (*el.C_bet) * (*el.C_bet).T() * (*Dao_x) * (*el.C_bet);
        *dP_bet_dx = *(el.C_bet) * ((*Dmo_a_x) * (*Oa) - (*Oa) * (*Dmo_a_x)) * (*el.C_bet).T() -
                     (*T + (*T).T());

        *T = (*el.C_bet) * (*el.C_bet).T() * (*Dao_y) * (*el.C_bet);
        *dP_bet_dy = *(el.C_bet) * ((*Dmo_a_y) * (*Oa) - (*Oa) * (*Dmo_a_y)) * (*el.C_bet).T() -
                     (*T + (*T).T());

        *T = (*el.C_bet) * (*el.C_bet).T() * (*Dao_z) * (*el.C_bet);
        *dP_bet_dz = *(el.C_bet) * ((*Dmo_a_z) * (*Oa) - (*Oa) * (*Dmo_a_z)) * (*el.C_bet).T() -
                     (*T + (*T).T());
  ...
  */
      ...
      double energy_and_forces(Electronic_Structure& el,
                               System& syst,
                               vector<AO>& basis_ao,
                               Control_Parameters& prms,
                               Model_Parameters& modprms,
...
*/
        ...
        Hamiltonian_core(
            syst, basis_ao, prms, modprms, atom_to_ao_map, ao_to_atom_map, *Hao, *Sao, debug);
  ...
  */
      ...
      listHamiltonian_QM::listHamiltonian_QM(const listHamiltonian_QM& ob) {  ///< Copy constructor;
        ...
        prms =
            ob.prms;  ///< Control parameters defining how to perform calculations for this sub-system
        ...
        //============ Excited states ================
        basis_ex =
            ob.basis_ex;  ///< Excitations for this sub-system -  may be the same as in prms, but may be different
      }

      void listHamiltonian_QM::operator=(
          const listHamiltonian_QM& ob) {  ///< Copying one listHamiltonian_QM into the other one
...
*/
        ...
        int verb = 0;
        set_basis_STO_3G_DZ(
            mol_at_types, R, modprms, verb, basis_ao, Nelec, Norb, atom_to_ao_map, ao_to_atom_map);

...
*/
        ...
        MATRIX* dP;
        dP = new MATRIX(Norb, Norb);  // dP = P_k+1 - P_k
        MATRIX* temp;
        temp = new MATRIX(Norb, Norb);
        MATRIX* P_old_alp;
        P_old_alp = new MATRIX(Norb, Norb);
        MATRIX* P_old_bet;
        P_old_bet = new MATRIX(Norb, Norb);
        MATRIX* P_old;
        P_old = new MATRIX(Norb, Norb);
        ...
        MATRIX* P_til_alp;
        P_til_alp = new MATRIX(Norb, Norb);
        MATRIX* P_til_bet;
        P_til_bet = new MATRIX(Norb, Norb);
        MATRIX* P_til;
        P_til = new MATRIX(Norb, Norb);
        ...
        MATRIX* Fao_til_alp;
        Fao_til_alp = new MATRIX(Norb, Norb);
        MATRIX* Fao_til_bet;
        Fao_til_bet = new MATRIX(Norb, Norb);
        ...
        *P_til = *P;
        ...
        Hamiltonian_Fock(el,
                         ...
                         modprms,
                         atom_to_ao_map,
                         ao_to_atom_map);  // el - now contains an updated Fock matrix
        ...
        *Fao_til_bet = *el->Fao_bet;
        ...
        Eelec_prev = energy_elec(el->P_alp,
                                 el->P_bet,
                                 el->Hao,
                                 el->Hao,
                                 el->Fao_alp,
        ...
        do {
            ...
            *Fao_bet = *el_tmp->Fao_bet;
            ...
            if (prms.use_damping) {
              ...
              *el_tmp->P_bet = *P_til_bet + lamb * (0.5 * *dP);
              ...
              Hamiltonian_Fock(
                  el_tmp, syst, basis_ao, prms, modprms, atom_to_ao_map, ao_to_atom_map);
              if (BM) {
                bench_t[1].stop();
              }
              ...
              double en1 = energy_elec(el_tmp->P_alp,
                                       el_tmp->P_bet,
                                       el_tmp->Hao,
                                       el_tmp->Hao,
                                       el_tmp->Fao_alp,
                                       el_tmp->Fao_bet,
                                       el_tmp->dFao_alp_dP_alp,
                                       el_tmp->dFao_alp_dP_bet,
                                       el_tmp->dFao_bet_dP_alp,
                                       el_tmp->dFao_bet_dP_bet,
                                       temp);
              if (BM) {
                bench_t[0].stop();
              }
              ...
              *el_tmp->P_bet = *P_til_bet + lamb * (0.5 * *dP);
              ...
              double en2 = energy_elec(el_tmp->P_alp,
                                       el_tmp->P_bet,
                                       el_tmp->Hao,
                                       el_tmp->Hao,
                                       el_tmp->Fao_alp,
              ...
              *el_tmp->P_bet = *P_til_bet + lamb * (0.5 * *dP);
              ...
              double en0 = energy_elec(el_tmp->P_alp,
                                       el_tmp->P_bet,
                                       el_tmp->Hao,
                                       el_tmp->Hao,
                                       el_tmp->Fao_alp,
              ...
              cout << "Interpolation polynomial = " << _a << " * lamb^2 + " << _b << " * lamb + "
                   << _c << endl;
              lamb_min = 0.0;
              if (fabs(_a) > 1e-10) {
                lamb_min = -_b / (2.0 * _a);
              ...
              if (0 < lamb_min && lamb_min < 1) {
                ...
                cout << "infinum at = " << ((en0 < en1) ? "lamb_min = 0.0" : "lamb_min = 1.0")
                     << " value = " << Eelec << endl;
              }
          ...
          *el_tmp->P_bet = *P_til_bet;  // + lamb_min * (0.5* *dP);
          ...
          Eelec = energy_elec(el_tmp->P_alp,
                              el_tmp->P_bet,
                              el_tmp->Hao,
                              el_tmp->Hao,
                              el_tmp->Fao_alp,

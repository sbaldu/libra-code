...
*/
        ...
        struct bond_interaction {
          ...
          VECTOR* f2;
          VECTOR* t1;
          VECTOR* t2;  // periodic translations of each atoms
          double K, D, r0, alpha;
        };
        struct angle_interaction {
          ...
          VECTOR* t1;
          VECTOR* t2;
          VECTOR* t3;  // periodic translations of each atoms
        ...
        int int_type;
        int is_int_type;  // Type of interaction
        int functional;
        int is_functional;  // Type of potential to use for given interaction type
        int respa_type;
        int is_respa_type;  // Type of the potential in RESPA methods (fast = 0, medium = 1, slow = 2)
      ...
      public:
        double energy;
        int is_energy;  ///< Energy for the given Hamiltonian and the status flag
        MATRIX3x3 hessian;
        int is_hessian;  ///< Hessian for the given Hamiltonian and the status flag
        MATRIX3x3 stress_at;
        int is_stress_at;  ///< atomic stress tensor and status
        MATRIX3x3 stress_fr;
        int is_stress_fr;  ///< fragmental stress tensor and status
        MATRIX3x3 stress_ml;
        int is_stress_ml;  ///< molecular stress tensor and status
        ...
        int get_type() {
          return int_type;
        }  ///< Returns the type of interaction for this Hamiltonian (bonds, angles, etc.)
        int get_status() {
          return is_active;
        }  ///< Returns the "active" status of the interaction (Hamiltonian) - if it is deactivated,
        ...
        void set_respa_type(int int_type_, int respa_type_) {
          if (int_type_ == int_type && respa_type >= 0) {
            respa_type = respa_type_;
            is_respa_type = 1;
          }
        }
        ...
        void set_2a_interaction(std::string t,
                                std::string f,
                                int,
                                int,
                                VECTOR& r1,
        ...
        void set_3a_interaction(std::string t,
                                std::string f,
                                int,
                                int,
                                int,
        ...
        void set_4a_interaction(std::string t,
                                std::string f,
                                int,
                                int,
                                int,
        ...
        void set_mb_interaction(std::string t,
                                std::string f,
                                int,
                                int*,
                                VECTOR**,
        ...
        void set_2f_interaction(std::string t,
                                std::string f,
                                int,
                                int,
                                VECTOR& r1,
...
*/
      ...
      public:
        ...
        vector<Hamiltonian_MM>
            interactions;  ///< The list of classical interaction (individual, primitive Hamiltonians)
        vector<int>
            active_interactions;  ///< The list with the indices showing which of these interactions are actually active
        ///< Note that only "active" interactions are computed when "compute" is applied

        std::string stress_opt;
        int is_stress_opt;  ///< The option that selects which type of stress to compute: "at", "fr", "ml"
        MATRIX3x3 stress_at;
        int is_stress_at;  ///< Total atomic stress and the status flag
        MATRIX3x3 stress_fr;
        int is_stress_fr;  ///< Total fragment stress and the status flag
        MATRIX3x3 stress_ml;
        int is_stress_ml;  ///< Total molecular stress and the status flag
        MATRIX3x3 hessian;
        int is_hessian;  ///< Total hessian and the status flag
        ...
        void set_atom_interactions_for_atoms(System& syst,
                                             string int_type,
                                             vector<Atom>& top_elt,
                                             vector<int>& lst1,
                                             vector<int>& lst2,
                                             ForceField& ff,
                                             int verb);
        void set_group_interactions_for_atoms(System& syst,
                                              string int_type,
        ...
        void set_interactions_for_atoms(System& syst,
                                        boost::python::list,
                                        boost::python::list,
                                        ForceField&,
                                        int verb,
                                        int assign_rings);
        void set_interactions_for_fragments(System& syst,
                                            boost::python::list,
                                            boost::python::list,
                                            ForceField&);

...
namespace liblibra {
  ...
  namespace libatomistic {
    ...
    namespace libhamiltonian_mm {
      ...
      void Interaction_2_Body::set_coords(VECTOR* r1_, VECTOR* r2_) {
        r[0] = r1_;
        r[1] = r2_;
      }
      void Interaction_2_Body::set_coords(VECTOR& r1_, VECTOR& r2_) {
      ...
      void Interaction_2_Body::set_transl(VECTOR* t1_, VECTOR* t2_) {
        t[0] = t1_;
        t[1] = t2_;
      }
      void Interaction_2_Body::set_transl(VECTOR& t1_, VECTOR& t2_) {
      ...
      void Interaction_2_Body::set_forces(VECTOR* f1_, VECTOR* f2_) {
        f[0] = f1_;
        f[1] = f2_;
      }
      void Interaction_2_Body::set_forces(VECTOR& f1_, VECTOR& f2_) {
      ...
      void Interaction_2_Body::set_charges(double* q1_, double* q2_) {
        q[0] = q1_;
        q[1] = q2_;
      }
      void Interaction_2_Body::set_charges(double& q1_, double& q2_) {
...
*/
        if (is_active) {
          ...
          if (t[0] == NULL || t[1] == NULL) {
            ...
            cout << "Warning in Bond_Interaction::compute() : translations are not set. Not using "
                    "them\n";
            r1 = *r[0];
            r2 = *r[1];
          } else {
          ...
          if (functional == 0) {
            energy += Bond_Harmonic(r1, r2, f1, f2, hess, K, r0, 2);
          } else if (functional == 1) {
            energy += Bond_Quartic(r1, r2, f1, f2, K, r0);
          } else if (functional == 2) {
            energy += Bond_Morse(r1, r2, f1, f2, D, r0, alpha);
          }
...
*/
        if (is_active) {
          ...
          if (t[0] == NULL || t[1] == NULL) {
            // if the translations are not set, they are not used (this effectively means they are all zeroes)
            cout << "Warning in VdW_Interaction::compute() : translations are not set. Not using "
                    "them\n";
            r1 = *r[0];
            r2 = *r[1];
          } else {
          ...
          if (SW > 0.0) {
            if (functional == 0) {
              en = Vdw_LJ(r1, r2, f1, f2, sigma, scale * epsilon);
            } else if (functional == 1) {
              en = Vdw_Buffered14_7(r1, r2, f1, f2, sigma, scale * epsilon);
            } else if (functional == 2) {
              en = Vdw_Morse(r1, r2, f1, f2, scale * D, r0, alpha);
            }
...
*/
        if (is_active) {
          ...
          if (t[0] == NULL || t[1] == NULL) {
            // if the translations are not set, they are not used (this effectively means they are all zeroes)
            cout << "Warning in Elec_Interaction::compute() : translations are not set. Not using "
                    "them\n";
            r1 = *r[0];
            r2 = *r[1];
          } else {
          ...
          if (SW > 0.0) {
            if (functional == 0) {
              en = Elec_Coulomb(r1, r2, f1, f2, q1, q2, eps, delta);
            }

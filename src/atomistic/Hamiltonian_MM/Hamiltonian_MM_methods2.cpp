...
*/
        ...
        for (int i = 0; i < sz; i++) {
          ...
          if (syst.Atoms[at].is_Atom_ff_type) {
            ...
            cout << "Atom i= " << i << " type is defined manually: " << syst.Atoms[at].Atom_ff_type
                 << endl;
          }
          if (!_defined) {
            ...
            if (ff_type != "none") {
              ...
              cout << "Atom i= " << i
                   << " type is defined automatically: " << syst.Atoms[at].Atom_ff_type << endl;
            } else {
              cout << "Warning!!!: The FF type of atom i= " << i << " can not be defined\n";
              cout << "Automatic determination doesn't work, so better consider defining this atom "
                      "type manually\n";
              res = 0;
            }
...
*/
      ...
      void listHamiltonian_MM::set_atom_interactions_for_atoms(System& syst,
                                                               string int_type,
                                                               vector<Atom>& top_elt,
                                                               vector<int>& lst1,
                                                               vector<int>& lst2,
...
*/
        ...
        if (x) {
          ...
          if (verb > 0) {
            ...
            cout << "In set_atom_interactions_for_atoms, mb_functional = " << ff.mb_functional
                 << endl;
            cout << "In set_atom_interactions_for_atoms, res after get_mb_parameters or "
                    "get_mb_excl_parameters = "
                 << res << endl;
          }
          //------------------------------------------------------------------------------------
          ...
          if (res) {
            ...
            if (int_type == "mb" || int_type == "mb_excl") {
              ...
              if (int_type == "mb") {
                ...
                } else if (ff.mb_functional == "vdw_LJ" || ff.mb_functional == "vdw_LJ1") {
              ...
              for (i = 0; i < nexcl; i++) {
                ...
                scale[i] = vscale[i];
                if (verb > 1) {
                  cout << "i = " << i << " excl1 = " << excl1[i] << " excl2 = " << excl2[i]
                       << " scale = " << scale[i] << endl;
                }
              }

              inter.set_mb_interaction(int_type,
                                       ff.mb_functional,
                                       sz,
                                       id,
                                       r,
      ...
      void listHamiltonian_MM::set_group_interactions_for_atoms(System& syst,
                                                                string int_type,
                                                                vector<Group>& top_elt,
                                                                vector<int>& lst1,
                                                                vector<int>& lst2,
                                                                ForceField& ff) {
        /**
        ...
        for (int b = 0; b < top_elt.size(); b++) {
          ...
          if (x) {
            ...
            if (int_type == "bond") {
              ...
              res = ff.get_bond_parameters(
                  syst.Atoms[at[0]].Atom_ff_type, syst.Atoms[at[1]].Atom_ff_type, bond_order, prms);
              ...
              res = ff.get_angle_parameters(syst.Atoms[at[0]].Atom_ff_type,
              ...
              res = ff.get_dihedral_parameters(syst.Atoms[at[0]].Atom_ff_type,
                                               syst.Atoms[at[1]].Atom_ff_type,
                                               syst.Atoms[at[2]].Atom_ff_type,
                                               syst.Atoms[at[3]].Atom_ff_type,
                                               bo12,
                                               bo23,
                                               bo34,
                                               prms);
              ...
              res = ff.get_vdw_parameters(
                  syst.Atoms[at[0]].Atom_ff_type, syst.Atoms[at[1]].Atom_ff_type, excl_pairs, prms);
              ...
              res = ff.get_elec_parameters(syst.Atoms[at[0]].Atom_ff_type,
                                           ...
                                           q1,
                                           q2,
                                           syst.Atoms[at[0]].is_Atom_charge,
                                           syst.Atoms[at[1]].is_Atom_charge,
                                           prms);
            ...
            if (res) {
              ...
              if (int_type == "bond") {
                inter.set_2a_interaction("bond",
                                         ff.bond_functional,
                                         syst.Atoms[at[0]].Atom_id,
                                         syst.Atoms[at[1]].Atom_id,
                                         syst.Atoms[at[0]].Atom_RB.rb_cm,
                                         ...
                                         syst.Atoms[at[0]].Atom_displ2,
                                         syst.Atoms[at[1]].Atom_displ2,
                                         syst.dT_2,
                                         prms);
              } else if (int_type == "angle") {
                inter.set_3a_interaction("angle",
                                         ff.angle_functional,
                                         syst.Atoms[at[0]].Atom_id,
                                         syst.Atoms[at[1]].Atom_id,
                                         syst.Atoms[at[2]].Atom_id,
                                         syst.Atoms[at[0]].Atom_RB.rb_cm,
                                         syst.Atoms[at[1]].Atom_RB.rb_cm,
                ...
                inter.set_4a_interaction("dihedral",
                                         ff.dihedral_functional,
                                         syst.Atoms[at[0]].Atom_id,
                                         syst.Atoms[at[1]].Atom_id,
                                         syst.Atoms[at[2]].Atom_id,
                                         syst.Atoms[at[3]].Atom_id,
                                         syst.Atoms[at[0]].Atom_RB.rb_cm,
                                         syst.Atoms[at[1]].Atom_RB.rb_cm,
                                         syst.Atoms[at[2]].Atom_RB.rb_cm,
                                         ...
                                         syst.Atoms[at[1]].Atom_RB.rb_force,
                                         syst.Atoms[at[2]].Atom_RB.rb_force,
                                         syst.Atoms[at[3]].Atom_RB.rb_force,
                                         prms);
              } else if (int_type == "oop") {
                inter.set_4a_interaction("oop",
                                         ff.oop_functional,
                                         syst.Atoms[at[0]].Atom_id,
                                         syst.Atoms[at[1]].Atom_id,
                                         syst.Atoms[at[2]].Atom_id,
              ...
              if (is_new_interaction(inter)) {
                ...
                if (sz == 2) {
                  ...
                  if (!is_active(syst.Atoms[at[0]], syst.Atoms[at[1]], syst.Atoms[at[2]])) {
                    inter.deactivate();
                  }
                } else if (sz == 4) {
                  if (!is_active(syst.Atoms[at[0]],
                                 syst.Atoms[at[1]],
                                 syst.Atoms[at[2]],
                                 syst.Atoms[at[3]])) {
                    inter.deactivate();
...
*/
        ...
        if (verb > 0) {
          cout << "Setting interactions with the force field " << ff.ForceField_Name << endl;
        }
        ...
        if (ff.is_mb_functional) {
          set_atom_interactions_for_atoms(syst, "mb", syst.Atoms, vlst1, vlst2, ff, verb);
        }  // Atoms
        if (ff.is_mb_excl_functional) {
          set_atom_interactions_for_atoms(syst, "mb_excl", syst.Atoms, vlst1, vlst2, ff, verb);
        }  // Atoms - exclusions

        // Setup the interactions for differen topological groups
        if (ff.is_bond_functional) {
          set_group_interactions_for_atoms(syst, "bond", syst.Bonds, vlst1, vlst2, ff);
        }  // Bonds
        if (ff.is_angle_functional) {
          set_group_interactions_for_atoms(syst, "angle", syst.Angles, vlst1, vlst2, ff);
        }  // Angles
        if (ff.is_dihedral_functional) {
          set_group_interactions_for_atoms(syst, "dihedral", syst.Dihedrals, vlst1, vlst2, ff);
        }  // Dihedrals/Torsions
        if (ff.is_oop_functional) {
          set_group_interactions_for_atoms(syst, "oop", syst.Impropers, vlst1, vlst2, ff);
        }  // OOP
        if (ff.is_vdw_functional) {
          set_group_interactions_for_atoms(syst, "vdw", syst.Pairs, vlst1, vlst2, ff);
        }  // Vdw
        if (ff.is_elec_functional) {
          set_group_interactions_for_atoms(syst, "elec", syst.Pairs, vlst1, vlst2, ff);
        }  // Electrostatic
      ...
      void listHamiltonian_MM::set_interactions_for_fragments(System& syst,
                                                              boost::python::list lst1,
                                                              boost::python::list lst2,
                                                              ForceField& ff) {
        /**
...
*/
      ...
      void listHamiltonian_MM::apply_pbc_to_interactions(
          System& syst, int int_type, int nx, int ny, int nz) {
        /**
...
*/
        ...
        if (s_int_type == "bond") {
          ...
          cout << "Error in set_respa_types: s_int_type = " << s_int_type << " is unknown\n";
          exit(0);
        }
        ...
        if (s_respa_type == "fast") {
          ...
          cout << "Error in set_respa_types: s_respa_type = " << s_respa_type << "is unknown\n";
          exit(0);
        }

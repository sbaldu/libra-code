...
namespace liblibra {
  ...
  namespace libatomistic {
    ...
    namespace libhamiltonian_mm {
      ...
      void Interaction_4_Body::set_coords(VECTOR* r1_, VECTOR* r2_, VECTOR* r3_, VECTOR* r4_) {
        r[0] = r1_;
        r[1] = r2_;
        r[2] = r3_;
        r[3] = r4_;
      ...
      void Interaction_4_Body::set_transl(VECTOR* t1_, VECTOR* t2_, VECTOR* t3_, VECTOR* t4_) {
        t[0] = t1_;
        t[1] = t2_;
        t[2] = t3_;
        t[3] = t4_;
      ...
      void Interaction_4_Body::set_forces(VECTOR* f1_, VECTOR* f2_, VECTOR* f3_, VECTOR* f4_) {
        f[0] = f1_;
        f[1] = f2_;
        f[2] = f3_;
        f[3] = f4_;
      ...
      void Interaction_4_Body::set_charges(double* q1_, double* q2_, double* q3_, double* q4_) {
        q[0] = q1_;
        q[1] = q2_;
        q[2] = q3_;
        q[3] = q4_;
...
*/
        ...
        if (f == "General0") {
          ...
          std::cout << "Warning: Dihedral functional " << f << " is not implemented\n";
        }
      }
...
*/
        if (is_active) {
          ...
          if (r[0] == NULL || r[1] == NULL || r[2] == NULL || r[3] == NULL) {
            cout
                << "Error in Dihedral_Interaction::compute() : coordinates are not set\nExiting...";
            exit(0);
          }
          ...
          if (t[0] == NULL || t[1] == NULL || t[2] == NULL || t[3] == NULL) {
            ...
            cout << "Warning in Dihedral_Interaction::compute() : translations are not set. Not "
                    "using them\n";
            r1 = *r[0];
            r2 = *r[1];
            r3 = *r[2];
          ...
          if (functional == 0) {
            energy += Dihedral_General(r1, r2, r3, r4, f1, f2, f3, f4, Vphi, phi0, n, opt);
          } else if (functional == 1) {
            energy += Dihedral_Fourier(r1, r2, r3, r4, f1, f2, f3, f4, Vphi1, Vphi2, Vphi3, opt);
          }
          ...
          if (f[0] == NULL || f[1] == NULL || f[2] == NULL || f[3] == NULL) {
            ...
            *f[3] += f4;
            ...
            MATRIX3x3 tmp;
            tmp.tensor_product(r1, f1);
            stress_at += tmp;
            tmp.tensor_product(r2, f2);
            stress_at += tmp;
            tmp.tensor_product(r3, f3);
            stress_at += tmp;
            tmp.tensor_product(r4, f4);
            stress_at += tmp;
          }
...
*/
        if (is_active) {
          ...
          if (t[0] == NULL || t[1] == NULL || t[2] == NULL || t[3] == NULL) {
            // if the translations are not set, they are not used (this effectively means they are all zeroes)
            cout << "Warning in OOP_Interaction::compute() : translations are not set. Not using "
                    "them\n";
            r1 = *r[0];
            r2 = *r[1];
            r3 = *r[2];
          ...
          if (functional == 0) {
            energy += OOP_Fourier(r1, r2, r3, r4, f1, f2, f3, f4, K, C0, C1, C2, opt);
          } else if (functional == 1) {
            energy += OOP_Wilson(r1, r2, r3, r4, f1, f2, f3, f4, K, xi_0);
          } else if (functional == 2) {
            energy += OOP_Harmonic(r1, r2, r3, r4, f1, f2, f3, f4, K);
          }

...
*/
      ...
      for (i = 0; i < sz; i++) {  // the size of A and B is assumed to be the same
        ...
        aA.push_back(mA);
        aB.push_back(mB);
        if (!is_in_vector(mA, aC)) {
          aC.push_back(mA);
        }
        if (!is_in_vector(mB, aC)) {
          aC.push_back(mB);
        }
      }  // for i
...
******************************************************************/
      ...
      for (int i = 0; i < sz; i++) {
        ...
        for (int j = 0; j < sz; j++) {
          line[j] = in[templ[i] + shift][templ[j] + shift];
        }
        out.push_back(line);
      }
    }
...
******************************************************************/
      ...
      for (i = 0; i < len(critical_params); i++) {
        ...
        if (!params.has_key(key)) {
          std::cout << "ERROR: The critical parameter " << key
                    << " must be defined!\nExiting now...\n";
          exit(0);
        }
      }

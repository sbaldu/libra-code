...
*/
      ...
      for (j = 0; j < m; j++) {
        ...
        for (i = 0; i < n; i++) {
          sum += denom_coeff.get(i, j) * flux.get(i, j);
        }
        if (fabs(sum) > 0.0) {
          for (i = 0; i < n; i++) {
            flux.scale(i, j, num_coeff.get(i, j) / sum);
          }
        }
      }  // for j
    }
...
*/
      ...
      for (b = 0; b < n; b++) {
        for (a = 0; a < n; a++) {
          N[b] += denom_coeff.get(a, b) * f.get(a, b);
        }
      }  // for b
...
*/
      ...
      for (i = 0; i < n; i++) {
        ...
        if (opt == 0) {
          ...
          for (j = 0; j < n; j++) {
            gradA.set(i, j, 2.0 * mu.get(i, 0) * b[i]);
          }
...
*/
      ...
      for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
          ...
          if (opt2 == 0) {
            if (i != j) {
              sum += J.get(i, j) * J.get(i, j) * sij * sij;
            }
          } else {
            sum += J.get(i, j) * J.get(i, j) * sij * sij;
          }
    ...
    double derivs4(
        MATRIX& x, MATRIX& y, MATRIX& A, MATRIX& gradA, MATRIX& mu, MATRIX& gradmu, int opt) {
      /**
      ...
      for (i = 0; i < n; i++) {
        ...
        if (opt == 0) {
          ...
          for (j = 0; j < n; j++) {
            ...
            for (k = 0; k < n; k++) {
              val += (mu.get(i, 0) * b[i] - mu.get(k, 0) * b[k]) * J.get(k, j);
            }
          ...
          for (j = 0; j < n; j++) {
            ...
            for (k = 0; k < n; k++) {
              val += (mu.get(i, 0) * mu.get(i, 0) * b[i] - mu.get(k, 0) * mu.get(k, 0) * b[k]) *
                     J.get(k, j);
            }

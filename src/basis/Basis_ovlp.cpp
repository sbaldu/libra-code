...
namespace liblibra {
  ...
  namespace libbasis {
    ...
    void update_overlap_matrix(int x_period,
                               int y_period,
                               int z_period,
                               const VECTOR& t1,
                               const VECTOR& t2,
            ...
            for (int ny = -y_period; ny <= y_period; ny++) {
              for (int nz = -z_period; nz <= z_period; nz++) {
                ...
                if (i == j) {
                  ...
                  Sao.M[i * Norb + j] +=
                      gaussian_overlap(basis_ao[i], tmp_ao);  //,0,dIdA,dIdB,aux,n_aux,TV);
                } else {
                  basis_ao[j].shift_position(TV);
                  Sao.M[i * Norb + j] +=
                      gaussian_overlap(basis_ao[i], basis_ao[j]);  //,0,dIdA,dIdB,aux,n_aux,TV);
                  basis_ao[j].shift_position(-TV);
...
*/
      ...
      MATRIX* ci;
      ci = new MATRIX(Nbas_i, Norb_i_act);
      MATRIX* cj;
      cj = new MATRIX(Nbas_j, Norb_j_act);
      MATRIX* Sao;
      Sao = new MATRIX(Nbas_i, Nbas_j);
        ...
        for (j = 0; j < Nbas_j; j++) {
          ...
          if (dist2 < max_d2) {
            double res = gaussian_overlap(
                ao_i[i], ao_j[j], is_normalize, is_derivs, dIdA, dIdB, auxd, n_aux);
            Sao->set(i, j, res);
          } else {
            Sao->set(i, j, 0.0);
          }
      ...
      Smo =
          (*ci).T() * (*Sao) *
          (*cj);  // ( Norb_i_act x Nbas_i ) x (Nbas_i x Nbas_j) x (Nbas_j x Norb_j_act) = Norb_i_act x Norb_j_act
...
*/
      ...
      CMATRIX* ci;
      ci = new CMATRIX(Nbas_i, Norb_i_act);
      CMATRIX* cj;
      cj = new CMATRIX(Nbas_j, Norb_j_act);
      CMATRIX* Sao;
      Sao = new CMATRIX(Nbas_i, Nbas_j);
...
*/
      ...
      if (Nbas_i != Nbas_j) {
        cout << "Error in MO_overlap: The number of rows of the MO-LCAO matrix Ci " << Ci.n_rows
             << " is not equal to the number of rows of the MO-LCAO matrix Cj " << Cj.n_rows
             << endl;
        cout << "Exiting...";
        exit(0);
      }
      ...
      Smo = (*ci).T() *
            (*cj);  // ( Norb_i_act x Nbas_i ) x (Nbas_j x Norb_j_act) = Norb_i_act x Norb_j_act
...
*/
      ...
      if (sd_i.N_bas != sd_j.N_bas) {
        cout << "Error in SD_overlap: The number of basis functions in which MOs of the SD sd_i "
                "are expanded "
             << sd_i.N_bas
             << " is not equal to the number of basis functions in which MOs of the SD sd_j are "
                "expanded "
             << sd_j.N_bas << endl;
        cout << "Exiting...";
        exit(0);
      ...
      CMATRIX* Smo;
      Smo = new CMATRIX(sd_i.N, sd_j.N);  // square matrix
      *Smo = sd_i.get().H() *
             sd_j.get();  // ( sd_i.N x sd_i.N_bas ) x (sd_j.N_bas x sd_j.N) = sd_i.N x sd_j.N

      // Now, apply spin considerations
      for (int i = 0; i < sd_i.N; i++) {
        for (int j = 0; j < sd_j.N; j++) {
          if (sd_i.spin[i] != sd_j.spin[j]) {
            Smo->set(i, j, 0.0, 0.0);
          }
        }  // for j
      }  // for i
...
*/
      ...
      if (SD_ovlp.n_rows != Ni) {
        std::cout
            << "Error in SD_overlap : the # of rows of the output matrix, SD_ovlp ( "
            << SD_ovlp.n_rows
            << " ) is not equal to the number of Slater Determinants in the first (left) set ( "
            << Ni << " )\n";
        exit(0);
      }
      if (SD_ovlp.n_cols != Nj) {
        std::cout
            << "Error in SD_overlap : the # of cols of the output matrix, SD_ovlp ( "
            << SD_ovlp.n_cols
            << " ) is not equal to the number of Slater Determinants in the second (right) set ( "
            << Nj << " )\n";
        exit(0);
      }

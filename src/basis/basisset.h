...
#define BASIS_FUNC_POLY_MAX_DEGREE 5
    ...
    public:
      ...
      friend int operator!=(const basisset_shell_struct& m1, const basisset_shell_struct& m2) {
        ...
        if (m1.contrCount != m2.contrCount) {
          return 1;
        }
        if (m1.shell_ID != m2.shell_ID) {
          return 1;
        }
        if (m1.exponentList.size() != m2.exponentList.size()) {
          return 1;
        }
        if (m1.coeffList.size() != m2.coeffList.size()) {
          return 1;
        }

        int sz = m1.exponentList.size();
        for (int i = 0; i < sz; i++) {
          if (m1.exponentList[i] != m2.exponentList[i]) {
            return 1;
          }
          if (m1.coeffList[i] != m2.coeffList[i]) {
            return 1;
          }
        }
    ...
    public:
      ...
      basisset_atom_struct(int max_no_of_shells_per_atom, int max_no_of_contr) {
        noOfShells = 0;
        shells = std::vector<basisset_shell_struct>(max_no_of_shells_per_atom,
                                                    basisset_shell_struct(max_no_of_contr));
      }
      ...
      friend int operator!=(const basisset_atom_struct& m1, const basisset_atom_struct& m2) {
        if (m1.noOfShells != m2.noOfShells) {
          return 1;
        }

        int sz = m1.shells.size();
        for (int i = 0; i < sz; i++) {
          if (m1.shells[i] != m2.shells[i]) {
            return 1;
          }
        }
    ...
    public:
      ...
      basisset_struct(int max_no_of_atom_types,
                      int max_no_of_shells_per_atom,
                      int max_no_of_contr) {
        ...
        atoms = std::vector<basisset_atom_struct>(
            max_no_of_atom_types, basisset_atom_struct(max_no_of_shells_per_atom, max_no_of_contr));
      }

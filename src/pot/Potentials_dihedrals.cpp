  ...
  namespace libpot {
    ...
    double Dihedral_Fourier(VECTOR& ri,
      ...
      if ((modt != 0.0) && (modu != 0.0)) {
        ...
        // Energy and forces
        K1 = Vphi1 * (1.0 + cos(phi)) + Vphi2 * (1.0 - cos(2.0 * phi)) +
             Vphi3 * (1.0 + cos(3.0 * phi));
        K2 = -Vphi1 * sin(phi) + 2.0 * Vphi2 * sin(2.0 * phi) -
             3.0 * Vphi3 * sin(3.0 * phi);  // d(K1)/d(phi))

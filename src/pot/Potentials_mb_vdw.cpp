...
*/
              ...
              for (int j = 0; j < sz; j++) {
                ...
                if (na == 0 && nb == 0 && nc == 0 && i == j) {
                  ...
                  if (SW > 0.0) {
                    ...
                    VECTOR dpref = -(Qij * (6.0 * a_minus6 + 4.0 * a_minus4 + a_minus2) /
                                     (a * a * etha * etha)) *
                                   rij;
        ...
        for (int Ly = -rec_deg; Ly <= rec_deg; Ly++) {
          for (int Lz = -rec_deg; Lz <= rec_deg; Lz++) {
            ...
            if ((Lx == 0) && (Ly == 0) && (Lz == 0)) {
              ...
              fact = ((exp(-b * b)) * (0.5 / (b * b) - 1.0) / b + sqrt_M_PI * ERFC(b)) /
                     (hmod * hmod * hmod);
              ...
              for (i = 0; i < sz; i++) {
                for (j = 0; j < sz; j++) {
                  f_mod = h;
                  f_mod *= -const2 * Bij[types[i] * max_type + types[j]] *
                           sin(h * (r[i] - r[j]));  // -dE2/dr_i
                  f[i] += f_mod;
                  f[j] -= f_mod;
      ...
      double E3 = 0.0;
      for (i = 0; i < sz; i++) {
        E3 += Bij[types[i] * max_type + types[i]];
      }
...
*/
          ...
          for (int im = 0; im < n_images; im++) {
            ...
            if ((na == xshift) && (nb == yshift) && (nc == zshift) &&
                (i == j)) { /* skip this - atom interacts with itself*/
            } else {
              if ((na == xshift) && (nb == yshift) && (nc == zshift)) {
                scl1 = scl;
              } else {
              ...
              if (scl1 > 0.0) {
                ...
                if (SW > 0.0) {
                  ...
                  tp.tensor_product(rij, f12);
                  at_stress += tscale * tp;
                  tp.tensor_product(gij, f12);
                  fr_stress += tscale * tp;
                }
              }  // scl1>0.0
...
*/
        ...
        for (b = minb.n2; b <= maxb.n2; b++) {
          for (c = minb.n3; c <= maxb.n3; c++) {
            for (i = 0; i < Nat; i++) {
              ...
              locqt.n3 = c;
              globqt.push_back(
                  locqt);  // R[indx] = tmp[i] + (*box)*(globqt[indx]), that is globqt[indx] is an integer vector
      ...
      vector<vector<int> > cell2at(
          Ncells, dummy);  // cell2at[i] - contains complex indexes of all atoms in cell i

      // Calculate neighbors of each cell (sub-cell)
      // we use serial indexes of both central cell and its neighbors
      vector<vector<int> > neibc(Ncells,
                                 dummy);  // indexes of neighboring cells for given cell index
      ...
      for (i = 0; i < indx; i++) {
        ...
        if (c >= Ncells) {
          cout << "This can not be!!! Dynamical error: c>=Ncells - array not allocated.\n";
          exit(0);
        }
      ...
      int cc = (maxb.n3 - minb.n3 + 1) * (maxb.n2 - minb.n2 + 1) * (-minb.n1) +
               ...
               (-minb.n3);  // index of central image/cell (not sub-cell)
      ...
      for (int at_indx1 = 0; at_indx1 < Nat; at_indx1++) {
        ...
        for (int excl = 0; excl < excl_scales.size(); excl++) {
          if (excl_scales[excl][0].at_indx1 == at_indx1) {
            excl_indx = excl;
            break;
          }
        }
        ...
        for (c = 0; c < sz1; c++) {
          ...
          for (int a = 0; a < sz2; a++) {  // iterations over atoms in cell ci2
            ...
            int at_indx2 =
                i2 % Nat;  // real index of atom, corresponding to the atom with complex index i2
              ...
              if (modR <= Roff2) {
                ...
                if (modR <= Roff2) {
                  ...
                  if (modR <= Roff2) {
                    ...
                    if (excl_indx > -1) {
                      for (excl = 0; excl < excl_scales[excl_indx].size(); excl++) {
                        if (excl_scales[excl_indx][excl].at_indx2 == at_indx2) {
                          is_scaled = 1;
                          scl = scl_const * excl_scales[excl_indx][excl].scale;
                          break;
                        }
                      }
                    }
                    ...
                    //============= Calculation part =========================
                    if ((is_central == 1) &&
                        (at_indx1 == at_indx2)) { /* skip this - singular case*/
                    } else {
                      ...
                      if (scl1 * scl1 > 0.0) {
                        ...
                        if (is_cutoff) {
                          SWITCH(r[at_indx1], rj, R_on, R_off, SW, dSW);
                        }
                        if (SW > 0.0) {
                          f1 = f2 = 0.0;
...
**********************************************************************************/
      ...
      for (int excli = 0; excli < excl_scales.size(); excli++) {
        for (int exclij = 0; exclij < excl_scales[excli].size(); exclij++) {
          ...
          //============= Calculation part =========================
          if (at_indx1 == at_indx2) {
          }  // skip this - singular case
          else {
            if (scl1 > 0.0) {
              ...
              dSW = 0.0;
              if (is_cutoff) {
                SWITCH(r[at_indx1], r[at_indx2], R_on, R_off, SW, dSW);
              }
              if (SW > 0.0) {
                f1 = f2 = 0.0;
                sig = (sigma[at_indx1] * sigma[at_indx2]);

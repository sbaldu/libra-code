...
*/
      ...
      if (ham_dia_mem_status == 0) {
        cout << "Error in generate_m_matrices(): the diabatic Hamiltonian matrix is not allocated \
...
*/
      ...
      if (ham_dia_mem_status == 0) {
        cout << "Error in kcrpmd_effective_potential(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
        exit(0);
      }

      if (ndia != 2) {
        cout << "Error in kcrpmd_effective_potential(): implementation only for ndia=2\n";
        exit(0);
      }
      ...
      if (children.size() == 1 and ntraj == 1) {
        ...
        if (beta * K > 1e-3) {
          VKP = Vg -
                log(1 + exp(-beta * (Ve - Vg)) - exp(-beta * (V0 - Vg)) - exp(-beta * (V1 - Vg))) /
                    beta;
        } else if (beta * abs(V0 - V1) > 1e-7) {
          VKP = 0.5 * (V0 + V1) -
                log(pow(beta * K, 2) * sinh(0.5 * beta * (V0 - V1)) / (0.5 * beta * (V0 - V1))) /
                    beta;
        } else {
        ...
        if ((beta * K - 1) < 0.0) {
          ...
          C = 1 + (eta * sqrt(a / 3.1415) / sqrt(1 + exp(2 * c * (beta * K - 1))) - 1) /
                      (1 + exp(2 * d * (beta * K - 1)));
        } else {
          ...
          C = 1 + (eta * sqrt(a / 3.1415) * exp(-c * (beta * K - 1)) /
                   ...
                   1) *
                      exp(-2 * d * (beta * K - 1)) / (1 + exp(-2 * d * (beta * K - 1)));
        }
...
*/
  ...
  but it is needed for the calculations\n";
      ...
      if (children.size() == 1 and ntraj == 1) {
        ...
        Fg = 0.5 * (F0 + F1) -
             ((V0 - V1) * (F0 - F1) + 4 * K * FK) / (2 * sqrt(pow(V0 - V1, 2) + pow(2 * K, 2)));
        Fe = 0.5 * (F0 + F1) +
             ((V0 - V1) * (F0 - F1) + 4 * K * FK) / (2 * sqrt(pow(V0 - V1, 2) + pow(2 * K, 2)));
        if (beta * K > 1e-3) {
          FKP = (Fg + exp(-beta * (Ve - Vg)) * Fe - exp(-beta * (V0 - Vg)) * F0 -
                 exp(-beta * (V1 - Vg)) * F1) /
                (1 + exp(-beta * (Ve - Vg)) - exp(-beta * (V0 - Vg)) - exp(-beta * (V1 - Vg)));
        } else if (beta * abs(V0 - V1) > 1e-7) {
          FKP = 0.5 * (F0 + F1) +
                (F0 - F1) * (1 / (beta * (V0 - V1)) -
                             0.5 * cosh(0.5 * beta * (V0 - V1)) / sinh(0.5 * beta * (V0 - V1))) -
                2 * FK / (beta * K);
        } else {
          ...
          FC = -beta *
               (eta * sqrt(a / 3.1415) * c * exp(2 * c * (beta * K - 1)) /
                    (pow(sqrt(1 + exp(2 * c * (beta * K - 1))), 3) *
                     (1 + exp(2 * d * (beta * K - 1)))) +
                (eta * sqrt(a / 3.1415) / sqrt(1 + exp(2 * c * (beta * K - 1))) - 1) * 2 * d *
                    exp(2 * d * (beta * K - 1)) / pow((1 + exp(2 * d * (beta * K - 1))), 2)) *
               FK;
        } else {
          FA = -2 * beta * a * c * exp(-2 * c * (beta * K - 1)) /
               pow((1 + exp(-2 * c * (beta * K - 1))), 2) * FK;
    ...
    vector<double> nHamiltonian::kcrpmd_effective_auxiliary_force(
        vector<double>& y_aux_var, double beta, double eta, double a, double b, double c, double d) {
      /**

...
*/
      ...
      for (traj_k = 0; traj_k < ntraj; traj_k++) {
        ...
        MATRIX d1rho_k(
            ndof,
            1);  // derivatives of the total density at the position of trajectory k (Q_k) w.r.t. q_{dof}
        MATRIX d2rho_k(
            ndof,
            1);  // second derivatives of the total density at the position of trajectory k (Q_k) w.r.t. q_{dof}
      ...
      for (traj_k = 0; traj_k < ntraj; traj_k++) {
        for (traj_j = 0; traj_j < ntraj; traj_j++) {
          ...
          for (dof_i = 0; dof_i < ndof; dof_i++) {
            ...
            d2rho.add(dof_i,
                      traj_k,
                      (4.0 * alp * alp * dq.get(dof_i) * dq.get(dof_i) - 2.0 * alp) * g_kj);
          }  // for dof
      ...
      for (traj_k = 0; traj_k < ntraj; traj_k++) {
        for (traj_j = 0; traj_j < ntraj; traj_j++) {
          for (dof_i = 0; dof_i < ndof; dof_i++) {
            A.add(0,
                  traj_k,
                  d1rho.get(dof_i, traj_j) * invM.get(dof_i, 0) * d1rho.get(dof_i, traj_j));
        ...
        for (dof_a = 0; dof_a < ndof; dof_a++) {
          for (traj_k = 0; traj_k < ntraj; traj_k++) {
            ...
            for (dof_i = 0; dof_i < ndof; dof_i++) {
              ...
              for (traj_j = 0; traj_j < ntraj; traj_j++) {
                if (traj_n == traj_j || traj_n == traj_k) {
                  double pref = -2.0 * alp * (q.get(dof_i, traj_k) - q.get(dof_i, traj_j)) *
            ...
            double val_k = ((dAk_dQan + 2.0 * Bk * drhok_dQan) * rhok2 -
      ...
      for (traj_n = 0; traj_n < ntraj; traj_n++) {
        for (dof_a = 0; dof_a < ndof; dof_a++) {
          for (traj_k = 0; traj_k < ntraj; traj_k++) {
            ...
            double val_k = ((dAk_dPan + 2.0 * Bk * drhok_dPan) * rhok2 -
...
*/
    ...
    void nHamiltonian::add_ethd3_adi(
        const MATRIX& q, const MATRIX& p, const MATRIX& invM, double alp, double bet, int der_lvl) {
      /**
  Add ETHD3 energies and (optionally) forces to all the children Hamiltonians in the adiabatic representation
*/

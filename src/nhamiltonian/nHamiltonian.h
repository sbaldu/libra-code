...
namespace liblibra {
  ...
  namespace libnhamiltonian {
    ...
    void check_vector_dimensions(vector<CMATRIX*> ptx,
                                 vector<CMATRIX>& x_,
                                 vector<int>& x_mem_status,
                                 int nnucl);
    ...
    void set_X2_by_ref(vector<CMATRIX*> ptx,
                       vector<CMATRIX>& x_,
                       vector<int>& x_mem_status,
                       int nrows,
                       int ncols,
                       int nnucl);
    void set_X2_by_val(vector<CMATRIX*> ptx,
                       vector<CMATRIX>& x_,
                       vector<int>& x_mem_status,
                       int nrows,
                       int ncols,
...
*/
      ...
      void check_cmatrix_list(
          bp::object obj, std::string matrix_name, int nrows, int ncols, int length);
  ...
  */
      ...
      CMATRIX*
          nac_dia;  ///< Nonadiabatic couplings (time-derivative couplings) in diabatic representation
      ...
      vector<CMATRIX*>
          d1ham_dia;  ///< first order derivatives of the Hamiltonian matrix in the diabatic basis
      vector<int> d1ham_dia_mem_status;

      vector<CMATRIX*>
          d2ham_dia;  ///< second order derivatives of the Hamiltonian in the diabatic basis
  ...
  */
      ...
      CMATRIX*
          nac_adi;  ///< Nonadiabatic couplings (time-derivative couplings) in adiabatic representation
      ...
      vector<CMATRIX*>
          d1ham_adi;  ///< first order derivatives of the Hamiltonian matrix in the adiabatic basis (diagonal)
      vector<int> d1ham_adi_mem_status;

      vector<CMATRIX*>
          d2ham_adi;  ///< second order derivatives of the Hamiltonian matrix in the adiabatic basis
  ...
  */
      ...
      CMATRIX*
          basis_transform;  ///< These are the eigenvectors of the generalized eigenvalue problem for diabatic Hamiltonian
      ...
      vector<int>*
          ordering_adi;  ///< the permutation describing how the indices of the internally-stored
                         /// properties like ham_adi or nac_adi relate to the indices of the
                         /// actual "physical" states.

      CMATRIX*
          cum_phase_corr;  /// These are the cumulative phase corrections applied to basis_transform (this correction is indexed by
  ...
  **/
      ...
      double
          phase_corr_ovlp_tol;  /// phase correction overlap tolerance, we only compute phase corrections
  ...
  **/
      ...
      void add_child(
          nHamiltonian& child);   ///< Associate an existing Hamiltonian with the present one
                                  ///< to become its child
      ...
      void compute_diabatic(int model,
                            vector<double>& q,
      ...
      void compute_diabatic(bp::object py_funct,
                            MATRIX& q,
                            bp::object params,
      ...
      void add_ethd3_adi(const MATRIX& q,
                         const MATRIX& p,
                         const MATRIX& invM,
                         double alp,
                         double bet,
...
*/
      ...
      void compute_adiabatic(bp::object py_funct,
                             MATRIX& q,
                             bp::object params,
                             int lvl);  // for models defined in Python
      void compute_adiabatic(bp::object py_funct,
      ...
      CMATRIX forces_adi(
          vector<int>& act_states);  // -dH_adi/dR in the adiabatic basis for several trajectories
      CMATRIX forces_dia(
          vector<int>& act_states);  // -dH_dia/dR in the diabatic basis for several trajectories
...
*/
      ...
      CMATRIX Ehrenfest_forces_dia_unit(CMATRIX& ampl_dia,
      ...
      CMATRIX Ehrenfest_forces_adi_unit(CMATRIX& ampl_adi,
                                        int option,
                                        CMATRIX& transform,
                                        double gamma);  ///< Ehrenfest forces in adiabatic basis
      CMATRIX Ehrenfest_forces_adi_unit(
          CMATRIX& ampl_adi,
          int option,
          CMATRIX& transform);  ///< Ehrenfest forces in adiabatic basis
      CMATRIX Ehrenfest_forces_adi_unit(CMATRIX& ampl_adi,
      ...
      CMATRIX Ehrenfest_forces_adi(CMATRIX& ampl_adi,
                                   int lvl,
                                   int option,
                                   vector<CMATRIX*>& transforms,
                                   vector<double>& gammas);  ///< Ehrenfest forces in adiabatic basis
      CMATRIX Ehrenfest_forces_adi(
          CMATRIX& ampl_adi,
          int lvl,
          int option,
          vector<CMATRIX*>& transforms);  ///< Ehrenfest forces in adiabatic basis
      CMATRIX Ehrenfest_forces_adi(CMATRIX& ampl_adi,
                                   int lvl,
                                   int option);  ///< Ehrenfest forces in adiabatic basis
      CMATRIX Ehrenfest_forces_adi(CMATRIX& ampl_adi,
      ...
      vector<CMATRIX> Ehrenfest_forces_tens_adi(
          CMATRIX& ampl_adi);  // Force tensor in adiabatic basis, assuming Cadi = Cadi(t)
      vector<CMATRIX> Ehrenfest_forces_tens_adi(
          CMATRIX& ampl_adi,
          vector<int>& id_);  // Force tensor in adiabatic basis, assuming Cadi = Cadi(t)
      vector<CMATRIX> Ehrenfest_forces_tens_dia(
          CMATRIX& ampl_dia);  // Force tensor in diabatic basis, assuming Cdia = Cdia(t)
      vector<CMATRIX> Ehrenfest_forces_tens_dia(
          CMATRIX& ampl_dia,
          vector<int>& id_);  // Force tensor in diabatic basis, assuming Cdia = Cdia(t)
      ...
      CMATRIX QTSH_forces_dia_unit(CMATRIX& ampl_dia,
                                   int option);         ///< QTSH forces in diabatic basis
      CMATRIX QTSH_forces_dia_unit(CMATRIX& ampl_dia);  ///< QTSH forces in diabatic basis
      CMATRIX QTSH_forces_dia(CMATRIX& ampl_dia,
                              int lvl,
                              int option);                  ///< QTSH forces in diabatic basis
      ...
      CMATRIX QTSH_forces_adi_unit(CMATRIX& ampl_adi,
                                   int option,
                                   CMATRIX& transform);  ///< QTSH forces in adiabatic basis
      CMATRIX QTSH_forces_adi_unit(CMATRIX& ampl_adi,
                                   int option);         ///< QTSH forces in adiabatic basis
      CMATRIX QTSH_forces_adi_unit(CMATRIX& ampl_adi);  ///< QTSH forces in adiabatic basis
      CMATRIX QTSH_forces_adi(CMATRIX& ampl_adi,
                              int lvl,
                              int option,
                              vector<CMATRIX*>& transforms);  ///< QTSH forces in adiabatic basis
      CMATRIX QTSH_forces_adi(CMATRIX& ampl_adi,
                              int lvl,
                              int option);                  ///< QTSH forces in adiabatic basis
      ...
      double kcrpmd_effective_potential(vector<double>& y_aux_var,
                                        const MATRIX& q,
                                        const MATRIX& invM,
                                        double beta,
                                        double eta,
                                        ...
                                        double c,
                                        double d);
      MATRIX kcrpmd_effective_force(vector<double>& y_aux_var,
                                    const MATRIX& q,
                                    const MATRIX& invM,
                                    double beta,
                                    double eta,
      ...
      friend bool operator==(const nHamiltonian& h1, const nHamiltonian& h2) { return &h1 == &h2; }
    ...
    typedef std::vector<nHamiltonian>
        nHamiltonianList;  ///< data type for keeping a list of generic Hamiltonians of their derived classes
...
*/
    ...
    MATRIX ETHD3_forces(const MATRIX& q,
                        const MATRIX& p,
                        const MATRIX& invM,
                        double alp,
                        double bet);  // -dH_{ETHD3}/dQ

    MATRIX ETHD3_friction(const MATRIX& q,
                          const MATRIX& p,
                          const MATRIX& invM,
                          double alp,
                          double bet);  // dH_{ETHD3}/dP

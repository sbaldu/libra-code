...
*/
      ...
      if (ovlp_dia_mem_status == 0) {
        cout << "Error in QTSH_energy_dia(): the overlap matrix in the diabatic basis is not allocated \
  but it is needed for the calculations\n";
        exit(0);
      }

      if (ham_dia_mem_status == 0) {
        cout << "Error in QTSH_energy_dia(): the diabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
        exit(0);
      }
...
//  not allocated, but they are needed for the calculations\n"; exit(0); }
      ...
      CMATRIX* H_diag;
      H_diag = new CMATRIX(nadi, nadi);
      CMATRIX* V;
      V = new CMATRIX(nadi, nadi);
      for (int i = 0; i < nadi; i++) {
        H_diag->set(i, i, (*ham_dia).get(i, i));
      }
      *V = *ham_dia - *H_diag;

      complex<double> norm = (ampl_dia.H() * (*ovlp_dia) * ampl_dia).M[0];
      complex<double> res =
          (ampl_dia.H() * ((*V) + complex<double>(0.0, -1.0) * (*nac_dia)) * ampl_dia).M[0] / norm;
...
*/
      if (id_.size() == 1) {
        if (id_[0] == id) {
          return QTSH_energy_dia(ampl_dia);
        } else {
...
*/
      ...
      if (ham_adi_mem_status == 0) {
        cout << "Error in QTSH_energy_adi(): the adiabatic Hamiltonian matrix is not allocated \
  but it is needed for the calculations\n";
        exit(0);
      }
...
//  not allocated, but they are needed for the calculations\n"; exit(0); }
      ...
      return (ampl_adi.H() * (T.H() * complex<double>(0.0, -1.0) * (*nac_adi) * T) * ampl_adi).M[0] /
             norm;
    }
...
*/
      if (id_.size() == 1) {
        if (id_[0] == id) {
          return QTSH_energy_adi(ampl_adi);
        } else {
          cout << "ERROR in force_dia: No Hamiltonian matching the requested id\n";
          exit(0);
        }

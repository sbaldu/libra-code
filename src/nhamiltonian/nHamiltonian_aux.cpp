...
namespace liblibra {
  ...
  namespace libnhamiltonian {
    ...
    void check_mat_dimensions(CMATRIX* x, int nrows, int ncols) {
      ...
      if (x->n_rows != nrows) {
        cout << "Error in check_mat_dimensions: The number of rows of the matrix object "
             << x->n_rows << "\
    does not match the expected dimensionality of "
      ...
      if (x->n_cols != ncols) {
        cout << "Error in check_mat_dimensions: The number of cols of the matrix object "
             << x->n_cols << "\
    does not match the expected dimensionality of "
             << ncols << endl;
        exit(0);
...
*/
      if (ptx.size() != x_.size()) {
        cout << "Error in check_vector_dimensions: the size of the ptx (" << ptx.size()
             << ") is not equal to the \
    size of x_ ("
             << x_.size() << ")\n";
        exit(0);
...
*/
    ...
    void nHamiltonian::check_cmatrix_list(
...
*/
      ...
      if (tmp.size() != length) {
        cout << "ERROR in nHamiltonian...\n";
        cout << "The number of elements in the extracted " << matrix_name << " object ("
             << tmp.size() << ")"
      ...
      if (tmp.size() > 0) {
        if (tmp[0].n_rows != nrows) {
          cout << "ERROR in nHamiltonian...\n";
          cout << "The number of rows in the extracted " << matrix_name << "[0] object ("
               << tmp[0].n_rows << ")"
               << " is not equal to the expected number of rows = " << nrows << "\nExiting...\n";
          exit(0);
        }

...
class Qeq {
  //***************** Variables **************
  std::string idempotential;
  int is_idempotential;  // Way to get idempotential values (parameters)
  // allowed values = rappe_goddard, parr_pearson, bakowies_thiel
  int solution_method;
  int is_solution_method;  // Solution method (which algorithm to use)
  int integral_method;
  int is_integral_method;  // How we calculate Jab integrals
  // allowed values = 0, 1, 2, 3
  double epsilon;
  int is_epsilon;  // Dielectric constant
  double threshold;
  int is_threshold;  // Convergence precision
  int MaxCount;
  int is_MaxCount;  // Maximal number of iterations
...
double Coulomb_Integral(
    double& R, int i, int j, double& qi, double& qj, vector<_GMP>& GMP, double& epsilon, int& mode);

...
void Qeq::show_info() {
  ...
  if (is_idempotential) {
    std::cout << "idempotential = " << idempotential << std::endl;
  }
  if (is_solution_method) {
    std::cout << "solution_method = " << solution_method << std::endl;
  }
  if (is_integral_method) {
    std::cout << "integral_method = " << integral_method << std::endl;
  }
  if (is_epsilon) {
...
void Qeq::init_variables() {
  // Default parameters
  idempotential = "parr_pearson";
  is_idempotential = 1;
  solution_method = 0;
  is_solution_method = 1;  // standard QEq methods (no dJad/aq derivatives)
  integral_method = 7;
  is_integral_method = 1;  // Louwen-Vogt equation
  epsilon = 2.0;
  is_epsilon = 1;
  threshold = 0.001;
  is_threshold = 1;
...
void Qeq::copy_content(const Qeq& qeq) {
  if (qeq.is_idempotential) {
    idempotential = qeq.idempotential;
    is_idempotential = 1;
  }
  if (qeq.is_solution_method) {
    solution_method = qeq.solution_method;
    is_solution_method = 1;
  }
  if (qeq.is_integral_method) {
    integral_method = qeq.integral_method;
    is_integral_method = 1;
  }
  if (qeq.is_epsilon) {
    epsilon = qeq.epsilon;
    is_epsilon = 1;
  ...
  if (qeq.is_threshold) {
    threshold = qeq.threshold;
    is_threshold = 1;
  }
  if (qeq.is_MaxCount) {
    MaxCount = qeq.MaxCount;
    is_MaxCount = 1;
  }
}

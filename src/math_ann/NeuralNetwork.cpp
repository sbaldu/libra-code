...
*/
      ...
      MATRIX w(Npe[0], Npe[0]);
      w.Init_Unit_Matrix(1.0);  // weights
      MATRIX b(Npe[0], 1);
      b = 0.0;  // biases
      MATRIX d(Npe[0], 1);
      d = 0.0;  // just general matrices
    ...
    NeuralNetwork::NeuralNetwork(vector<int>& arch) { allocate(arch); }
...
*/
    ...
    void NeuralNetwork::load(boost::property_tree::ptree& pt, std::string path, int& status) {
      ...
      libio::load(pt, path + ".Nlayers", Nlayers, st);
      if (st == 1) {
        status = 1;
      }
      libio::load(pt, path + ".Npe", Npe, st);
      if (st == 1) {
        status = 1;
      }
      libio::load(pt, path + ".sz_x", sz_x, st);
      if (st == 1) {
        status = 1;
      }
      libio::load(pt, path + ".sz_y", sz_y, st);
      if (st == 1) {
        status = 1;
      }

      liblinalg::load(pt, path + ".B", B, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".grad_b", grad_b, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".grad_b_old", grad_b_old, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".dB", dB, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".dBold", dBold, st);
      if (st == 1) {
        status = 1;
      }

      liblinalg::load(pt, path + ".W", W, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".grad_w", grad_w, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".grad_w_old", grad_w_old, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".dW", dW, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".dWold", dWold, st);
      if (st == 1) {
        status = 1;
      }

      liblinalg::load(pt, path + ".D", D, st);
      if (st == 1) {
        status = 1;
      }
      liblinalg::load(pt, path + ".Delta", Delta, st);
      if (st == 1) {
        status = 1;
      }
    }

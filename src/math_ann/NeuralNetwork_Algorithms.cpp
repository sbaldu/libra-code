...
namespace liblibra {
  ...
  namespace libann {
    ...
    void NeuralNetwork::init_weights_biases_normal(
        Random& rnd, double scaling_w, double shift_w, double scaling_b, double shift_b) {
    ...
    void NeuralNetwork::init_weights_biases_uniform(
        Random& rnd, double left_w, double right_w, double left_b, double right_b) {
      for (int L = 1; L < Nlayers; L++) {
        for (int i = 0; i < Npe[L]; i++) {
          ///========== Weights ============
          for (int j = 0; j < Npe[L - 1]; j++) {
...
*/
      ...
      if (input.n_rows != sz_x) {
        std::cout << "Error: Size of the input " << input.n_rows
                  << " does not match the ANN architecture " << sz_x << std::endl;
        exit(0);
      }
  ...
  */
      ...
      for (p = 0; p < sz; p++) {
        ...
        for (L = 1; L < Nlayers; L++) {
          ...
          for (i = 0; i < Npe[L]; i++) {
            D_L.set(i, i, 1.0 - Y[L].get(i, p) * Y[L].get(i, p));
          }
...
*/
      ...
      if (target.n_rows != sz_y) {
        std::cout << "Error: Size of the target output " << target.n_rows
                  << " does not match the ANN architecture " << sz_y << std::endl;
        exit(0);
      }
  ...
  */
      ...
      /// Allocate deltas:
      for (L = 0; L < Nlayers; L++) {
        delta.push_back(MATRIX(Npe[L], sz));
      }
...
*/
      ...
      if (target.n_cols != inputs.n_cols) {
        std::cout << "Error: The number of patterns is different for inputs " << inputs.n_cols
                  << " and targets " << target.n_cols << std::endl;
        exit(0);
      }
  ...
  */
    ...
    vector<double> NeuralNetwork::train(Random& rnd,
                                        bp::dict params,
                                        MATRIX& inputs,
                                        MATRIX& targets) {
      /**
  ...
  */
      ...
      for (int i = 0; i < len(params.values()); i++) {
        ...
        if (key == "learning_rate") {
          learning_rate = bp::extract<double>(params.values()[i]);
        } else if (key == "learning_method") {
          learning_method = bp::extract<int>(params.values()[i]);
        } else if (key == "momentum_term") {
          momentum_term = bp::extract<double>(params.values()[i]);
        } else if (key == "weight_decay_lambda") {
          weight_decay_lambda = bp::extract<double>(params.values()[i]);
        } else if (key == "etha") {
          etha = bp::extract<double>(params.values()[i]);
        }

        else if (key == "num_epochs") {
          num_epochs = bp::extract<int>(params.values()[i]);
        } else if (key == "steps_per_epoch") {
          steps_per_epoch = bp::extract<int>(params.values()[i]);
        } else if (key == "epoch_size") {
          epoch_size = bp::extract<int>(params.values()[i]);
          ...
          a_plus = bp::extract<double>(params.values()[i]);
        } else if (key == "a_minus") {
          a_minus = bp::extract<double>(params.values()[i]);
        } else if (key == "dB_min") {
          dB_min = bp::extract<double>(params.values()[i]);
        } else if (key == "dB_max") {
          dB_max = bp::extract<double>(params.values()[i]);
        } else if (key == "dW_min") {
          dW_min = bp::extract<double>(params.values()[i]);
        } else if (key == "dW_max") {
          dW_max = bp::extract<double>(params.values()[i]);
        }

      }  // for i

      // Sanity check and other setups
      if (learning_method == 1 || learning_method == 11 || learning_method == 12 ||
          learning_method == 13 || learning_method == 14) {
        learning_class = 1;
      } else if (learning_method == 2 || learning_method == 21 || learning_method == 22 ||
                 learning_method == 23) {
        learning_class = 2;
      }

      // Implementation status
      if (learning_method == 13) {
        cout << "The method 13 is not yet implemented: using 11 instead";
        learning_method = 11;
      }
      if (learning_method == 14) {
        cout << "The method 14 is not yet implemented: using 12 instead";
        learning_method = 12;
      }

      if (learning_method == 21) {
        cout << "The method 22 is not yet implemented: using 2 instead";
        learning_method = 2;
      }
      if (learning_method == 22) {
        cout << "The method 22 is not yet implemented: using 2 instead";
        learning_method = 2;
      }
      if (learning_method == 23) {
        cout << "The method 23 is not yet implemented: using 2 instead";
        learning_method = 2;
      }
      ...
      if (!is_error_collect_frequency) {
        error_collect_frequency = steps_per_epoch;
      }

      MATRIX input_subset(sz_x, epoch_size);
      MATRIX target_subset(sz_y, epoch_size);
      vector<int> subset(epoch_size);
      vector<int> inp_dim(sz_x);
      for (i = 0; i < sz_x; i++) {
        inp_dim[i] = i;
      }
      vector<int> tar_dim(sz_y);
      for (i = 0; i < sz_y; i++) {
        tar_dim[i] = i;
      }
      vector<MATRIX> Y;
      ...
      for (epoch = 0; epoch < num_epochs; epoch++) {
        for (i = 0; i < steps_per_epoch; i++) {
          ...
          if (counter % error_collect_frequency == 0) {
            err.push_back(err_loc);
          }
          ...
          for (L = 0; L < Nlayers; L++) {
            ...
            // RProp   basic version:
              ...
              for (a1 = 0; a1 < Npe[L]; a1++) {
                // Biases:
                double gr_prod =
                    grad_b[L].get(a1, 0) * grad_b_old[L].get(a1, 0);  // product of B gradients

                // Scaling
                if (gr_prod > 0.0) {
                  dB[L].scale(a1, 0, a_plus);
                } else if (gr_prod < 0.0) {
                  dB[L].scale(a1, 0, a_minus);
                } else {
                  ;
                  ;
                }  // don't change
                ...
                if (learning_method == 2) {
                  // Obey the bounds
                  if (dB[L].get(a1, 0) > dB_max) {
                    dB[L].set(a1, 0, dB_max);
                  } else if (dB[L].get(a1, 0) < dB_min) {
                    dB[L].set(a1, 0, dB_min);
                  }
                ...
                for (a2 = 0; a2 < Npe[L - 1]; a2++) {
                  gr_prod =
                      grad_w[L].get(a1, a2) * grad_w_old[L].get(a1, a2);  // product of B gradients

                  // Scaling
                  if (gr_prod > 0.0) {
                    dW[L].scale(a1, a2, a_plus);
                  } else if (gr_prod < 0.0) {
                    dW[L].scale(a1, a2, a_minus);
                  } else {
                    ;
                    ;
                  }  // don't change
                  ...
                  if (learning_method == 2) {
                    // Obey the bounds
                    if (dW[L].get(a1, a2) > dW_max) {
                      dW[L].set(a1, a2, dW_max);
                    } else if (dW[L].get(a1, a2) < dW_min) {
                      dW[L].set(a1, a2, dW_min);
                    }

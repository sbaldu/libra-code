...
*/
      ...
      if (re_part.size() != im_part.size()) {
        cout << "Error in CMATRIX constructor: y-dimensions(num of rows) of the real and imaginary "
                "arrays are not equal\n";
        exit(0);
      }
      if (re_part[0].size() != im_part[0].size()) {
        cout << "Error in CMATRIX constructor: x-dimensions(num of cols) of the real and imaginary "
                "arrays are not equal\n";
        exit(0);
      }
      n_rows = re_part.size();
...
*/
    ...
    CMATRIX::CMATRIX(MATRIX& re_part, MATRIX& im_part) {
      if (re_part.n_rows != im_part.n_rows) {
        cout << "Error in CMATRIX constructor: num_of_rows of the real and imaginary matrices are "
                "not equal\n";
        exit(0);
      }
      if (re_part.n_cols != im_part.n_cols) {
        cout << "Error in CMATRIX constructor: num_of_cols of the real and imaginary arrays are "
                "not equal\n";
        exit(0);
      }
      n_rows = re_part.n_rows;
    ...
    void CMATRIX::get_components(MATRIX& re_part, MATRIX& im_part) {
      if (re_part.n_cols != n_cols) {
        std::cout << "Error in CMATRIX::get_components : The number of columns of the target real "
                     "component matrix ("
                  << re_part.n_cols
                  << ") is not equal to the number of columns of the source complex matrix ("
                  << n_cols << ")\n";
        exit(0);
      }
      if (im_part.n_cols != n_cols) {
        std::cout << "Error in CMATRIX::get_components : The number of columns of the target "
                     "imaginary component matrix ("
                  << im_part.n_cols
                  << ") is not equal to the number of columns of the source complex matrix ("
                  << n_cols << ")\n";
        exit(0);
      ...
      if (re_part.n_rows != n_rows) {
        std::cout << "Error in CMATRIX::get_components : The number of rows of the target real "
                     "component matrix ("
                  << re_part.n_rows
                  << ") is not equal to the number of rows of the source complex matrix (" << n_rows
                  << ")\n";
        exit(0);
      }
      if (im_part.n_rows != n_rows) {
        std::cout << "Error in CMATRIX::get_components : The number of rows of the target "
                     "imaginary component matrix ("
                  << im_part.n_rows
                  << ") is not equal to the number of rows of the source complex matrix (" << n_rows
                  << ")\n";

...
*/
      ...
      pt.put(boost::property_tree::ptree::path_type(path + std::string(1, path_separator) + "x",
                                                    path_separator),
             vt.x);
      pt.put(boost::property_tree::ptree::path_type(path + std::string(1, path_separator) + "y",
                                                    path_separator),
             vt.y);
      pt.put(boost::property_tree::ptree::path_type(path + std::string(1, path_separator) + "z",
                                                    path_separator),
             vt.z);
    }
...
*/
      ...
      status = 0;
      int st;
      libio::load(pt, path + std::string(1, path_separator) + "x", path_separator, vt.x, st);
      if (st == 1) {
        status = 1;
      }
      libio::load(pt, path + std::string(1, path_separator) + "y", path_separator, vt.y, st);
      if (st == 1) {
        status = 1;
      }
      libio::load(pt, path + std::string(1, path_separator) + "z", path_separator, vt.z, st);
      if (st == 1) {
        status = 1;
      }
    }

...
*/
      ...
      for (int a = 0; a < fragment.size(); a++) {  // loop over all atoms in the given fragment
        ...
        fprintf(fpa,
                "   Energy(alpha)   Total(alpha)    DOS(s,alpha)   DOS(p,alpha)   DOS(d,alpha)\n");
        fprintf(fpb,
                "   Energy(beta)    Total(beta)     DOS(s,beta)    DOS(p,beta)    DOS(d,beta)\n");
        ...
        for (int k = 0; k < el.Norb; k++) {  // loop over all energies (ordered bands)
          ...
          int kk_a = el.bands_alp[k]
                         .first;  // local index of the MO with this energy (usually it is just "k")
          int kk_b = el.bands_bet[k]
                         .first;  // local index of the MO with this energy (usually it is just "k")
          ...
          for (int n = 0; n < atom_to_ao_map[A].size(); n++) {  // Loop over all AOs on atom A
            ...
            for (int j = 0; j < el.Norb; j++) {
              ...
              tmp += el.C_alp->M[ii * el.Norb + kk_a] * el.C_alp->M[jj * el.Norb + kk_a] *
            ...
            for (int j = 0; j < el.Norb; j++) {
              ...
              tmp += el.C_bet->M[ii * el.Norb + kk_b] * el.C_bet->M[jj * el.Norb + kk_b] *
                     el.Sao->M[jj * el.Norb + ii];
          ...
          fprintf(fpa,
                  ...
                  el.bands_alp[k].second,
                  pops_a + popp_a + popd_a,
                  pops_a,
                  popp_a,
                  popd_a);
          fprintf(fpb,
                  " %8.3f  %8.3f %8.3f %8.3f %8.3f   \n",
                  el.bands_bet[k].second,
                  pops_b + popp_b + popd_b,
                  pops_b,
                  popp_b,
                  popd_b);

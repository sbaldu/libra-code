...
namespace liblibra {
  ...
  namespace libqchem_tools {
    ...
    void charge_density(Electronic_Structure& el,
                        System& syst,
                        vector<AO>& basis_ao,
...
*/
      ...
      for (int i = 0; i < prms.orbs.size(); i++) {
        ...
        if (fp == NULL) {
          cout << "Error: Can not create/open file with prefix " << prms.charge_density_prefix
               << endl;
        ...
        for (int n = 0; n < syst.Number_of_atoms; n++) {
          fprintf(fp,
                  ...
                  syst.Atoms[n].Atom_Z,
                  0.0,
                  syst.Atoms[n].Atom_RB.rb_cm.x,
                  syst.Atoms[n].Atom_RB.rb_cm.y,
                  syst.Atoms[n].Atom_RB.rb_cm.z);
        }
    ...
    void charge_density(MATRIX& C,
                        vector<listHamiltonian_QM>& ham,
                        System& syst,
                        vector<vector<int> >& active_orb,
                        Control_Parameters& prms) {
      /**
...
*/
      ...
      for (i = 0; i < nfrags; i++) {
        summ +=
            active_orb[i]
                .size();  // how many FMOs does the fragment i contribute to the total pool of the orbitals
      }
      if (summ != nfmo) {
        cout << "Error in void charge_density(...) : The total number of FMOs due to all fragments "
                "is not equal to the number expected from the LC matrix\n";
      ...
      for (i = 0; i < prms.orbs.size();
           i++) {  // Note that these "orbs" will now have a meaning of the superpositions of fragment states

        int orb = prms.orbs[i];

        if (orb >= nfmo) {
          cout << "Error in void charge_density(...) : can only compute orbitals with indices "
                  "below the total # of FMOs\n";
              ...
              for (int fr = 0; fr < nfrags; fr++) {
                for (int fr_i = 0; fr_i < active_orb[fr].size(); fr_i++) {
                  ...
                  for (int a = 0; a < Norb; a++) {
                    psi_f += ham[fr].el->C_alp->M[a * Norb + active_orb[fr][fr_i]] *
                             ham[fr].basis_ao[a].compute(pos);
    ...
    void charge_density(MATRIX& C,
                        boost::python::list ham,
                        System& syst,
                        boost::python::list active_orb,
                        Control_Parameters& prms) {
      ...
      int sz = len(ham);
      for (int i = 0; i < sz; i++) {
        ham_int.push_back(extract<listHamiltonian_QM>(ham[i]));
      }  // extract<int>(at.values()[i])
      ...
      for (int fr = 0; fr < sz; fr++) {
        ...
        for (int i = 0; i < len(lst); i++) {
          lst_i.push_back(extract<int>(lst[i]));
        }

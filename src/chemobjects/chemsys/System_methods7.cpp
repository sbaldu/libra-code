...
*/
        ...
        if (is_Box) {
          ...
          fprintf(fp,
                  ...
                  alp,
                  bet,
                  gam);  // convert from internal units
                         // (Bohrs) to Angstroms
        }
        ...
        for (int i = 0; i < Number_of_atoms; i++) {
          ...
          if (Atoms[i].is_Atom_id) {
            at_ser_num = Atoms[i].Atom_id;
          }
          if (Atoms[i].is_Atom_element) {
            at_name = Atoms[i].Atom_element;
          }
          grp_num = Atoms[i].globGroup_Index;
          if (Atoms[i].is_Atom_RB) {
            if (Atoms[i].Atom_RB.is_rb_cm) {
              r = Atoms[i].Atom_RB.rb_cm;
            }
          }
          if (Atoms[i].is_Atom_charge) {
            b_fact = Atoms[i].Atom_charge;
          }
          ...
          fprintf(fp,
                  "HETATM%5i %4s MOL C%4i    %8.3f%8.3f%8.3f%6.2f%6.2f    \n",
                  at_ser_num,
                  at_name.c_str(),
                  grp_num,
                  r.x,
...
*/
        ...
        for (i = 0; i < sz; i++) {
          ...
          if (Atoms[indx].is_Atom_id) {
            at_ser_num = Atoms[indx].Atom_id;
          }
          if (Atoms[indx].is_Atom_element) {
            at_name = Atoms[indx].Atom_element;
          }
          grp_num = Atoms[indx].globGroup_Index;
          if (Atoms[indx].is_Atom_RB) {
            if (Atoms[indx].Atom_RB.is_rb_cm) {
              r = Atoms[indx].Atom_RB.rb_cm;
            }
          }
          if (Atoms[indx].is_Atom_charge) {
            b_fact = Atoms[indx].Atom_charge;
          }
...
*/
        ...
        if (is_Box) {
          ...
          gam = acos(tv1 * tv2) * radians_to_degrees;
          fprintf(fp,
                  "Molecule frame= %6d %9.3f%9.3f%9.3f%7.2f%7.2f%7.2f    \n",
                  frame,
                  a * bohr,
                  b * bohr,
                  c * bohr,
...
*/
        ...
        if (is_Box) {
          ...
          ss << "Molecule frame= " << frame << " " << a * bohr << " " << b * bohr << " "
             << c * bohr;
          ss << " " << alp << " " << bet << " " << gam

...
*/
        ...
        is_stress_ml = 0;
        for (int i = 0; i < Number_of_fragments; i++) {
          Fragments[i].Group_RB.rb_force = 0.0;
        }
      }
...
*/
        ...
        for (int i = 0; i < Number_of_fragments; i++) {
          Fragments[i].Group_RB.rb_torque_e = 0.0;
        }
      }
...
*/
        ...
        for (int i = 0; i < Number_of_fragments; i++) {
          for (int j = 0; j < Fragments[i].Group_Size; j++) {
            int at_indx = Fragments[i].globAtom_Index[j];
            Fragments[i].Group_RB.rb_torque_e +=
                cross(1.0,
                      Fragments[i].Group_RB.rb_centers[j],
                      Fragments[i].Group_RB.rb_A_I_to_e * Atoms[at_indx].Atom_RB.rb_force);
          }
        }
      }
...
*/
        ...
        if (respa_type == "fast") {
          ...
          if (respa_f_medium.size() == Number_of_fragments) {
            frcs = respa_f_medium;
          }
          if (respa_t_medium.size() == Number_of_fragments) {
            trcs = respa_t_medium;
          }
        }
...
*/
        ...
        for (int i = 0; i < Number_of_fragments; i++) {
          ...
          double* masses;
          masses = new double[sz];
          VECTOR* coords;
          coords = new VECTOR[sz];
          for (int j = 0; j < sz; j++) {
            int at_indx = Fragments[i].globAtom_Index[j];
...
*/
        ...
        for (int i = 0; i < Number_of_atoms; i++) {
          if (Atoms[i].Atom_RB.rb_cm.x < Box_origin.x) {
            Box_origin.x = Atoms[i].Atom_RB.rb_cm.x;
          }
          if (Atoms[i].Atom_RB.rb_cm.y < Box_origin.y) {
            Box_origin.y = Atoms[i].Atom_RB.rb_cm.y;
          }
          if (Atoms[i].Atom_RB.rb_cm.z < Box_origin.z) {
            Box_origin.z = Atoms[i].Atom_RB.rb_cm.z;
          }
        }
        is_Box_origin = 1;
      }
...
*/
        ...
        Box.init(tv1, tv2, tv3);
        is_Box = 1;
        Boxold.init(tv1, tv2, tv3);
        is_Boxold = 1;
...
*/
        ...
        double res = 0.0;
        VECTOR p_tot;
        p_tot = 0.0;  // total momentum of the system
        double m_tot;
...
*/
        ...
        if (stress_opt == "at") {
          for (int i = 0; i < Number_of_atoms; i++) {
            RigidBody& top = Atoms[i].Atom_RB;
            tmp.tensor_product(top.rb_p, top.rb_p);
            Pid += top.rb_iM * tmp;
            p_tot += top.rb_p;
            m_tot += top.rb_mass;
          }  // for i - all fragments
        ...
        if ((stress_opt == "at") && (is_stress_at == 0)) {
          stat = 1;
        }
        if ((stress_opt == "fr") && (is_stress_fr == 0)) {
          stat = 1;
        }
        if ((stress_opt == "ml") && (is_stress_ml == 0)) {
          stat = 1;
        }
...
*/
        ...
        double target_ekin_tr =
            0.5 * ((double)Nf_t) * (boltzmann * Temp) / hartree;  // in a.u. of energy
        double scaling_factor_tr = (temp_tr == 0.0) ? 0.0 : sqrt(target_ekin_tr / temp_tr);
        for (i = 0; i < Number_of_fragments; i++) {
          temp_p[i] *= scaling_factor_tr;
        }
        ...
        for (i = 0; i < Number_of_fragments; i++) {
          RigidBody& top = Fragments[i].Group_RB;
          VECTOR tmp;
          tmp.cross(top.rb_cm, temp_p[i]);
          tot_l += tmp;
        }
        tot_l = (TOT_L - tot_l) / size;
        for (i = 0; i < Number_of_fragments; i++) {
          temp_l[i] = Fragments[i].Group_RB.rb_A_I_to_e * tot_l;
        }
        ...
        double scaling_factor =
            ((temp_rot + temp_tr) == 0.0) ? 0.0 : sqrt(target_ekin / (temp_rot + temp_tr));

...
*/
      ...
      void System::create_bond(int a1, int a2, int exclude12) {
        ...
        if (is_valid) {
          ...
          if (!is_bond_exist) {
            //----------------- Create normal bond ----------------------
            if (bond.globAtom_Index.size() > 0) {
              bond.globAtom_Index.clear();
            }
            ...
            if (g1 == g2) {
            }       // All atoms belong to the same group
...
*/
      ...
      void System::create_angle(int a1, int a2, int a3, int exclude13) {
        ...
        if (is_valid) {
          ...
          if (!is_angle_exist) {
            //----------------- Create normal angle ----------------------
            if (angle.globAtom_Index.size() > 0) {
              angle.globAtom_Index.clear();
            }
...
*/
      ...
      void System::create_dihedral(int a1, int a2, int a3, int a4, int exclude14) {
        ...
        if ((a1 == a4) || (a1 == a3) || (a1 == a2) || (a2 == a4) || (a2 == a3) || (a3 == a4)) {
          is_valid = 0;
        }
        ...
        if (is_valid) {
          ...
          if (!is_dihedral_exist) {
            ...
            if (dihedral.globAtom_Index.size() > 0) {
              dihedral.globAtom_Index.clear();
            }
...
*/
      ...
      void System::create_improper(int a1, int a2, int a3, int a4) {
        ...
        if (Atoms[a1].globAtom_Adjacent_Atoms.size() == 2) {
          ...
          if (improper.globAtom_Index.size() > 0) {
            improper.globAtom_Index.clear();
          }
          ...
          if (impr_indx >= 0) {
            ...
            if (is_in_vector(Impropers[impr_indx].globGroup_Index, Frag_impropers)) {
              ...
              for (int s = 0; s < Number_of_frag_impropers; s++) {
                if (Impropers[impr_indx].globGroup_Index == Frag_impropers[s]) {
                  impr_frag_indx = s;
                  break;
                }
              }
...
*/
        ...
        if (Number_of_atoms > 0) {
          if (Atoms[0].is_Atom_id) {
            max_atom_id = Atoms[0].Atom_id;
          }
        ...
        if (Number_of_fragments > 0) {
          if (Fragments[0].is_Group_id) {
            max_frag_id = Fragments[0].Group_id;
          }
          for (int i = 1; i < Number_of_fragments; i++) {
            if (Fragments[i].is_Group_id) {
              max_frag_id =
                  (max_frag_id >= Fragments[i].Group_id) ? max_frag_id : Fragments[i].Group_id;
            }
          }
        }
        ...
        if (Number_of_molecules > 0) {
          if (Molecules[0].is_Molecule_id) {
            max_mol_id = Molecules[0].Molecule_id;
          }
          for (int i = 1; i < Number_of_molecules; i++) {
            if (Molecules[i].is_Molecule_id) {
              max_mol_id =
                  (max_mol_id >= Molecules[i].Molecule_id) ? max_mol_id : Molecules[i].Molecule_id;
            }
          }
        }

        max_atom_id = max_atom_id;
        is_max_atom_id = 1;
        max_fragment_id = max_frag_id;
        is_max_fragment_id = 1;
        max_molecule_id = max_atom_id;
        is_max_molecule_id = 1;
      }
...
*/
        ...
        Molecule mol;
        if (max_molecule_id) {
          mol.Molecule_id = max_molecule_id + 1;
          mol.is_Molecule_id = 1;
          max_molecule_id++;
        } else {
          mol.Molecule_id = 1;
          mol.is_Molecule_id = 1;
...
*/
        ...
        if (!is_exist) {  // Create only if this bond is not yet defined
          ...
          if (at1.globMolecule_Index == at2.globMolecule_Index) {
            ...
            int max = (at1.globMolecule_Index > at2.globMolecule_Index) ? at1.globMolecule_Index
                                                                        : at2.globMolecule_Index;
            int min = (at1.globMolecule_Index < at2.globMolecule_Index) ? at1.globMolecule_Index
                                                                        : at2.globMolecule_Index;
            ...
            Molecules[min].Molecule_Number_of_dihedrals +=
                Molecules[max].Molecule_Number_of_dihedrals;
	 ...
	 Molecules[min].locGroup_Index.push_back(Molecules[min].Molecule_Size + Molecules[max].locGroup_Index[i]);        
         */
              ...
              Molecules[min].locAtom_Index.push_back(Molecules[min].Molecule_Size +
                                                     Molecules[max].locAtom_Index[i]);
          ...
          for (adj1 = 0; adj1 < at1.globAtom_Adjacent_Atoms.size(); adj1++) {
            for (adj2 = 0; adj2 < at2.globAtom_Adjacent_Atoms.size(); adj2++) {
              ...
              create_dihedral(
                  adj_indx1, at1.globAtom_Index, at2.globAtom_Index, adj_indx2, exclude14);
            }
          }
          ...
          for (adj2 = 0; adj2 < at2.globAtom_Adjacent_Atoms.size(); adj2++) {
            ...
            for (int adj22 = 0; adj22 < at22.globAtom_Adjacent_Atoms.size(); adj22++) {
              ...
              create_dihedral(
                  at1.globAtom_Index, at2.globAtom_Index, at22.globAtom_Index, adj_indx2, exclude14);
            }
          }
          ...
          for (adj1 = 0; adj1 < at1.globAtom_Adjacent_Atoms.size(); adj1++) {
            ...
            for (int adj11 = 0; adj11 < at11.globAtom_Adjacent_Atoms.size(); adj11++) {
              ...
              create_dihedral(
                  adj_indx1, at11.globAtom_Index, at1.globAtom_Index, at2.globAtom_Index, exclude14);
            }
          }

          //--------------- Impropers -------------------------------
          if (at1.globAtom_Adjacent_Atoms.size() >= 2) {
            create_improper(at1.globAtom_Index,
                            at1.globAtom_Adjacent_Atoms[0],
                            at1.globAtom_Adjacent_Atoms[1],
                            at2.globAtom_Index);
          }
          if (at2.globAtom_Adjacent_Atoms.size() >= 2) {
            create_improper(at2.globAtom_Index,
                            at2.globAtom_Adjacent_Atoms[0],
                            at2.globAtom_Adjacent_Atoms[1],
                            at1.globAtom_Index);
          }
...
*/
        ...
        for (i = 0; i < at_indxs.size(); i++) {
          ...
          if (at.globGroup_Index != g_indx) {
            for (it = Fragments[at.globGroup_Index].globAtom_Index.begin();
                 it != Fragments[at.globGroup_Index].globAtom_Index.end();
                 it++) {
      ...
      void System::CREATE_BONDS(boost::python::list atoms_list,
                                boost::python::dict valence_by_element) {
        /**
...
*/
        ...
        for (i = 0; i < sz; i++) {
          ...
          if (j != -1) {
            at_indexes.push_back(j);
          } else {
            cout << "Error: Can not find atom with id = " << id << endl;
          }
        }
        ...
        for (i = 0; i < (sz - 1); i++) {
          ...
          for (j = i + 1; j < sz; j++) {
            ...
            if ((at_valences_sorted[i] >= 1) &&
                (at_valences_sorted[j] >= 1)) {  // Valence criterion
              ...
              dist_exp =
                  2.0;  // Default value - atoms which are separated by this much assumed to be connected
...
*/
        ...
        if (m == -1) {
          std::cout << "Error: Can not duplicate (clone) molecule with mol_id = " << mol_id
                    << " No such molecule\n";
          exit(97);
        }
        ...
        for (i = 0; i < init_size; i++) {
          if (Atoms[i].globMolecule_Index == m) {
            ...
            if (it == gr_map.end()) {
              gr_map[Atoms[i].globGroup_Index] = init_size1 + k1;
              k1++;
            }
            k++;
          }

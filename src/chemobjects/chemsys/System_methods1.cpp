...
*/
        ...
        GroupConnMatrix =
            new MATRIX(Number_of_fragments, Number_of_fragments);  // Group connectivity matrix
        *GroupConnMatrix = 0.0;
...
*/
        ...
        for (i = 0; i < Number_of_atoms; i++) {
          ...
          int min_sz = 1;
          if (sz >= 1) {
            min_sz = Atoms[i].Atom_ring_sizes[0];
          } else {
            for (j = 0; j < sz - 1; j++) {
              min_sz = (Atoms[i].Atom_ring_sizes[j] <= Atoms[i].Atom_ring_sizes[j + 1])
                           ? Atoms[i].Atom_ring_sizes[j]
                           : Atoms[i].Atom_ring_sizes[j + 1];
            }
          }
          Atoms[i].Atom_min_ring_size = min_sz;

          cout << "Atom i=" << i
               << " belongs to the minimal ring of size = " << Atoms[i].Atom_min_ring_size << endl;
        }  // for i
      }
...
*/
        ...
        for (int i = 0; i < GroupConnMatrix->n_cols; i++) {
          if ((GroupConnMatrix->M[g1 * GroupConnMatrix->n_cols + i] == 1.0) && (i != g2)) {
            ...
            if (st == 1) {
              DIVIDE_GRAPH(i, IndxException, P);
            }
          }  // if
        }  // for i
      }

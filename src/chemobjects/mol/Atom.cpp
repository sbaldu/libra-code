    ...
    namespace libmol {
      ...
      void Atom::copy_content(const Atom& at) {
        ...
        if (at.globAtom_Adjacent_Atoms.size()) {
          if (globAtom_Adjacent_Atoms.size() > 0) {
            globAtom_Adjacent_Atoms.clear();
          }
        ...
        if (at.is_Atom_RB_old) {
          Atom_RB_old = at.Atom_RB_old;
          is_Atom_RB_old = 1;
        }
        if (at.is_Atom_displ2) {
          Atom_displ2 = at.Atom_displ2;
          is_Atom_displ2 = 1;
        }
        ...
        if (at.is_Atom_atomic_radius) {
          Atom_atomic_radius = at.Atom_atomic_radius;
          is_Atom_atomic_radius = 1;
        }
        if (at.is_Atom_charge) {
          Atom_charge = at.Atom_charge;
          is_Atom_charge = 1;
        }
        ...
        if (at.is_Atom_formal_charge) {
          Atom_formal_charge = at.Atom_formal_charge;
          is_Atom_formal_charge = 1;
        }
        if (at.is_Atom_coordination) {
          Atom_coordination = at.Atom_coordination;
          is_Atom_coordination = 1;
        }
        if (at.is_Atom_functional_group) {
          Atom_functional_group = at.Atom_functional_group;
          is_Atom_functional_group = 1;
        }
        if (at.is_Atom_ring_sizes) {
          Atom_ring_sizes = at.Atom_ring_sizes;
          is_Atom_ring_sizes = 1;
        }
        if (at.is_Atom_min_ring_size) {
          Atom_min_ring_size = at.Atom_min_ring_size;
          is_Atom_min_ring_size = 1;
        }
        ...
        if (at.is_Atom_Zeff) {
          Atom_Zeff = at.Atom_Zeff;
          is_Atom_Zeff = 1;
        }

        if (at.is_Atom_mull_charge_gross) {
          Atom_mull_charge_gross = at.Atom_mull_charge_gross;
          is_Atom_mull_charge_gross = 1;
        }
        if (at.is_Atom_mull_charge_net) {
          Atom_mull_charge_net = at.Atom_mull_charge_net;
          is_Atom_mull_charge_net = 1;
        }
...
*/
        ...
        for (int i = 0; i < len(at.values()); i++) {
          ...
          if (key == "Atom_id") {
            Atom_id = extract<int>(at.values()[i]);
            is_Atom_id = 1;
          } else if (key == "Atom_element") {
            Atom_element = extract<std::string>(at.values()[i]);
            is_Atom_element = 1;
            Element elt = universe->Get_Element(Atom_element);

            // Get basic atomic properties
            if (elt.is_Elt_mass) {
              Atom_RB.set_mass(elt.Elt_mass);
            }
            if (elt.is_Elt_number) {
              Atom_Z = elt.Elt_number;
              is_Atom_Z = 1;
            }
            if (!is_Atom_Z && elt.is_Elt_nucleus_charge) {
              Atom_Z = elt.Elt_nucleus_charge;
              is_Atom_Z = 1;
            }

          } else if (key == "Atom_atomic_radius") {
            Atom_atomic_radius = extract<double>(at.values()[i]);
            is_Atom_atomic_radius = 1;
          } else if (key == "Atom_charge") {
            Atom_charge = extract<double>(at.values()[i]);
            is_Atom_charge = 1;
          } else if (key == "Atom_electronegativity") {
            Atom_electronegativity = extract<double>(at.values()[i]);
            is_Atom_electronegativity = 1;
          } else if (key == "Atom_formal_charge") {
            Atom_formal_charge = extract<double>(at.values()[i]);
            is_Atom_formal_charge = 1;
          } else if (key == "Atom_ff_type") {
            Atom_ff_type = extract<std::string>(at.values()[i]);
            is_Atom_ff_type = 1;
          } else if (key == "Atom_Zeff") {
            Atom_Zeff = extract<double>(at.values()[i]);
            is_Atom_Zeff = 1;
          } else if (key == "Atom_Z") {
            Atom_Z = extract<int>(at.values()[i]);
            is_Atom_Z = 1;
          }

          else if (key == "Atom_cm_x") {
            Atom_RB.rb_cm.x = extract<double>(at.values()[i]);
            is_Atom_RB = 1;
            Atom_RB.is_rb_cm = 1;
            Atom_RB_old.rb_cm.x = Atom_RB.rb_cm.x;
            is_Atom_RB_old = 1;
            Atom_RB.is_rb_cm = 1;
          } else if (key == "Atom_cm_y") {
            Atom_RB.rb_cm.y = extract<double>(at.values()[i]);
            is_Atom_RB = 1;
            Atom_RB.is_rb_cm = 1;
            Atom_RB_old.rb_cm.y = Atom_RB.rb_cm.y;
            is_Atom_RB_old = 1;
            Atom_RB.is_rb_cm = 1;
          } else if (key == "Atom_cm_z") {
            Atom_RB.rb_cm.z = extract<double>(at.values()[i]);
            is_Atom_RB = 1;
            Atom_RB.is_rb_cm = 1;
            Atom_RB_old.rb_cm.z = Atom_RB.rb_cm.z;
            is_Atom_RB_old = 1;
            Atom_RB.is_rb_cm = 1;
          }

          else if (key == "Atom_mull_charge_gross") {
            Atom_mull_charge_gross = extract<double>(at.values()[i]);
            is_Atom_mull_charge_gross = 1;
          } else if (key == "Atom_mull_charge_net") {
            Atom_mull_charge_net = extract<double>(at.values()[i]);
            is_Atom_mull_charge_net = 1;
          }
...
*/
        ...
        if (is_Atom_displ2) {
          std::cout << "Atom_displ2 = " << Atom_displ2 << std::endl;
        }
        //   if(is_Atom_RB){  Atom_RB.show_info(); }

        if (is_Atom_Z) {
          std::cout << "Atom_Z = " << Atom_Z << std::endl;
        }
        if (is_Atom_element) {
          std::cout << "Atom_element = " << Atom_element << std::endl;
        }
        if (is_Atom_atomic_radius) {
          std::cout << "Atom_atomic_radius = " << Atom_atomic_radius << std::endl;
        }
        if (is_Atom_charge) {
          std::cout << "Atom_charge = " << Atom_charge << std::endl;
        }
        if (is_Atom_electronegativity) {
          std::cout << "Atom_electronegativity = " << Atom_electronegativity << std::endl;
        }

        if (is_Atom_formal_charge) {
          std::cout << "Atom_formal_charge = " << Atom_formal_charge << std::endl;
        }
        if (is_Atom_coordination) {
          std::cout << "Atom_coordination = " << Atom_coordination << std::endl;
        }
        if (is_Atom_functional_group) {
          std::cout << "Atom_functional_group = " << Atom_functional_group << std::endl;
        }
        if (is_Atom_ring_sizes) {
          std::cout << "Atom_ring_sizes = ";
          for (int i = 0; i < Atom_ring_sizes.size(); i++) {
            std::cout << Atom_ring_sizes[i] << "  ";
          }
          std::cout << std::endl;
        }
        if (is_Atom_min_ring_size) {
          std::cout << "Atom_min_ring_size = " << Atom_min_ring_size << std::endl;
        }
        ...
        if (is_Atom_Zeff) {
          std::cout << "Atom_Zeff = " << Atom_Zeff << std::endl;
        }

        if (is_Atom_mull_charge_gross) {
          std::cout << "Atom_mull_charge_gross = " << Atom_mull_charge_gross << std::endl;
        }
        if (is_Atom_mull_charge_net) {
          std::cout << "Atom_mull_charge_net = " << Atom_mull_charge_net << std::endl;
        }
...
*/
        ...
        if (is_Atom_id) {
          libio::save(pt, path + ".Atom_id", Atom_id);
        }
        ...
        if (is_Atom_RB) {
          Atom_RB.save(pt, path + ".Atom_RB");
        }
        if (is_Atom_RB_old) {
          Atom_RB_old.save(pt, path + ".Atom_RB_old");
        }
        //if(is_Atom_RB){  ::save(pt,path+".Atom_RB",Atom_RB);    }
        //if(is_Atom_RB_old){  ::save(pt,path+".Atom_RB_old",Atom_RB_old);    }

        if (is_Atom_displ2) {
          libio::save(pt, path + ".Atom_displ2", Atom_displ2);
        }

        if (is_Atom_Z) {
          libio::save(pt, path + ".Atom_Z", Atom_Z);
        }
        if (is_Atom_element) {
          libio::save(pt, path + ".Atom_element", Atom_element);
        }
        if (is_Atom_atomic_radius) {
          libio::save(pt, path + ".Atom_atomic_radius", Atom_atomic_radius);
        }
        if (is_Atom_charge) {
          libio::save(pt, path + ".Atom_charge", Atom_charge);
        }
        if (is_Atom_electronegativity) {
          libio::save(pt, path + ".Atom_electronegativity", Atom_electronegativity);
        }
        if (is_Atom_formal_charge) {
          libio::save(pt, path + ".Atom_formal_charge", Atom_formal_charge);
        }
        if (is_Atom_coordination) {
          libio::save(pt, path + ".Atom_coordination", Atom_coordination);
        }
        if (is_Atom_functional_group) {
          libio::save(pt, path + ".Atom_functional_group", Atom_functional_group);
        }
        if (is_Atom_ring_sizes) {
          libio::save(pt, path + ".Atom_ring_sizes", Atom_ring_sizes);
        }
        if (is_Atom_min_ring_size) {
          libio::save(pt, path + ".Atom_min_ring_size", Atom_min_ring_size);
        }
        if (is_Atom_ff_type) {
          libio::save(pt, path + ".Atom_ff_type", Atom_ff_type);
        }
        if (is_Atom_Zeff) {
          libio::save(pt, path + ".Atom_Zeff", Atom_Zeff);
        }

        if (is_Atom_mull_charge_gross) {
          libio::save(pt, path + ".Atom_mull_charge_gross", Atom_mull_charge_gross);
        }
        if (is_Atom_mull_charge_net) {
          libio::save(pt, path + ".Atom_mull_charge_net", Atom_mull_charge_net);
        }
      }
...
*/
        ...
        libio::load(pt, path + ".Atom_id", Atom_id, is_Atom_id);
        if (is_Atom_id == 1) {
          status = 1;
        }

        libio::load(pt, path + ".globAtom_Index", globAtom_Index, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".locAtom_Index", locAtom_Index, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".globGroup_Index", globGroup_Index, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".globMolecule_Index", globMolecule_Index, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".globAtom_Adjacent_Atoms", globAtom_Adjacent_Atoms, st);
        if (st == 1) {
          status = 1;
        }

        Atom_RB.load(pt, path + ".Atom_RB", is_Atom_RB);
        if (is_Atom_RB == 1) {
          status = 1;
        }
        Atom_RB_old.load(pt, path + ".Atom_RB_old", is_Atom_RB_old);
        if (is_Atom_RB_old == 1) {
          status = 1;
        }
        ...
        libio::load(pt, path + ".Atom_displ2", Atom_displ2, is_Atom_displ2);
        if (is_Atom_displ2 == 1) {
          status = 1;
        }

        libio::load(pt, path + ".Atom_Z", Atom_Z, is_Atom_Z);
        if (is_Atom_Z == 1) {
          status = 1;
        }
        ...
        libio::load(pt, path + ".Atom_atomic_radius", Atom_atomic_radius, is_Atom_atomic_radius);
        if (is_Atom_atomic_radius == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Atom_charge", Atom_charge, is_Atom_charge);
        if (is_Atom_charge == 1) {
          status = 1;
        }
        libio::load(pt,
                    path + ".Atom_electronegativity",
                    Atom_electronegativity,
                    is_Atom_electronegativity);
        if (is_Atom_electronegativity == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Atom_formal_charge", Atom_formal_charge, is_Atom_formal_charge);
        if (is_Atom_formal_charge == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Atom_coordination", Atom_coordination, is_Atom_coordination);
        if (is_Atom_coordination == 1) {
          status = 1;
        }
        libio::load(
            pt, path + ".Atom_functional_group", Atom_functional_group, is_Atom_functional_group);
        if (is_Atom_functional_group == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Atom_ring_sizes", Atom_ring_sizes, is_Atom_ring_sizes);
        if (is_Atom_ring_sizes == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Atom_min_ring_size", Atom_min_ring_size, is_Atom_min_ring_size);
        if (is_Atom_min_ring_size == 1) {
          status = 1;
        }
        ...
        libio::load(pt, path + ".Atom_Zeff", Atom_Zeff, is_Atom_Zeff);
        if (is_Atom_Zeff == 1) {
          status = 1;
        }

        libio::load(pt,
                    path + ".Atom_mull_charge_gross",
                    Atom_mull_charge_gross,
                    is_Atom_mull_charge_gross);
        if (is_Atom_mull_charge_gross == 1) {
          status = 1;
        }
        libio::load(
            pt, path + ".Atom_mull_charge_net", Atom_mull_charge_net, is_Atom_mull_charge_net);
        if (is_Atom_mull_charge_net == 1) {
          status = 1;
        }
      }

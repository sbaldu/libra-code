...
namespace liblibra {
  ...
  namespace libchemobjects {
    namespace libmol {
      ...
      void Molecule::copy_content(const Molecule& mol) {
        ...
        if (mol.is_Molecule_name) {
          Molecule_name = mol.Molecule_name;
          is_Molecule_name = 1;
        }
        if (mol.is_Molecule_id) {
          Molecule_id = mol.Molecule_id;
          is_Molecule_id = 1;
        }
        if (mol.is_Molecule_RB) {
          Molecule_RB = mol.Molecule_RB;
          is_Molecule_RB = 1;
        }
      }
  ...
  *********************/
      ...
      void Molecule::show_info() {
        ...
        for (int i = 0; i < globMolecule_Size; i++) {
        ...
        if (is_Molecule_name) {
          std::cout << "Molecule_name = " << Molecule_name << std::endl;
        }
        if (is_Molecule_id) {
          std::cout << "Molecule_id = " << Molecule_id << std::endl;
        }
      }

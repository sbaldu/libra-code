...
*/
      ...
      public:
        //--------- Atom identification ----------------------------
        int Atom_id;
        int is_Atom_id;  ///< the atomic integer-valued ID - can be any number
        ...
        vector<int>
            globAtom_Adjacent_Atoms;  ///< The vector of globar atomic indices for the atoms directly connected (in MM) to the present atom

        //--------- Dynamical properties of the atom --------------
        RigidBody Atom_RB;
        int is_Atom_RB;  ///< this is the object containing the spatial information about the atom (position, velocity, etc, even the orientation, if needed)
        RigidBody Atom_RB_old;
        int is_Atom_RB_old;  ///< previous position: from some previous calculations - needed to comparison-based calculations
        double Atom_displ2;
        int is_Atom_displ2;  ///< square of atomic displacements (present - old)

        //----------Basic physical properties of the atom ---------
        int Atom_Z;
        int is_Atom_Z;  ///< the nucleus charge of the given atom (e.g. 3 for Li)
        std::string Atom_element;
        int is_Atom_element;  ///< name of the element of the given atom
        double Atom_atomic_radius;
        int is_Atom_atomic_radius;  ///< atomic radius
        double Atom_charge;
        int is_Atom_charge;  ///< The partial charge of the atom - e.g. Mulliken charge
        double Atom_electronegativity;
        int is_Atom_electronegativity;  ///< Atomic electronegativity

        //--------- Atom-in-Molecule topological properties --------------
        double Atom_formal_charge;
        int is_Atom_formal_charge;  ///< This is the formal charge - e.g. the valence state charge - e.g. +1 on N in protonated amino group
        int Atom_coordination;
        int is_Atom_coordination;  ///< defines an equilibrium angle
        std::string Atom_functional_group;
        int is_Atom_functional_group;  ///< Name of the functional group to which the atom belongs
        vector<int> Atom_ring_sizes;
        int is_Atom_ring_sizes;  ///< Sizes of all rings to which this atom belongs
        int Atom_min_ring_size;
        int is_Atom_min_ring_size;  ///< The size of the minimal ring to which the atom belongs

        //----------- Force-field related properties ---------------
        std::string Atom_ff_type;
        int is_Atom_ff_type;  ///< Force-field atom name
        double Atom_Zeff;
        int is_Atom_Zeff;  ///< Effective atomic core charge - e.g. +1 for Li, +2 for Ca, etc.

        //----------- Electronic structure-related properties ---------------
        double Atom_mull_charge_gross;
        int is_Atom_mull_charge_gross;  ///< Specifically, the Mulliken gross charge
        double Atom_mull_charge_net;
        int is_Atom_mull_charge_net;  ///< Specifically, the Mulliken net charge
        ...
        bool operator!=(const Atom& a) const { return globAtom_Index != a.globAtom_Index; }
        friend ostream& operator<<(ostream& strm, Atom& ob) {
          ob.show_info();
          return strm;
        }
        void set(object);

...
namespace liblibra {
  ...
  namespace libchemobjects {
    namespace libmol {
      ...
      void Group::copy_content(const Group& at) {
        ...
        if (at.is_Group_radius) {
          Group_radius = at.Group_radius;
          is_Group_radius = 1;
        }
        if (at.is_Group_RB) {
          Group_RB = at.Group_RB;
          is_Group_RB = 1;
        }
        if (at.is_Group_ff_type) {
          Group_ff_type = at.Group_ff_type;
          is_Group_ff_type = 1;
        }
        ...
        if (at.is_Group_bond_order) {
          Group_bond_order = at.Group_bond_order;
          is_Group_bond_order = 1;
        }
        if (at.is_Group_bond_alpha) {
          Group_bond_alpha = at.Group_bond_alpha;
          is_Group_bond_alpha = 1;
        }
  ...
  *********************/
      ...
      void Group::show_info() {
        ...
        if (is_Group_ff_type) {
          std::cout << "Group_ff_type = " << Group_ff_type << std::endl;
        }
        ...
        if (is_Group_bond_order) {
          std::cout << "Group_bond_order = " << Group_bond_order << std::endl;
        }
      ...
      void Group::save(boost::property_tree::ptree& pt, std::string path) {
        ...
        if (is_Group_name) {
          libio::save(pt, path + ".Group_name", Group_name);
        }
        if (is_Group_id) {
          libio::save(pt, path + ".Group_id", Group_id);
        }
        if (is_Group_radius) {
          libio::save(pt, path + ".Group_radius", Group_radius);
        }
        if (is_Group_RB) {
          Group_RB.save(pt, path + ".Group_RB");
        }
        if (is_Group_ff_type) {
          libio::save(pt, path + ".Group_ff_type", Group_ff_type);
        }
        if (is_Group_bond_order) {
          libio::save(pt, path + ".Group_bond_order", Group_bond_order);
        }
        if (is_Group_bond_alpha) {
          libio::save(pt, path + ".Group_bond_alpha", Group_bond_alpha);
        }
      }
      ...
      void Group::load(boost::property_tree::ptree& pt, std::string path, int& status) {
        ...
        libio::load(pt, path + ".globGroup_Size", globGroup_Size, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".locGroup_Size", locGroup_Size, st);
        if (st == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_Size", Group_Size, st);
        if (st == 1) {
          status = 1;
        }
        ...
        libio::load(pt, path + ".locGroup_Index", locGroup_Index, st);
        if (st == 1) {
          status = 1;
        }
        ...
        libio::load(pt, path + ".Group_name", Group_name, is_Group_name);
        if (is_Group_name == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_id", Group_id, is_Group_id);
        if (is_Group_id == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_radius", Group_radius, is_Group_radius);
        if (is_Group_radius == 1) {
          status = 1;
        }
        Group_RB.load(pt, path + ".Group_RB", is_Group_RB);
        if (is_Group_RB == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_ff_type", Group_ff_type, is_Group_ff_type);
        if (is_Group_ff_type == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_bond_order", Group_bond_order, is_Group_bond_order);
        if (is_Group_bond_order == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Group_bond_alpha", Group_bond_alpha, is_Group_bond_alpha);
        if (is_Group_bond_alpha == 1) {
          status = 1;
        }
      }

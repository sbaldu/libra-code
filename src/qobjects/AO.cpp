...
*/
      ...
      if (g.is_primitives) {
        for (int i = 0; i < g.primitives.size(); i++) {
          primitives.push_back(g.primitives[i]);
        }
        is_primitives = 1;
      }
      if (g.is_coefficients) {
        for (int i = 0; i < g.coefficients.size(); i++) {
...
*/
      ...
      if (is_expansion_size) {
        std::cout << "expansion_size = " << expansion_size << " primitives" << std::endl;
...
*/
      ...
      for (int i = 0; i < expansion_size; i++) {
        for (int j = 0; j < expansion_size; j++) {
          ...
          res +=
              coefficients[i] * coefficients[j] * gaussian_overlap(primitives[i], primitives[j], 0);
        }
      }
      return res;
    }
...
*/
    ...
    double gaussian_overlap(AO& AOa,
                            AO& AOb,
                            int is_normalize,
                            int is_derivs,
                            VECTOR& dIdA,
...
*/
      ...
      for (int i = 0; i < AOa.expansion_size; i++) {
        for (int j = 0; j < AOb.expansion_size; j++) {
          ...
          res +=
              w * gaussian_overlap(
                      AOa.primitives[i], AOb.primitives[j], 0, is_derivs, dida, didb, auxd, n_aux);
          dIdA += w * dida;
          dIdB += w * didb;
    ...
    double gaussian_moment(AO& AOa,
      ...
      for (int i = 0; i < AOa.expansion_size; i++) {
        for (int j = 0; j < AOb.expansion_size; j++) {
          w = AOa.coefficients[i] * AOb.coefficients[j];
          // use unnormalized primitives!
          res += w * gaussian_moment(AOa.primitives[i],
                                     G,
                                     AOb.primitives[j],
                                     0,
                                     is_derivs,
    ...
    double gaussian_moment(AO& AOa,
      ...
      // Do computations
      double res =
          gaussian_moment(AOa, G, AOb, is_normalize, is_derivs, dIdA, dIdR, dIdB, auxd, n_aux);
    ...
    double pseudopot02(double C0,
      ...
      // Do computations
      double res = pseudopot02(
          C0, C2, alp, R, AOa, AOb, is_normalize, is_derivs, dIdR, dIdA, dIdB, auxd, n_aux);
    ...
    // Reference verions
    VECTOR transition_dipole_moment(AO& AOa,
                                    AO& AOb,
                                    int is_normalize,
                                    int is_derivs,
                                    MATRIX3x3& dMdA,
      ...
      for (int i = 0; i < AOa.expansion_size; i++) {
        for (int j = 0; j < AOb.expansion_size; j++) {
          ...
          // use unnormalized primitives!
          res +=
              w * transition_dipole_moment(
                      AOa.primitives[i], AOb.primitives[j], 0, is_derivs, dmda, dmdb, auxd, n_aux);
          dMdA += w * dmda;
          dMdB += w * dmdb;
    ...
    VECTOR transition_dipole_moment(
      ...
      // Do computations
      VECTOR res;
      res = transition_dipole_moment(AOa, AOb, is_normalize, is_derivs, dMdA, dMdB, auxd, n_aux);
    ...
    // Reference verions
    VECTOR derivative_coupling_integral(AO& AOa,
                                        AO& AOb,
                                        int is_normalize,
                                        int is_derivs,
                                        MATRIX3x3& dMdA,
      ...
      for (int i = 0; i < AOa.expansion_size; i++) {
        for (int j = 0; j < AOb.expansion_size; j++) {
          ...
          // use unnormalized primitives!
          res +=
              w * derivative_coupling_integral(
                      AOa.primitives[i], AOb.primitives[j], 0, is_derivs, dmda, dmdb, auxd, n_aux);
          dMdA += w * dmda;
          dMdB += w * dmdb;
    ...
    VECTOR derivative_coupling_integral(
      ...
      VECTOR res;
      res =
          derivative_coupling_integral(AOa, AOb, is_normalize, is_derivs, dMdA, dMdB, auxd, n_aux);
    ...
    boost::python::list derivative_coupling_integral(AO& AOa,
                                                     AO& AOb,
                                                     int is_normalize,
                                                     int is_derivs) {
      MATRIX3x3 dMdA, dMdB;
      VECTOR I;
      I = derivative_coupling_integral(AOa, AOb, is_normalize, is_derivs, dMdA, dMdB);
    ...
    double kinetic_integral(AO& AOa,
                            AO& AOb,
                            int is_normalize,
                            int is_derivs,
                            VECTOR& dIdA,
    ...
    double nuclear_attraction_integral(AO& AOa,
                                       AO& AOb,
                                       VECTOR& Rc,
                                       int is_normalize,
                                       int is_derivs,
      ...
      for (int i = 0; i < AOa.expansion_size; i++) {
        for (int j = 0; j < AOb.expansion_size; j++) {
          ...
          // use unnormalized primitives!
          res += w * nuclear_attraction_integral(AOa.primitives[i],
                                                 AOb.primitives[j],
                                                 Rc,
                                                 0,
                                                 is_derivs,
    ...
    double nuclear_attraction_integral(AO& AOa,
      ...
      double res = nuclear_attraction_integral(
          AOa, AOb, Rc, is_normalize, is_derivs, DA, DB, DC, auxd, n_aux, auxv, n_auxv);
      // Clean working memory
      for (i = 0; i < 20; i++) {
        delete[] auxd[i];
      }
    ...
    // Versions with references
    double electron_repulsion_integral(AO& AOa,
                                       AO& AOb,
                                       AO& AOc,
                                       AO& AOd,
                                       int is_normalize,
        ...
        for (int j = 0; j < AOb.expansion_size; j++) {
          for (int k = 0; k < AOc.expansion_size; k++) {
            for (int l = 0; l < AOd.expansion_size; l++) {
              ...
              // use unnormalized primitives!
              res += w * electron_repulsion_integral(AOa.primitives[i],
                                                     AOb.primitives[j],
                                                     AOc.primitives[k],
                                                     AOd.primitives[l],
                                                     0,
    ...
    double electron_repulsion_integral(AO& AOa,
      ...
      // Do computations
      double res = electron_repulsion_integral(
          AOa, AOb, AOc, AOd, is_normalize, is_derivs, DA, DB, DC, DD, auxd, n_aux, auxv, n_auxv);
      // Clean working memory
      for (i = 0; i < 30; i++) {
        delete[] auxd[i];
      }
    ...
    boost::python::list electron_repulsion_integral(
      ...
      VECTOR DA, DB, DC, DD;
      double I =
          electron_repulsion_integral(AOa, AOb, AOc, AOd, is_normalize, is_derivs, DA, DB, DC, DD);

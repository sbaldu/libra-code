...
*/
      ...
      if (!(m.kx == this->kx && m.ky == this->ky && m.kz == this->kz)) {
        cout
            << "Error in PW::operator-: can subtract orbitals only if the k-points are the same!\n";
        exit(0);
      }
...
*/
      ...
      if (!(m.kx == this->kx && m.ky == this->ky && m.kz == this->kz)) {
        cout << "Error in PW::operator+=: can increment orbitals only if the k-points are the "
                "same!\n";
        exit(0);
      }
...
*/
    ...
    PW operator*(const complex<double>& f, const PW& m1) {
      return multiply<const complex<double> >(f, m1);
    }
    PW operator*(const PW& m1, const complex<double>& f) {
      return multiply<const complex<double> >(f, m1);
    }
    PW operator*(const complex<float>& f, const PW& m1) {
      return multiply<const complex<float> >(f, m1);
    }
    PW operator*(const PW& m1, const complex<float>& f) {
      return multiply<const complex<float> >(f, m1);
    }
    ...
    complex<double> operator*(const PW& m1, const PW& m2) {
      ...
      if (m1.npw != m2.npw) {
        cout << "Error: Can not multiply MOs with different basis sizes\n";
        exit(0);
      }

      else {
        for (int i = 0; i < m1.npw; i++) {
          res += m1.coeff[i] * m2.coeff[i];
        }
      }
      return res;
    }
...
*/
      ...
      complex<double> res =
          I_1D(k.x, kp.x, g.x, gp.x) * I_1D(k.y, kp.y, g.y, gp.y) * I_1D(k.z, kp.z, g.z, gp.z);

      return res;
    }

    CMATRIX pw_overlap(VECTOR& k1,
                       VECTOR& k2,
                       CMATRIX& coeff1,
                       CMATRIX& coeff2,
                       vector<VECTOR>& grid1,
...
*/
      ...
      CMATRIX* S;
      S = new CMATRIX(
          nbands1, nbands2);  // all orbitals for given pair of k-points (a block of entire matrix)
...
*/
      ...
      if (verbose) {
        cout << "Number of K-points = " << nkpts << endl;
      }
      ...
      if (kpt < 0) {
        cout << "Error: The k-point index ( " << kpt << " ) can not be less than 0\n";
        exit(0);
      }
      if (kpt > nkpts - 1) {
        cout << "Error: The k-point index (" << kpt << " ) is not allowed. The maximal\
                   value is = "
             << nkpts - 1 << "\n";
        exit(0);
      }
      ...
      while (status != 0) {
        ...
        if (status == 1) {
          start = e + 1;
          knbnd++;
          kbeg[kpt].push_back(b);
          kend[kpt].push_back(e);
        }
      }
      if (verbose) {
        cout << "Number of bands for k-point " << kpt << " is = " << knbnd << endl;
      }
      ...
      for (int i = 0; i < sz; i++) {
        ...
        if (act_space[i] > nbands - 1) {
          cout << "Error: Orbital index can not be larger than " << nbands - 1
               << ", given = " << act_space[i] << endl;
          cout << "Exiting...\n";
          exit(0);
        }
      }  // for i
...
*/
      ...
      if (g_sz != npw) {
        if (npw == (2 * g_sz - 1)) {
          ...
          cout << "Warning in compute_Hprime: number of plane waves = " << npw
               << " is different from the grid size =" << g_sz << endl;
        }
        g_sz = min(g_sz, npw);
      }
      ...
      for (int i = 0; i < nmo; i++) {
        for (int j = 0; j < nmo; j++) {
          for (int g = 0; g < g_sz; g++) {
            ...
            gx = grid.get(g, 0) * reci.get(0, 0) + grid.get(g, 1) * reci.get(0, 1) +
                 grid.get(g, 2) * reci.get(0, 2);
            gy = grid.get(g, 0) * reci.get(1, 0) + grid.get(g, 1) * reci.get(1, 1) +
                 grid.get(g, 2) * reci.get(1, 2);
            gz = grid.get(g, 0) * reci.get(2, 0) + grid.get(g, 1) * reci.get(2, 1) +
                 grid.get(g, 2) * reci.get(2, 2);

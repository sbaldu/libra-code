...
#define FIX_FLOAT(x) FIX_LONG(x)
    ...
    void get_value(T& val, char* memblock, int& pos) {
      memcpy((void*)&val, &memblock[pos], sizeof(T));
      pos += sizeof(T);
    }
...
*/
    ...
    public:
      ...
      vector<complex<double> >
          coeff;  ///< coefficients of the plane waves: c(G) = exp(-i*(g+k)*r), where we run over all G points
      ...
      friend PW operator*(const complex<double>& f,
                          const PW& m1);  // Multiplication of PW and complex<double>;
      friend PW operator*(const PW& m1,
                          const complex<double>& f);  // Multiplication of PW and complex<double>;
      friend PW operator*(const complex<float>& f,
                          const PW& m1);  // Multiplication of PW and complex<float>;
      friend PW operator*(const PW& m1,
                          const complex<float>& f);  // Multiplication of PW and complex<float>;
      ...
      friend int operator==(const PW& pw1, const PW& pw2) {
        ...
        if (res) {
          for (int i = 0; i < pw1.npw; i++) {
            res *= (pw1.coeff[i] == pw2.coeff[i]);
          }
        }
        return res;
      }

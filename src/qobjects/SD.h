...
*/
    ...
    public:
      ...
      vector<int>
          orb_indx_alp;  ///< Indices of the spatial orbitals that enter the present determinant as
      ...
      vector<int>
          orb_indx_bet;  ///< Indices of the spatial orbitals that enter the present determinant as
      ///< beta spin-orbitals. The indexing is w.r.t. the external pool of beta orbitals
      ///< The length of this array corresponds to N_bet
      vector<int>
          spin;  ///< The list of indices defining the spin of each orbital in this SD, in the same order as the MOs
      ...
      SD(CMATRIX& _mo_pool_alp,
         CMATRIX& _mo_pool_bet,
         vector<int>& _orb_indx_alp,
         vector<int>& _orb_indx_bet);  ///< c-tor
      ...
      friend int operator==(const SD& sd1, const SD& sd2) {
        int i;
        int res = ((sd1.N == sd2.N) && (sd1.N_bas == sd2.N_bas) && (sd1.N_alp == sd2.N_alp) &&
                   (sd1.N_bet == sd2.N_bet));
        if (res == 1) {
          for (i = 0; i < sd1.N; i++) {
            res *= (sd1.spin[i] == sd2.spin[i]);
          }
          for (i = 0; i < sd1.N_alp; i++) {
            res *= (sd1.orb_indx_alp[i] == sd2.orb_indx_alp[i]);
          }
          for (i = 0; i < sd1.N_bet; i++) {
            res *= (sd1.orb_indx_bet[i] == sd2.orb_indx_bet[i]);
          }
        }
        return res;
      }

...
*/
    ...
    public:
      // Members
      std::string element;
      int is_element;  ///< name of chemical element on which this AO is localized (if so)
      std::string ao_shell;
      int is_ao_shell;  ///< name of the AO shell e.g. 1s, 2p, 3p, 3d, etc.
      std::string ao_shell_type;
      int is_ao_shell_type;  ///< type of the AO shell e.g. s, p, d,  etc.
      std::string ao_name;
      int is_ao_name;  ///< name of the AO e.g. 1s, 2px, 3dxy,  etc.  int at_indx;

      // Angular momentum is defined for AOs, but each AO may have many Gaussians with different
      // alpha constants
      int x_exp;
      int is_x_exp;  ///< angular momentum quantum number (x projection) - if all primitives have it same
      int y_exp;
      int is_y_exp;  ///< angular momentum quantum number (y projection) - if all primitives have it same
      int z_exp;
      int is_z_exp;  ///< angular momentum quantum number (z projection) - if all primitives have it same
      int expansion_size;
      int is_expansion_size;  ///< The number of Primitive Gaussians in the contraction
      vector<PrimitiveG> primitives;
      int is_primitives;  ///< Primitive Gaussians that constitute this AO
      vector<double> coefficients;
      int is_coefficients;  ///< Contraction coefficients: these coefficients correspond to normalized primitive Gaussians

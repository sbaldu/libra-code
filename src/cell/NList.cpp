...
*/
      ...
      int ax, ay,
          az;  // how many cells in each direction are necessary to exceed Roff in corresponding direction
               /*
  ax = floor(Roff/cellx);
  ay = floor(Roff/celly);
  az = floor(Roff/cellz);
...
************************************************************************/
      ...
      int cc = (maxb.n3 - minb.n3 + 1) * (maxb.n2 - minb.n2 + 1) * (-minb.n1) +
               (maxb.n3 - minb.n3 + 1) * (-minb.n2) + (-minb.n3);  // index of central cell
      ...
      for (int at_indx1 = 0; at_indx1 < Nat; at_indx1++) {
        ...
        if (nlist[at_indx1].capacity() <= newsize) {
...
*/
      ...
      for (int i = 0; i < Nat; i++) {
        ...
        for (int k = 0; k < sz; k++) {
          int j = nlist[i][k].j;
          res += (r[i] - (r[j] + nlist[i][k].n1 * t1 + nlist[i][k].n2 * t2 + nlist[i][k].n3 * t3))
                     .length();
          cnt += 1.0;
        }  // for k
      }  // for i

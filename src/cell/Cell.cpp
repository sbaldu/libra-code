...
namespace liblibra {
  ...
  namespace libcell {
    ...
    void Cell::brute_force(VECTOR& rij,
                           int degree,
                           vector<triple>& res,
      ...
      if (det >= 0.0) {
        ...
        for (n1 = N1_min; n1 <= N1_max; n1++) {
          ...
          for (n2 = N2_min; n2 <= N2_max; n2++) {
            Zb = (C - n1 * T13 - n2 * T23);
            Zc = (n1 * n1 * a2 - 2.0 * A * n1 + n2 * n2 * b2 - 2.0 * B * n2 - D +
                  2.0 * n1 * n2 * T12);
            det2 = Zb * Zb - Za * Zc;
            if (det2 < 0.0 && fabs(det2) < 1e-5) {
              det2 = 0.0;
            }
            delta = sqrt(det2);
            nm = (Zb - delta) / Za;
            ...
            for (n3 = N3_min; n3 <= N3_max; n3++) {
              ...
              t.n2 = n2;
              t.n3 = n3;
              if ((n1 == central_translation.n1) && (n2 == central_translation.n2) &&
...
*/
      ...
      if (central_translation.size() > 0) {
        central_translation.clear();
      }

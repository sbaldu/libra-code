...
*********************************************************************************/
    ...
    void set_parameters_eht(Control_Parameters& prms, Model_Parameters& modprms) {
      ...
      if (prms.eht_sce_formula == 1 && n_extra_params < 2) {
        cout << "Error: sce_formula = 1 requires at least 2 additional parameters: 1) slope for "
                "positive charges, 2) slope for negative charges\n";
        cout << "Change eht_params_format to \"eht+2\" or more and ensure that the parameters file "
                "has required number of fields in a line\n";
        exit(0);
      }
      //------------------------------------
          ...
          if (file[i][0] == "<At_constants>") {
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              ...
              for (j = 1; j <= nsh; j++) {
                ...
                for (k = 0; k < modprms.PT[elt].Nzeta[sh]; k++) {
                  ...
                  cout << " coeff[" << k << "]= " << modprms.PT[elt].coeffs[sh][k] << " zetas[" << k
                       << "]= " << modprms.PT[elt].zetas[sh][k] << " Bohr^-1  ";
                }  //for k
                cout << endl;

                if (modprms.PT[elt].Nzeta[sh] == 1) {
                  if (file[i1 + j].size() >= (8 + n_extra_params)) {
                    ;
                    ;
                  }  // all is fine
                  else {
                  ...
                  if (n_extra_params > 0) {
                    modprms.PT[elt].J_param1[sh] = atof(file[i1 + j][8].c_str()) * eV;
                  }  // slope for positive charges, SC-EHT
                  if (n_extra_params > 1) {
                    modprms.PT[elt].J_param2[sh] = atof(file[i1 + j][9].c_str()) * eV;
                  ...
                  if (file[i1 + j].size() >= (10 + n_extra_params)) {
                    ;
                    ;
                  }  // all is fine
                  else {
                    cout << "Error in parameters file: line \n"
                         ...
                         << "\n is not complient with expected format which is = "
                         << prms.eht_params_format << endl;
                    cout << "Missing " << (10 + n_extra_params) - file[i1 + j].size()
                         << " more parameter(s)\n";
                    exit(0);
                  }

                  if (n_extra_params > 0) {
                    modprms.PT[elt].J_param1[sh] = atof(file[i1 + j][10].c_str()) * eV;
                  }  // slope for positive charges, SC-EHT
                  if (n_extra_params > 1) {
                    modprms.PT[elt].J_param2[sh] = atof(file[i1 + j][11].c_str()) * eV;
                  }  // slope for negative charges, SC-EHT
                  if (n_extra_params > 2) {
              ...
              if (file[i1].size() >= 7) {
                ...
                if (file[i1].size() >= 8) {
                  ...
                  modprms.eht_k.set_K_value(
                      ...
                      file[i1][2],
                      file[i1][3],
                      atof(file[i1][7].c_str()) * Angst);  // convert into a.u. of length
                  ...
                  if (file[i1].size() >= 9) {
                    ...
                    modprms.eht_k.set_K_value(
                        ...
                        file[i1][2],
                        file[i1][3],
                        atof(file[i1][8].c_str()) * Angst);  // convert into a.u. of length
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() >= 7) {
                ...
                modprms.eht_k.set_C_value(
                    ...
                    file[i1][2],
                    file[i1][3],
                    atof(file[i1][6].c_str()) * (eV / (Angst * Angst)));  // convert into a.u.
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() >= 6) {
                ...
                cout << "Adding PP parameters for atom: " << file[i1][0] << " and orbital type "
                     << file[i1][1] << " PPa^2 (1/Angst) = "
                ...
                modprms.eht_k.set_PP0_value(file[i1][0],
                                            file[i1][1],
                                            atof(file[i1][3].c_str()) * (eV));  // convert into a.u.
                modprms.eht_k.set_PP1_value(
                    file[i1][0],
                    file[i1][1],
                    atof(file[i1][4].c_str()) * (eV / (Angst)));  // convert into a.u.
                modprms.eht_k.set_PP2_value(
                    file[i1][0],
                    file[i1][1],
                    atof(file[i1][5].c_str()) * (eV / (Angst * Angst)));  // convert into a.u.
            ...
            for (int i1 = i + 1; i1 < end_i; i1++) {
              if (file[i1].size() >= 6) {
                ...
                cout << "Adding PSPS parameters: " << n1 << " , " << n2 << " , " << n3 << " , "
                     << n4 << " K(eV) = " << K << endl;

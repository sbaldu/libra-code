...
*********************************************************************************/
    ...
    void set_parameters_hf(Control_Parameters& prms,
        ...
        for (b = 0; b < Norb; b++) {
          for (c = 0; c < Norb; c++) {
            for (d = 0; d < Norb; d++) {
              ...
              double J_abcd =
                  electron_repulsion_integral(basis_ao[a], basis_ao[b], basis_ao[c], basis_ao[d]);
              double K_adcb =
                  electron_repulsion_integral(basis_ao[a], basis_ao[d], basis_ao[c], basis_ao[b]);

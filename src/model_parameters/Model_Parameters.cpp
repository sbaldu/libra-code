...
namespace liblibra {
  ...
  namespace libmodel_parameters {
    ...
    // EHT_K class:
    int EHT_K::find_data(std::string elt1,
...
*/
    ...
    void EHT_K::set_PPa_value(std::string elt1, std::string orb_type1, double val) {
      ...
      if (i > -1) {
        pp_data[i].PPa_value = val;
      } else {
        pp_data_element x;
        x.elt1 = elt1;
        x.orb_type1 = orb_type1;
        x.PPa_value = val;
        pp_data.push_back(x);
      }
    }
    ...
    double EHT_K::get_PP2_value(std::string elt1, std::string orb_type1) {
      ...
      double res = 0.0;
      if (i > -1) {
        res = pp_data[i].PP2_value;
      }
      return res;
    }

    void EHT_K::set_K_value(int k_indx,
                            std::string elt1,
                            std::string orb_type1,
                            std::string elt2,
                            std::string orb_type2,
...
*/
      ...
      if (i > -1) {
        data[i].K_value[k_indx] = K;
        data[i].is_K_value[k_indx] = 1;
      } else {
        data_element x;
        x.elt1 = elt1;
        x.orb_type1 = orb_type1;
        x.elt2 = elt2;
        x.orb_type2 = orb_type2;
        x.K_value[k_indx] = K;
        x.is_K_value[k_indx] = 1;
        data.push_back(x);
      }
    }

    double EHT_K::get_K_value(int k_indx,
                              std::string elt1,
                              std::string orb_type1,
                              std::string elt2,
                              std::string orb_type2) {
...
*/
      ...
      if (i > -1) {
        if (data[i].is_K_value[k_indx] == 1) {
          res = data[i].K_value[k_indx];
        }
      }
    ...
    void EHT_K::set_C_value(int c_indx,
                            std::string elt1,
                            std::string orb_type1,
                            std::string elt2,
                            std::string orb_type2,
...
*/
      ...
      int i = find_data(elt1, orb_type1, elt2, orb_type2);
      if (i > -1) {
        data[i].C_value[c_indx] = C;
        data[i].is_C_value[c_indx] = 1;
      } else {
        data_element x;
        x.elt1 = elt1;
        x.orb_type1 = orb_type1;
        ...
        x.orb_type2 = orb_type2;
        x.C_value[c_indx] = C;
        x.is_C_value[c_indx] = 1;
        data.push_back(x);
      }
    }

    double EHT_K::get_C_value(int c_indx,
                              std::string elt1,
                              std::string orb_type1,
                              std::string elt2,
                              std::string orb_type2) {
...
*/
      ...
      if (i > -1) {
        if (data[i].is_C_value[c_indx] == 1) {
          res = data[i].C_value[c_indx];
        }
      }
...
*/
      ...
      sz = data.size();
      for (i = 0; i < sz; i++) {
        cout << "i= " << i << " " << data[i].elt1 << " " << data[i].orb_type1 << " " << data[i].elt2
             << " " << data[i].orb_type2 << "\n";
...
*/
      ...
      for (k = 0; k < 5; k++) {
        eht_K[k].reserve(size * size);
        eht_K[k].resize(size * size, 0.0);
        eht_C[k].reserve(size * size);
        eht_C[k].resize(size * size, 0.0);
      }
      ...
      for (int I = 0; I < size; I++) {  // create a map of distinct types of orbitals
        ...
        if (it_type ==
            at_types.end()) {  // so, this combination of element and AO shell is new type
          at_types.insert(std::pair<pair<string, string>, int>(pr, ind));
          ind++;  // so add it to the array
        }
      ...
      for (it_type = at_types.begin(); it_type != at_types.end(); it_type++) {
        cout << it_type->first.first << "  " << it_type->first.second << "  " << it_type->second
             << endl;
      }

      //========================= Part 2 =======================================
      int ntyp = at_types.size();  // number of distinct orbital types
      vector<vector<vector<double> > > k_vals(
          5, vector<vector<double> >(ntyp, vector<double>(ntyp, 0.0)));
      vector<vector<vector<double> > > c_vals(
          5, vector<vector<double> >(ntyp, vector<double>(ntyp, 0.0)));
      ...
      for (int I = 0; I < size; I++) {
        int i1 =
            at_types[std::pair<std::string, std::string>(basis_ao[I].element, basis_ao[I].ao_shell)];

        for (int J = 0; J < size; J++) {
          // New procedure
          int i2 = at_types[std::pair<std::string, std::string>(basis_ao[J].element,
                                                                basis_ao[J].ao_shell)];
...
*/
      ...
      for (int n = 0; n < nat; n++) {  // create a map of distinct types of atoms
        ...
        if (it_type == at_types.end()) {  // new type
          at_types.insert(std::pair<std::string, int>(mol_at_types[n], ind));
          ind++;
        }
      ...
      for (it_type = at_types.begin(); it_type != at_types.end(); it_type++) {
        ...
        cout << "--- the pseudopotential consists of " << orb_types[cnt].size()
             << " orbital components" << endl;
        for (int i = 0; i < orb_types[cnt].size(); i++) {
          cout << "--- --- orbital component " << i << " is of type " << orb_types[cnt][i]
               << " with parameters: alp(a.u.)= " << ppa_vals[cnt][i]
               << " PP0(a.u.)= " << pp0_vals[cnt][i] << " PP1(a.u.)= " << pp1_vals[cnt][i]
               << " PP2(a.u.)= " << pp2_vals[cnt][i] << endl;
      ...
      for (int n = 0; n < nat; n++) {
        int i1 = at_types
            [mol_at_types
                 [n]];  // index of given atomic type within the set of the atom types present in this system
    ...
    void set_default_elements(map<std::string, pElement>& PT) {
      ...
      elt._set("Be", 4);
      PT["Be"] = elt;
      PT["Be"].set_mass(9.0122);
      elt._set("B", 5);
      PT["B"] = elt;
      PT["B"].set_mass(10.81);
      elt._set("C", 6);
      PT["C"] = elt;
      PT["C"].set_mass(12.011);
      elt._set("N", 7);
      PT["N"] = elt;
      PT["N"].set_mass(14.007);
      elt._set("O", 8);
      PT["O"] = elt;
      PT["O"].set_mass(15.999);
      elt._set("F", 9);
      PT["F"] = elt;
      PT["F"].set_mass(18.998);
      elt._set("Ne", 10);
      PT["Ne"] = elt;
      PT["Ne"].set_mass(20.180);

      elt._set("Na", 11);
      PT["Na"] = elt;
      PT["Na"].set_mass(22.990);
      elt._set("Mg", 12);
      PT["Mg"] = elt;
      PT["Mg"].set_mass(24.305);
      elt._set("Al", 13);
      PT["Al"] = elt;
      PT["Al"].set_mass(26.982);
      elt._set("Si", 14);
      PT["Si"] = elt;
      PT["Si"].set_mass(28.085);
      elt._set("P", 15);
      PT["P"] = elt;
      PT["P"].set_mass(30.974);
      elt._set("S", 16);
      PT["S"] = elt;
      PT["S"].set_mass(32.060);
      elt._set("Cl", 17);
      PT["Cl"] = elt;
      PT["Cl"].set_mass(35.450);
      elt._set("Ar", 18);
      PT["Ar"] = elt;
      PT["Ar"].set_mass(39.948);

      elt._set("K", 19);
      PT["K"] = elt;
      PT["K"].set_mass(39.098);
      elt._set("Ca", 20);
      PT["Ca"] = elt;
      PT["Ca"].set_mass(40.078);
      elt._set("Sc", 21);
      PT["Sc"] = elt;
      PT["Sc"].set_mass(44.956);
      elt._set("Ti", 22);
      PT["Ti"] = elt;
      PT["Ti"].set_mass(47.867);
      elt._set("V", 23);
      PT["V"] = elt;
      PT["V"].set_mass(50.942);
      elt._set("Cr", 24);
      PT["Cr"] = elt;
      PT["Cr"].set_mass(51.996);
      elt._set("Mn", 25);
      PT["Mn"] = elt;
      PT["Mn"].set_mass(54.938);
      elt._set("Fe", 26);
      PT["Fe"] = elt;
      PT["Fe"].set_mass(55.845);
      elt._set("Co", 27);
      PT["Co"] = elt;
      PT["Co"].set_mass(58.933);
      elt._set("Ni", 28);
      PT["Ni"] = elt;
      PT["Ni"].set_mass(58.693);
      elt._set("Cu", 29);
      PT["Cu"] = elt;
      PT["Cu"].set_mass(63.546);
      elt._set("Zn", 30);
      PT["Zn"] = elt;
      PT["Zn"].set_mass(65.380);
      elt._set("Ga", 31);
      PT["Ga"] = elt;
      PT["Ga"].set_mass(69.723);
      elt._set("Ge", 32);
      PT["Ge"] = elt;
      PT["Ge"].set_mass(72.630);
      elt._set("As", 33);
      PT["As"] = elt;
      PT["As"].set_mass(74.922);
      elt._set("Se", 34);
      PT["Se"] = elt;
      PT["Se"].set_mass(78.960);
      elt._set("Br", 35);
      PT["Br"] = elt;
      PT["Br"].set_mass(79.904);
      elt._set("Kr", 36);
      PT["Kr"] = elt;
      PT["Kr"].set_mass(83.798);

      elt._set("Ru", 44);
      PT["Ru"] = elt;
      PT["Ru"].set_mass(101.07);

      elt._set("Ta", 73);
      PT["Ta"] = elt;
      PT["Ta"].set_mass(180.95);
    }
    ...
    void Model_Parameters::set_PT_mapping(const vector<AO>& basis_ao) {
      ...
      orb_params.reserve(size);
      orb_params.resize(size);
    ...
    void set_parameters_eht_mapping1(Model_Parameters& modprms,
                                     int nat,

...
*/
    ...
    public:
      ...
      void set_K_value(int k_indx,
                       std::string elt1,
                       std::string orb_type1,
                       std::string elt2,
                       std::string orb_type2,
                       double K);
      double get_K_value(int k_indx,
                         std::string elt1,
                         std::string orb_type1,
                         std::string elt2,
                         std::string orb_type2);
      ...
      friend bool operator!=(const EHT_K& m1, const EHT_K& m2) { return (!(m1 == m2)); }
    };
...
*/
    ...
    public:
      ...
      vector<vector<double> >
          eht_K;  ///< eht_K[k_indx][I*N+J] = is the k_indx-type parameter for pair of orbitals I and J - each of special type and on special atom
      vector<vector<double> >
          eht_C;  ///< eht_C[c_indx][I*N+J] = is the parameter for pair of orbitals I and J - each of special type and on special atom
      ...
      friend bool operator!=(const mEHT_K& m1, const mEHT_K& m2) { return (!(m1 == m2)); }
    };
...
*/
    ...
    public:
      ...
      map<std::string, double>
          IP;  ///< Ionization potential for a given shell (first = 1s, 2s, 2p, ... 3d, so on)
      map<std::string, double>
          EA;  ///< Electron affinity for a given shell (first = 1s, 2s, 2p, ... 3d, so on)
      ...
      void _set(std::string en, int z, int nv, double zeff, map<std::string, double> ip) {
        elt_name = en;
        Z = z;
        Nval = nv;
        IP = ip;
      ...
      friend bool operator!=(const pElement& m1, const pElement& m2) { return (!(m1 == m2)); }
    };
...
*/
    ...
    public:
      ...
      friend bool operator!=(const OrbParams& m1, const OrbParams& m2) { return (!(m1 == m2)); }
    };
...
*/
    ...
    public:
      ...
      vector<double>
          eri;  ///< precomputed electron repulsion integrals (only (ss|ss) type between all pairs of atoms)

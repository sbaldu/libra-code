...
namespace liblibra {
  ...
  namespace libforcefield {
    ...
    int ForceField::get_elec_parameters(string ff_type1,
                                        string ff_type2,
                                        std::string excl_pair, /*Inputs*/
                                        double q1,
                                        double q2,
                                        int is_q1,
                                        int is_q2,
...
******************************************************************/
      ...
      if (!is_qi) {
        if (ff_atom1_indx > -1) {  // if the record has been found
          if (Atom_Records[ff_atom1_indx].is_Atom_partial_charge) {
            qi = Atom_Records[ff_atom1_indx].Atom_partial_charge;
            is_qi = 1;
          }
        }
      } else {
      ...
      if (!is_qj) {
        if (ff_atom2_indx > -1) {  // if the record has been found
          if (Atom_Records[ff_atom2_indx].is_Atom_partial_charge) {
            qj = Atom_Records[ff_atom2_indx].Atom_partial_charge;
            is_qj = 1;
          }
        }
      } else {
      ...
      if (excl_pair == "12") {
        if (is_elec_scale12) {
          prms["scale"] = elec_scale12;
        }
      } else if (excl_pair == "13") {
        if (is_elec_scale13) {
          prms["scale"] = elec_scale13;
        }
      } else if (excl_pair == "14") {
        if (is_elec_scale14) {
          prms["scale"] = elec_scale14;
        }
      }
      ...
      prms["is_cutoff"] = 0;
      double R_on, R_off;
      if ((is_R_elec_off == 1) && (is_R_elec_on == 1)) {
        is_cut = 1;
        R_off = R_elec_off;
        R_on = R_elec_on;

...
*/
  ...
  *********************/
    ...
    void ForceField::load(boost::property_tree::ptree& pt, std::string path, int& status) {
      ...
      libio::load(pt, path + ".ForceField_Name", ForceField_Name, is_ForceField_Name);
      if (is_ForceField_Name == 1) {
        status = 1;
      }
      libio::load(pt, path + ".bond_functional", bond_functional, is_bond_functional);
      if (is_bond_functional == 1) {
        status = 1;
      }
      libio::load(pt, path + ".angle_functional", angle_functional, is_angle_functional);
      if (is_angle_functional == 1) {
        status = 1;
      }
      libio::load(pt, path + ".dihedral_functional", dihedral_functional, is_dihedral_functional);
      ...
      libio::load(pt, path + ".elec_functional", elec_functional, is_elec_functional);
      ...
      libio::load(pt, path + ".mb_excl_functional", mb_excl_functional, is_mb_excl_functional);
      ...
      libio::load(pt, path + ".reciprocal_degree_x", reciprocal_degree_x, is_reciprocal_degree_x);
      if (is_reciprocal_degree_x == 1) {
        status = 1;
      }
      libio::load(pt, path + ".reciprocal_degree_y", reciprocal_degree_y, is_reciprocal_degree_y);
      if (is_reciprocal_degree_y == 1) {
        status = 1;
      }
      libio::load(pt, path + ".reciprocal_degree_z", reciprocal_degree_z, is_reciprocal_degree_z);
      ...
      libio::load(pt, path + ".sigma_comb_rule", sigma_comb_rule, is_sigma_comb_rule);
      if (is_sigma_comb_rule == 1) {
        status = 1;
      }
      libio::load(pt, path + ".epsilon_comb_rule", epsilon_comb_rule, is_epsilon_comb_rule);
...
*********************************************************************/
      ...
      if (last_indx == -2) {  // In this case the record was not added to Atom_Records array
        res = -1;
                 ...
                 -1) {  // The atom record is valid but does not exist in Atom_Records array
...
*********************************************************************/
      ...
      if (int_types) {
        ...
        last_indx =
            Bond_Record_Index(rec.Atom1_ff_int_type, rec.Atom2_ff_int_type, bt_indx, bnd_indxs);
      } else if (sym_types) {
      ...
      if (last_indx == -2) {  // In this case the record was not added to Bond_Records array
        res = -1;
                 ...
                 -1) {  // The bond record is valid but does not exist in Bond_Records array
                        // so it will be added completely
...
*/
    ...
    int ForceField::Angle_Record_Index(int Atom1_ff_int_type,
                                       int Atom2_ff_int_type,
...
*****************************************************************/
      ...
      for (int i = 0; i < sz; i++) {
        ...
        if (cmpr2) {
          ...
          cout << "cmpr11 = " << cmpr11 << " cmpr33 = " << cmpr33 << " cmpr13 = " << cmpr13
               << " cmpr31 = " << cmpr31 << endl;
...
*********************************************************************/
      ...
      int int_types =
          (rec.is_Atom1_ff_int_type && rec.is_Atom2_ff_int_type && rec.is_Atom3_ff_int_type);
      ...
      if (int_types) {
        // Search on the basis of int_types
        last_indx = Angle_Record_Index(rec.Atom1_ff_int_type,
                                       rec.Atom2_ff_int_type,
                                       rec.Atom3_ff_int_type,
                                       at_indx,
                                       order,
                                       ang_indxs);
      } else if (sym_types) {
        // Search on the basis of symbolic types
        last_indx = Angle_Record_Index(
            rec.Atom1_ff_type, rec.Atom2_ff_type, rec.Atom3_ff_type, at_indx, order, ang_indxs);
      } else {
        // Record will not be added to the table of Angle_Records since it is
        // useless (no labels defined neither integer nor symbolic)
        last_indx = -2;
      ...
      if (last_indx == -2) {  // In this case the record was not added to Angle_Records array
        res = -1;
                 ...
                 -1) {  // The angle record is valid but does not exist in Angle_Records array
                        // so it will be added completely
...
*********************************************************************/
      ...
      if (rec.is_Atom1_ff_type && rec.is_Atom2_ff_type && rec.is_Atom3_ff_type) {
        for (int i = 0; i < sz; i++) {
          if (Angle_Records[i].is_Atom1_ff_type && Angle_Records[i].is_Atom2_ff_type &&
        ...
        std::cout << "To add angle type record to the force field its Atom1_ff_type, Atom2_ff_type "
                     " and Atom3_ff_type should be defined"
                  << std::endl;
        res = -1;
      }
...
*****************************************************************/
      ...
      for (int i = 0; i < sz; i++) {
        if (Improper_Records[i].is_Atom1_ff_int_type && Improper_Records[i].is_Atom2_ff_int_type &&
            Improper_Records[i].is_Atom3_ff_int_type && Improper_Records[i].is_Atom4_ff_int_type) {
          res = 0;
          if (Improper_Records[i].Atom2_ff_int_type == Atom2_ff_int_type) {
            ...
            if (Improper_Records[i].Atom1_ff_int_type == Atom1_ff_int_type) {
              if (Improper_Records[i].Atom3_ff_int_type == Atom3_ff_int_type &&
                ...
                res = 1;
              } else if (Improper_Records[i].Atom3_ff_int_type == Atom4_ff_int_type &&
                         Improper_Records[i].Atom4_ff_int_type == Atom3_ff_int_type) {
                res = 1;
              }
            }
              ...
              if (Improper_Records[i].Atom1_ff_int_type == Atom1_ff_int_type &&
                  Improper_Records[i].Atom4_ff_int_type == Atom4_ff_int_type) {
                res = 1;
              } else if (Improper_Records[i].Atom1_ff_int_type == Atom4_ff_int_type &&
                         Improper_Records[i].Atom4_ff_int_type == Atom1_ff_int_type) {
                res = 1;
              }
            }
              ...
              if (Improper_Records[i].Atom3_ff_int_type == Atom3_ff_int_type &&
                ...
                res = 1;
              } else if (Improper_Records[i].Atom3_ff_int_type == Atom1_ff_int_type &&
                         Improper_Records[i].Atom1_ff_int_type == Atom3_ff_int_type) {
                res = 1;
              }
            }
...
*****************************************************************/
      ...
      for (int i = 0; i < sz; i++) {
        ...
        cmpr22 = cmpr33 = 0;
        if (Dihedral_Records[i].is_Atom1_ff_int_type && Dihedral_Records[i].is_Atom2_ff_int_type &&
            Dihedral_Records[i].is_Atom3_ff_int_type && Dihedral_Records[i].is_Atom4_ff_int_type) {
...
*****************************************************************/
      vector<int> tmp;
      return Dihedral_Record_Index(
          Atom1_ff_type, Atom2_ff_type, Atom3_ff_type, Atom4_ff_type, -1, 0, tmp);
    }
...
*********************************************************************/
      ...
      if (int_types) {
        // Search on the basis of int_types
        last_indx = Dihedral_Record_Index(rec.Atom1_ff_int_type,
                                          rec.Atom2_ff_int_type,
                                          rec.Atom3_ff_int_type,
                                          rec.Atom4_ff_int_type,
                                          dt_indx,
                                          order,
                                          dih_indxs);
      } else if (sym_types) {
        // Search on the basis of symbolic types
        last_indx = Dihedral_Record_Index(rec.Atom1_ff_type,
                                          rec.Atom2_ff_type,
                                          rec.Atom3_ff_type,
                                          rec.Atom4_ff_type,
                                          dt_indx,
      ...
      if (last_indx == -2) {  // In this case the record was not added to Dihedral_Records array
        res = -1;
                 ...
                 -1) {  // The dihedral record is valid but does not exist in Dihedral_Records array
                        // so it will be added completely
...
*********************************************************************/
      ...
      if (rec.is_Atom1_ff_type && rec.is_Atom2_ff_type && rec.is_Atom3_ff_type &&
        ...
        std::cout << "To add dihedral type record to the force field its Atom1_ff_type, "
...
*********************************************************************/
      ...
      if (int_types) {
        // Search on the basis of int_types
        last_indx = Improper_Record_Index(rec.Atom1_ff_int_type,
                                          rec.Atom2_ff_int_type,
                                          rec.Atom3_ff_int_type,
                                          rec.Atom4_ff_int_type);
...
*/
      ...
      if (last_indx == -2) {  // In this case the record was not added to Improper_Records array
        res = -1;
                 ...
                 -1) {  // The dihedral record is valid but does not exist in Improper_Records array
                        // so it will be added completely
...
*********************************************************************/
      ...
      if (last_indx == -2) {  // In this case the record was not added to Fragment_Records array
        res = -1;
                 ...
                 -1) {  // The atom record is valid but does not exist in Fragment_Records array
                        // so it will be added completely

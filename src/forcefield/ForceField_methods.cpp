  ...
  namespace libforcefield {
    ...
    int ForceField::is_valid_atom_type(std::string ff_type) {
      ...
      if (is_ForceField_Name) {
        ...
        if (indx == -1) {
          cout << "Error: Force field " << ForceField_Name << " does not have atom type " << ff_type
               << endl;
          res = 0;
        }
      } else {
        cout << "Warning!: Force field name is not defined. But this allows you to manually define "
                "any atom type!\n";
        res = 1;
      }
      return 1;
    ...
    int ForceField::is_valid_fragment_type(std::string ff_type) { return 1; }

    void ForceField::bond_r0_rule(
        string ff_type1, string ff_type2, double bond_order, double& r0, int& is_r0) {
      ...
      if (ff_type1_indx == -1 || ff_type2_indx == -1) {
        ...
        if (Atom_Records[ff_type1_indx].is_Atom_radius) {
          r1 = Atom_Records[ff_type1_indx].Atom_radius;
        } else {
          is_r0 = 0;
        }
        if (Atom_Records[ff_type2_indx].is_Atom_radius) {
          r2 = Atom_Records[ff_type2_indx].Atom_radius;
        } else {
          is_r0 = 0;
        }
    ...
    void ForceField::bond_K_rule(
        string ff_type1, string ff_type2, double bond_order, double& K, int& is_K) {
      int ff_type1_indx, ff_type2_indx;
      ...
      if (ff_type1_indx == -1 || ff_type2_indx == -1) {
        ...
        if (ForceField_Name == "UFF") {
          ...
          if (Atom_Records[ff_type1_indx].is_Atom_Z_star) {
            z1 = Atom_Records[ff_type1_indx].Atom_Z_star;
          } else {
            is_K = 0;
          }
          if (Atom_Records[ff_type2_indx].is_Atom_Z_star) {
            z2 = Atom_Records[ff_type2_indx].Atom_Z_star;
          } else {
            is_K = 0;
          }
          ...
          if (is_K) {
            ...
            if (Atom_Records[ff_type1_indx].is_Atom_GMP &&
              ...
              double xi2 = Atom_Records[ff_type2_indx].Atom_GMP;
              double el_corr =
                  r1 * r2 * (xi1 - 2.0 * sqrt(xi1 * xi2) + xi2) / (xi1 * r1 + xi2 * r2);
              r_tot -= el_corr;
            }
    ...
    void ForceField::bond_D_rule(
        string ff_type1, string ff_type2, double bond_order, double& D, int& is_D) {
      is_D = 1;
      if (ForceField_Name == "UFF") {
        is_D = 0;
      } else if (ForceField_Name == "DREIDING") {
    ...
    void ForceField::bond_alpha_rule(
        string ff_type1, string ff_type2, double bond_order, double& alpha, int& is_alpha) {
      is_alpha = 1;
      if (ForceField_Name == "UFF") {
        is_alpha = 0;
      } else if (ForceField_Name == "DREIDING") {
    ...
    int ForceField::get_bond_parameters(string ff_type1,
                                        string ff_type2, /*Inputs*/
                                        double bond_order,
...
******************************************************************/
      ...
      if (ff_bond_indx > -1) {  // if the record has been found
        ...
        if (Bond_Records[ff_bond_indx].is_Bond_r_eq) {
          r0 = Bond_Records[ff_bond_indx].Bond_r_eq;
          is_r0 = 1;
        }
        // Force constants
        if (Bond_Records[ff_bond_indx].is_Bond_k_bond) {
          K = Bond_Records[ff_bond_indx].Bond_k_bond;
          is_K = 1;
        }
        // Energy depth
        if (Bond_Records[ff_bond_indx].is_Bond_D_bond) {
          D = Bond_Records[ff_bond_indx].Bond_D_bond;
          is_D = 1;
        }
        // Morse shape parameter
        if (Bond_Records[ff_bond_indx].is_Bond_alpha) {
          alpha = Bond_Records[ff_bond_indx].Bond_alpha;
          is_alpha = 1;
        }
      }
      ...
      if (!is_r0) {
        bond_r0_rule(ff_type1, ff_type2, bond_order, r0, is_r0);
      }
      if (!is_K) {
        bond_K_rule(ff_type1, ff_type2, bond_order, K, is_K);
      }
      if (!is_D) {
        bond_D_rule(ff_type1, ff_type2, bond_order, D, is_D);
      }
      if (!is_alpha) {
        bond_alpha_rule(ff_type1, ff_type2, bond_order, alpha, is_alpha);
      }

...
namespace liblibra {
  ...
  namespace libforcefield {
    ...
    int ForceField::set_ff_epsilon_and_sigma(int sz,
                                             vector<string> types,
                                             double** epsilon,
                                             double** sigma) {
      int res = 1;
      for (int i = 0; i < sz; i++) {
        ...
        if (ff_atom1_indx == -1) {
          *epsilon[i] = 0.0;
          *sigma[i] = 3.0 * Angst;
          cout << "Warning: In ForceField::set_ff_epsilon_and_sigma : Can not fing the atom type "
                  "ff_atom type in force field "
               << ForceField_Name << "\n";

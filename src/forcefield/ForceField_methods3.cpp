...
namespace liblibra {
  ...
  namespace libforcefield {
    ...
    void ForceField::angle_theta_0_rule(std::string ff_type1,
                                        std::string ff_type2,
                                        std::string ff_type3,
                                        double bond_order12,
                                        double bond_order23,
      ...
      if (ff_type2_indx == -1) {
        ...
        if (ForceField_Name == "UFF" || ForceField_Name == "DREIDING") {
          if (Atom_Records[ff_type2_indx].is_Atom_theta) {
            theta_0 = deg_to_rad * Atom_Records[ff_type2_indx].Atom_theta;
          } else {
            is_theta_0 = 0;
          }
    ...
    void ForceField::angle_k_theta_rule(std::string ff_type1,
                                        std::string ff_type2,
                                        std::string ff_type3,
                                        double bond_order12,
                                        double bond_order23,
      ...
      if (ForceField_Name == "DREIDING") {
        ...
        if (ff_type1_indx == -1 || ff_type2_indx == -1 || ff_type3_indx == -1) {
          is_k_theta = 0;
        } else {
          if (ForceField_Name == "UFF") {
            ...
            if (Atom_Records[ff_type1_indx].is_Atom_Z_star) {
              Zi = Atom_Records[ff_type1_indx].Atom_Z_star;
              is_Zi = 1;
            }
            if (Atom_Records[ff_type3_indx].is_Atom_Z_star) {
              Zk = Atom_Records[ff_type3_indx].Atom_Z_star;
              is_Zk = 1;
            }
            ...
            if (is_rij && is_rjk && is_theta_0 && is_Zi && is_Zk) {
              double rik2, rik, cs;
              rik2 = rij * rij + rjk * rjk - 2.0 * rij * rjk * cos(theta_0);
              rik = sqrt(rik2);
              cs = cos(theta_0);
              k_theta = 664.12 * (Zi * Zk / (rik2 * rik2 * rik)) *
                        (3.0 * rij * rjk * (1.0 - cs * cs) - rik2 * cs);
            } else {
              is_k_theta = 0;
            }
          } else {
    ...
    int ForceField::get_angle_parameters(string ff_type1,
                                         string ff_type2,
                                         string ff_type3,
                                         double bond_order12,
                                         double bond_order23,
                                         int coordination,         /*Inputs*/
...
******************************************************************/
      ...
      cout << "ff_type1 = " << ff_type1 << "ff_type2 = " << ff_type2 << "ff_type3 = " << ff_type3
           << endl;
      ...
      if (ff_angle_indx > -1) {  // if the record has been found
        ...
        if (Angle_Records[ff_angle_indx].is_Angle_theta_eq) {
          theta_0 = deg_to_rad * Angle_Records[ff_angle_indx].Angle_theta_eq;
          is_theta_0 = 1;
        }
        // Angle bending force constant
        if (Angle_Records[ff_angle_indx].is_Angle_k_angle) {
          k_theta = Angle_Records[ff_angle_indx].Angle_k_angle;
          is_k_theta = 1;
        }
        //    // Angle coordination
        //    if(Angle_Records[ff_angle_indx].is_Angle_k_angle){
      ...
      cout << " is_theta_0 = " << is_theta_0 << " is_k_theta = " << is_k_theta
           << " is_cos_theta_0 = " << is_cos_theta_0 << " is_C0 = " << is_C0 << " is_C1 = " << is_C1
           << " is_C2 = " << is_C2 << endl;
      cout << " theta_0 = " << theta_0 << " k_theta = " << k_theta
           << " is_cos_theta_0 = " << cos_theta_0 << " C0 = " << C0 << " C1 = " << C1
           << " C2 = " << C2 << endl;
      ...
      cout << "The parameters determined will be used with the angle bending functional = "
           << angle_functional << endl;
      ...
      if (status) {
        ...
        cout << " insufficient (the interaction will not be added)\n";
      }

...
namespace liblibra {
  ...
  namespace libforcefield {
    ...
    public:
      ...
      int is_ForceField_Name;

      std::string bond_functional;
      int is_bond_functional;
      std::string angle_functional;
      int is_angle_functional;
      std::string dihedral_functional;
      int is_dihedral_functional;
      std::string oop_functional;
      int is_oop_functional;
      std::string vdw_functional;
      int is_vdw_functional;
      std::string elec_functional;
      int is_elec_functional;
      std::string mb_functional;
      int is_mb_functional;
      std::string cg_functional;
      int is_cg_functional;
      std::string mb_excl_functional;
      int is_mb_excl_functional;

      //   std::string stress_opt;            int is_sress_opt;

      std::string system_pbc;
      int is_system_pbc;
      int pbc_degree_x;
      int is_pbc_degree_x;
      int pbc_degree_y;
      ...
      int pbc_degree_z;
      int is_pbc_degree_z;
      int reciprocal_degree_x;
      int is_reciprocal_degree_x;
      int reciprocal_degree_y;
      int is_reciprocal_degree_y;
      int reciprocal_degree_z;
      int is_reciprocal_degree_z;
      double R_vdw_on;
      int is_R_vdw_on;
      double R_vdw_off;
      int is_R_vdw_off;
      ...
      int is_R_elec_off;
      double R_vlist;
      int is_R_vlist;  // Verlet list radius
      double elec_etha;
      int is_elec_etha;

      std::string sigma_comb_rule;
      int is_sigma_comb_rule;
      std::string epsilon_comb_rule;
      int is_epsilon_comb_rule;
      double vdw_scale12;
      int is_vdw_scale12;
      double vdw_scale13;
      int is_vdw_scale13;
      ...
      int Dihedral_Record_Index(
          std::string, std::string, std::string, std::string, int, int, vector<int>&);
      ...
      string dreiding_type(
          string elt, int geometry, string func_grp, int min_ring, int& coordination);
      ...
      void angle_theta_0_rule(std::string ff_type1,
                              std::string ff_type2,
                              std::string ff_type3,
                              double bond_order12,
                              double bond_order23,
                              ...
                              double& theta_0,
                              int& is_theta_0);
      void angle_k_theta_rule(std::string ff_type1,
                              std::string ff_type2,
                              std::string ff_type3,
                              double bond_order12,
                              double bond_order23,
      ...
      void dihedral_rule(std::string ff_type1,
                         std::string ff_type2,
                         std::string ff_type3,
                         std::string ff_type4,
                         double bond_order12,
      ...
      void oop_rule(std::string ff_type1,
                    std::string ff_type2,
                    std::string ff_type3,
                    std::string ff_type4,
                    double& K,
      ...
      string get_atom_type(
        ...
        if (ForceField_Name == "UFF") {
          res = uff_type(elt, geometry, func_grp, min_ring, coordination);
        } else if (ForceField_Name == "DREIDING") {
          res = dreiding_type(elt, geometry, func_grp, min_ring, coordination);
        } else if (ForceField_Name == "GAFF") {
          res = gaff_type(elt, geometry, func_grp, min_ring, coordination);
        } else if (ForceField_Name == "MMFF94") {
          res = mmff94_type(elt, geometry, func_grp, min_ring, coordination);
        } else if (ForceField_Name == "TRIPOS") {
          res = tripos_type(elt, geometry, func_grp, min_ring, coordination);
        } else if (ForceField_Name == "TIP3P") {
          res = tip3p_type(elt, geometry, func_grp, min_ring, coordination);
        }
        return res;
      }
      ...
      int get_bond_parameters(string,
                              string,
                              double,
                              map<string, double>&);  // ForceField_methods.cpp
      int get_angle_parameters(string,
                               string,
                               string,
                               double,
                               double,
                               int,
                               map<string, double>&);  // ForceField_methods3.cpp
      int get_dihedral_parameters(string,
                                  string,
                                  string,
                                  string,
                                  double,
                                  double,
                                  double,
                                  map<string, double>&);  // ForceField_methods4.cpp
      int get_oop_parameters(
          string, string, string, string, map<string, double>&);  // ForceField_methods5.cpp
      // ForceField_methods1.cpp
      int get_vdw_parameters(string,
                             string,
                             string,
                             map<string, double>&);  // ForceField_methods1.cpp
      int get_elec_parameters(string,
                              string,
                              string,
                              double,
                              double,
                              int,
                              int,
                              map<string, double>&);  // ForceField_methods6.cpp
      int get_vdw_parameters(int sz,
                             vector<string> types,
                             double** epsilon,
                             double** sigma);  // ForceField_methods1.cpp

...
namespace liblibra {
  ...
  namespace libforcefield {
    ...
    void ForceField::vdw_sigma_rule(string ff_type1, string ff_type2, double& sigma, int& is_sigma) {
      ...
      if (ff_type1_indx == -1 || ff_type2_indx == -1) {
        ...
        if (sigma_comb_rule == "SLATER-KIRKWOOD") {
          if (Atom_Records[ff_type1_indx].is_Atom_A_scale) {
            A_i = Atom_Records[ff_type1_indx].Atom_A_scale;
          } else {
            is_sigma = 0;
          }
          if (Atom_Records[ff_type2_indx].is_Atom_A_scale) {
            A_j = Atom_Records[ff_type2_indx].Atom_A_scale;
          } else {
            is_sigma = 0;
          }
          if (Atom_Records[ff_type1_indx].is_Atom_alpha) {
            a_i = Atom_Records[ff_type1_indx].Atom_alpha;
          } else {
            is_sigma = 0;
          }
          if (Atom_Records[ff_type2_indx].is_Atom_alpha) {
            a_j = Atom_Records[ff_type2_indx].Atom_alpha;
          } else {
            is_sigma = 0;
          }
        } else {
          if (Atom_Records[ff_type1_indx].is_Atom_sigma) {
            R_i = Atom_Records[ff_type1_indx].Atom_sigma;
          } else {
            is_sigma = 0;
          }
          if (Atom_Records[ff_type2_indx].is_Atom_sigma) {
            R_j = Atom_Records[ff_type2_indx].Atom_sigma;
          } else {
            is_sigma = 0;
          }
        }
        ...
        if (is_sigma) {
          if (is_sigma_comb_rule) {
            if (sigma_comb_rule == "SUM") {
              sigma = (R_i + R_j);
            } else if (sigma_comb_rule == "ARITHMETIC") {
              sigma = 0.5 * (R_i + R_j);
            } else if (sigma_comb_rule == "GEOMETRIC") {
              sigma = sqrt(R_i * R_j);
    ...
    void ForceField::vdw_epsilon_rule(string ff_type1,
                                      string ff_type2,
                                      double& epsilon,
                                      int& is_epsilon) {
      int ff_type1_indx, ff_type2_indx;
      ...
      if (epsilon_comb_rule == "SLATER-KIRKWOOD") {
        ...
        if (Atom_Records[ff_type1_indx].is_Atom_G_scale) {
          G_i = Atom_Records[ff_type1_indx].Atom_G_scale;
        } else {
          is_epsilon = 0;
        }
        if (Atom_Records[ff_type2_indx].is_Atom_G_scale) {
          G_j = Atom_Records[ff_type2_indx].Atom_G_scale;
        } else {
          is_epsilon = 0;
        }
        if (Atom_Records[ff_type1_indx].is_Atom_alpha) {
        ...
        if (Atom_Records[ff_type1_indx].is_Atom_N_eff) {
          N_i = Atom_Records[ff_type1_indx].Atom_N_eff;
        } else {
          is_epsilon = 0;
        }
        if (Atom_Records[ff_type2_indx].is_Atom_N_eff) {
          N_j = Atom_Records[ff_type2_indx].Atom_N_eff;
        } else {
          is_epsilon = 0;
        }
      } else {
        if (Atom_Records[ff_type1_indx].is_Atom_epsilon) {
          D_i = Atom_Records[ff_type1_indx].Atom_epsilon;
        } else {
          is_epsilon = 0;
        }
        if (Atom_Records[ff_type2_indx].is_Atom_epsilon) {
          D_j = Atom_Records[ff_type2_indx].Atom_epsilon;
        } else {
          is_epsilon = 0;
        }
      }
      ...
      if (is_epsilon) {
        if (is_epsilon_comb_rule) {
          if (epsilon_comb_rule == "PROD") {
            epsilon = (D_i * D_j);
          } else if (epsilon_comb_rule == "GEOMETRIC") {
    ...
    int ForceField::get_vdw_parameters(string ff_type1,
                                       string ff_type2,
                                       string excl_pair,         /*Inputs*/
...
******************************************************************/
      ...
      if (!is_sigma) {
        vdw_sigma_rule(ff_type1, ff_type2, sigma, is_sigma);
      }
      if (!is_epsilon) {
        vdw_epsilon_rule(ff_type1, ff_type2, epsilon, is_epsilon);
      }
      ...
      if (excl_pair == "12") {
        if (is_vdw_scale12) {
          prms["scale"] = vdw_scale12;
        }
      } else if (excl_pair == "13") {
        if (is_vdw_scale13) {
          prms["scale"] = vdw_scale13;
        }
      } else if (excl_pair == "14") {
        if (is_vdw_scale14) {
          prms["scale"] = vdw_scale14;
        }
      }
      ...
      double R_on, R_off;
      if ((is_R_vdw_off == 1) && (is_R_vdw_on == 1)) {
        is_cut = 1;
        R_off = R_vdw_off;
        R_on = R_vdw_on;
      ...
      if (vdw_functional == "LJ" || vdw_functional == "Buffered14_7" || mb_functional == "vdw_LJ" ||
          mb_functional == "vdw_LJ1" || mb_functional == "LJ_Coulomb") {
        prms["sigma"] = sigma;
    ...
    int ForceField::get_vdw_parameters(int sz,
                                       vector<string> types,
                                       double** epsilon,
                                       double** sigma) {
      // sz - number of atoms
      ...
      for (int i = 0; i < sz; i++) {
        ...
        if (ff_atom1_indx == -1) {
          *epsilon[i] = 0.0;
          *sigma[i] = 0.0;
          cout << "Warning: In ForceField::get_vdw_parameters : Can not fing the atom type ff_atom "
                  "type in force field "
               << ForceField_Name << "\n";
        ...
        cout << "i = " << i << "type = " << types[i] << " epsilon = " << *epsilon[i]
             << " sigma = " << *sigma[i] << endl;
      }  // for i

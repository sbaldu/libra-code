...
namespace liblibra {
  ...
  namespace libgraph {
    ...
    int delete_item(vector<DATA_TYPE>& data, DATA_TYPE item) {
      int sz = data.size();
      for (int i = 0; i < sz; i++) {
        if (data[i] == item) {
          data[i] = data[sz - 1];
    ...
    template <class DATA_TYPE>
    int replace_item(vector<DATA_TYPE>& data, DATA_TYPE item_old, DATA_TYPE item_new) {
      int sz = data.size();
      for (int i = 0; i < sz; i++) {
        if (data[i] == item_old) {
          data[i] = item_new;
    ...
    public:
      ...
      std::vector<VERTEX<VERTEX_DATA> > V;
      int _V_;  // _V_ - is a size of V array;
      std::vector<EDGE<EDGE_DATA> > E;
      int _E_;  // _E_ - is a size of E array;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    GRAPH<VERTEX_DATA, EDGE_DATA>::GRAPH() {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    GRAPH<VERTEX_DATA, EDGE_DATA>::GRAPH(int v) {
      is_allow_parallel_edges = 0;
      _V_ = v;
      _E_ = 0;
      for (int i = 0; i < _V_; i++) {
        VERTEX<VERTEX_DATA> vx;
        vx.vertex_index = i;
        V.push_back(vx);
      }
    }
    ...
    template <class VERTEX_DATA>
    VERTEX<VERTEX_DATA>::VERTEX(const VERTEX<VERTEX_DATA>& v) {
    ...
    template <class EDGE_DATA>
    EDGE<EDGE_DATA>::EDGE(const EDGE<EDGE_DATA>& e) {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    GRAPH<VERTEX_DATA, EDGE_DATA>::GRAPH(const GRAPH<VERTEX_DATA, EDGE_DATA>& g) {
      int i;
    ...
    template <class VERTEX_DATA>
    int VERTEX<VERTEX_DATA>::show_info(int lindr) {
      ...
      if (is_vertex_data) {
        if (lindr == -1) {
          std::cout << " &vertex_data = " << &vertex_data << std::endl;
        } else if (lindr == 0) {
          std::cout << "  vertex_data = " << vertex_data << std::endl;
        } else if (lindr == 1) {
          std::cout << " *vertex_data = " << *vertex_data << std::endl;
    ...
    template <class EDGE_DATA>
    int EDGE<EDGE_DATA>::show_info(int lindr) {
      // lindr - level of indirection
      // -1  = &x
      //  0  =  x
      //  1  = *x
      std::cout << "edge( " << edge_index << " ) info:" << std::endl;
      std::cout << " ( edge_vertex1 , edge_vertex2 ) = ( " << edge_vertex1 << " , " << edge_vertex2
                << " )" << std::endl;
      std::cout << " ( edge_vertex_id1 , edge_vertex_id2 ) = ( " << edge_vertex_id1 << " , "
                << edge_vertex_id2 << " )" << std::endl;
      std::cout << " edge_direction = " << edge_direction << std::endl;
      std::cout << " edge_weight = " << edge_weight << std::endl;
      if (is_edge_data) {
        if (lindr == -1) {
          std::cout << " &edge_data = " << &edge_data << std::endl;
        } else if (lindr == 0) {
          std::cout << "  edge_data = " << edge_data << std::endl;
        } else if (lindr == 1) {
          std::cout << " *edge_data = " << *edge_data << std::endl;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::is_vertex_id_reserved(int id) {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::vertex_id_to_index(int id) {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::is_adjacent_vertices(int v1, int v2) {
      int res = 0;  // Not adjacent to each other
      int i, st1, st2;
      st1 = st2 = 0;

      for (i = 0; i < V[v1].vertex_degree; i++) {
        if (V[v1].adjacent_vertices[i] == v2) {
          st1 = 1;
        }
      }

      for (i = 0; i < V[v2].vertex_degree; i++) {
        if (V[v2].adjacent_vertices[i] == v1) {
          st2 = 1;
        }
      }
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::ADD_VERTEX() {
      VERTEX<VERTEX_DATA> vx;
      vx.vertex_index = _V_;
      int id = _V_;
      while (is_vertex_id_reserved(id)) {
        id++;
      }
      vx.vertex_id = id;
      V.push_back(vx);
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::ADD_VERTEX(int id) {
      VERTEX<VERTEX_DATA> vx;
      vx.vertex_index = _V_;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::ADD_VERTEX(VERTEX<VERTEX_DATA> vx) {
      vx.vertex_index = _V_;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::show_vertices(int lindr) {
      for (int i = 0; i < _V_; i++) {
        V[i].show_info(lindr);
      }
      return _V_;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::show_edges(int lindr) {
      for (int i = 0; i < _E_; i++) {
        E[i].show_info(lindr);
      }
      return _E_;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::is_edge_exist(EDGE<EDGE_DATA> e) {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::ADD_EDGE(EDGE<EDGE_DATA> eg) {
      int v1, v2;
      ...
      if (v1 == -1) {
        ADD_VERTEX(e.edge_vertex_id1);
        v1 = _V_ - 1;
      }
      if (v2 == -1) {
        ADD_VERTEX(e.edge_vertex_id2);
        v2 = _V_ - 1;
      }
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::DELETE_EDGE(int e) {
      int v1, v2;
      if (e >= _E_) {
        std::cout << "Error: Can not delete edge with index = " << e
                  << " No such edge in a graph\n";
      } else {
        // Modify vertices
        v1 = E[e].edge_vertex1;
        v2 = E[e].edge_vertex2;
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::DELETE_VERTEX(int v) {
      if (v < _V_) {
        ...
        if (v == (_V_ - 1)) {
          ...
          for (int i = 0; i < V[v].vertex_degree; i++) {
            ...
            if (v1 == (_V_ - 1)) {
              E[e].edge_vertex1 = v;
              replace_item<int>(V[v2].adjacent_vertices, (_V_ - 1), v);
            }
            if (v2 == (_V_ - 1)) {
              E[e].edge_vertex2 = v;
              replace_item<int>(V[v1].adjacent_vertices, (_V_ - 1), v);
            }
          }
        }
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::REDUCE_GRAPH() {
      for (int i = 0; i < _V_; i++) {
        if (V[i].vertex_degree <= 1) {
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::FIND_PATHS_FLOYD_WARSHALL(double** D, vector<Path>** P) {
...
*/
    ...
    template <class VERTEX_DATA, class EDGE_DATA>
    int GRAPH<VERTEX_DATA, EDGE_DATA>::FIND_SSSR(vector<Path>& sssr) {
      ...
      for (int i = 0; i < _V_; i++) {
        for (int j = 0; j < _V_; j++) {
          if ((D[i][j] == 0.0) || ((P[i][j].size() == 1) && (Pp[i][j].size() == 0))) {
            ...
            if (Pp[i][j].size() != 0) {
              Cnum[i][j] = 2 * int(D[i][j]) + 1;
            } else {
              Cnum[i][j] = 2 * int(D[i][j]);
            }
          }

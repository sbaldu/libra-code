...
namespace liblibra {
  ...
  namespace librandom {
    ...
    void Random::bin(vector<double>& in,
                     double minx,
                     double maxx,
                     double dx,
    ...
    double Random::p_exponential(double x, double lambda) { return lambda * exp(-lambda * x); }
    ...
    double Random::p_normal(double x) { return sqrt(0.5 / M_PI) * exp(-0.5 * x * x); }
    ...
    id1:
      ...
      if (u <= 0.009572265238289) {
        ...
        x = b * (1.0 + s / d);
        u = uniform(0.0, 1.0);
        if (log(u) > mu * (2.0 + log(x / mu) - x / b) + 3.7203284924588 - b - log(sigma * d / b)) {
          goto id1;
        } else {
          return x;
        }

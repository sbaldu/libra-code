...
*/
      ...
      if (at_type.size() != R.size()) {
        cout << "Error: size of the atom names array is not equal to the size of the coordinates "
                "array\n";
        exit(0);
      }
      ...
      if (atom_to_ao_map.size() > 0) {
        atom_to_ao_map.clear();
      }
      if (ao_to_atom_map.size() > 0) {
      ...
      for (i = 0; i < Nat; i++) {
        ...
        for (iter = modpar.PT[at_type[i]].IP.begin(); iter != modpar.PT[at_type[i]].IP.end();
             iter++) {  // Iterate over all shells of the given atom
        ...
        if (verb) {
          cout << "Atom " << i << " (" << at_type[i] << ") contributes " << basis_ao.size() - Norb
               << " orbitals\n";
          cout << "Atom " << i << " (" << at_type[i] << ") contributes "
               << modpar.PT[at_type[i]].Nval << " electrons\n";
        }

        // Set atom to AOs mapping
        vector<int> orbs_i;  // orbitals of atom i
        for (int o = Norb; o < basis_ao.size(); o++) {
          orbs_i.push_back(o);
        }
    ...
    boost::python::list set_basis_STO_3G_DZ(vector<std::string>& at_type,
      ...
      set_basis_STO_3G_DZ(
          at_type, R, modpar, verb, basis_ao, Nelec, Norb, atom_to_ao_map, ao_to_atom_map);

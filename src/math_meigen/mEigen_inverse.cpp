...
*/
      ...
      if (S.n_cols != S_inv.n_cols) {
        cout << "Error in libmeigen::inv_matrix : size of matrix S is not the same as that of "
                "matrix S^{-1}\n";
        exit(0);
      }
      ...
      *C = complex<double>(0.0, 0.0);
      CMATRIX* Seig;
      Seig = new CMATRIX(sz, sz);
      *Seig = complex<double>(0.0, 0.0);
      CMATRIX* tmp;
      tmp = new CMATRIX(sz, sz);
...
*/
    ...
    void inv_matrix(MATRIX& S, MATRIX& S_inv) { inv_matrix(S, S_inv, -1.0, 1); }
...
*/
    ...
    void inv_matrix(CMATRIX& S, CMATRIX& S_inv, double thresh) { inv_matrix(S, S_inv, thresh, 1); }

    void inv_matrix(CMATRIX& S, CMATRIX& S_inv) { inv_matrix(S, S_inv, -1.0, 1); }

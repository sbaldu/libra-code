...
*/
      ...
      if (S.n_cols != S_half.n_cols) {
        cout << "Error in libmeigen::sqrt_matrix : size of matrix S is not the same as that of "
                "matrix S^{1/2}\n";
        exit(0);
      }
...
*/
      ...
      *Seig = complex<double>(0.0, 0.0);
      ...
      for (i = 0; i < sz; i++) {
        res.M[i * sz + i] = std::exp(dt * Seig->get(i, i));
      }
    ...
    void exp_matrix(CMATRIX& res, CMATRIX& S, complex<double> dt) { exp_matrix(res, S, dt, 0); }

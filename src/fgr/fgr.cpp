...
namespace liblibra {
  ...
  namespace libfgr {
    ...
    complex<double> Integrand_NE_exact(double tp,
                                       double tau,
                                       double omega_DA,
                                       double omega,
                                       double req,
...
*/
    ...
    complex<double> Linear_NE_exact(
        double tp, double tau, double gamma, double omega, double req, double shift, double beta) {
      /**
*/
      double re, im;
      double wtp = omega * tp;
...
*/
    ...
    complex<double> ACF_NE_exact(double tp,
                                 double tau,
                                 double omega_DA,
                                 double V,
                                 vector<double>& omega_nm,
                                 vector<double>& gamma_nm,
                                 vector<double>& req_nm,
                                 vector<double>& shift_NE,
                                 double beta,
                                 int type) {
      /**
    ...
    */
      ...
      if (type == 0) {
        ampl = complex<double>(V * V, 0.0);
      }

      // C = exp(argg) * ampl = exp[Re(argg)+i*Im(argg)] * [Re(ampl) + i*Im(ampl)] =
      // = exp(Re(argg)) * [cos(Im(argg)) + i*sin(Im(argg))] * [Re(ampl) + i*Im(ampl)]
      double C_re = exp(argg.real()) * (cos(argg.imag()) * ampl.real() -
                                        sin(argg.imag()) * ampl.imag());  // this is Re(C)
      double C_im = exp(argg.real()) * (cos(argg.imag()) * ampl.imag() +
                                        sin(argg.imag()) * ampl.real());  // this is Im(C)

      return complex<double>(C_re, C_im);
    }

    complex<double> Integrand_NE_LSC(double tp,
                                     double tau,
                                     double omega_DA,
                                     double omega,
                                     double req,
...
*/
      ...
      re -= req * req * 0.5 * Coth * Coth * pow(sin(0.5 * wt), 2) *
            (cos(4.0 * wtp - 2.0 * wt) + cos(wt));

      im = (1.0 - 2.0 * cos(wt)) * sin(wtp) + sin(wtp - 2.0 * wt) -
           4.0 * cos(3.0 * wtp - 1.5 * wt) * sin(0.5 * wt);
      im *= 0.25 * req * shift * Coth;

      return gamma * gamma * complex<double>(re, im);
    }

    complex<double> ACF_NE_LSC(double tp,
                               double tau,
                               double omega_DA,
                               double V,
                               vector<double>& omega_nm,
    ...
    */
    ...
    complex<double> Integrand_NE_CAV(double tp,
                                     double tau,
                                     double omega_DA,
                                     double omega,
                                     double req,
...
*/
      ...
      re = shift * shift * cos(wtp) * cos(wtp - wt) + 1.0 / (wb * omega) * cos(wt) -
           req * req * 0.5 / pow(wb * 0.5, 2) * pow(sin(0.5 * wt), 2) *
               (cos(4.0 * wtp - 2.0 * wt) + cos(wt));
      im = 0.5 * req * shift / wb *
           ((1.0 - 2.0 * cos(wt)) * sin(wtp) + sin(wtp - 2.0 * wt) -
            4.0 * cos(3.0 * wtp - 1.5 * wt) * sin(0.5 * wt));

      return gamma * gamma * complex<double>(re, im);
    }

    complex<double> ACF_NE_CAV(double tp,
                               double tau,
                               double omega_DA,
                               double V,
                               vector<double>& omega_nm,
    ...
    */
    ...
    complex<double> Integrand_NE_CD(double tp,
                                    double tau,
                                    double omega_DA,
                                    double omega,
                                    double req,
...
*/
      ...
      re = shift * shift * cos(wtp) * cos(wtp - wt) + cos(wt) / (wb * omega) -
           pow((req * sin(wt) / wb), 2);
    ...
    complex<double> ACF_NE_CD(double tp,
                              double tau,
                              double omega_DA,
                              double V,
                              vector<double>& omega_nm,
    ...
    */
    ...
    complex<double> Integrand_NE_W0(double tp,
                                    double tau,
                                    double omega_DA,
                                    double omega,
                                    double req,
...
*/
      ...
      re = Coth * 0.5 / omega + 0.5 * shift * shift * (1.0 + cos(2.0 * wtp)) -
           0.25 * argg * argg * Coth * Coth;
      im = -Coth * argg * shift * cos(wtp);

      return gamma * gamma * complex<double>(re, im);
    }

    complex<double> ACF_NE_W0(double tp,
                              double tau,
                              double omega_DA,
                              double V,
                              vector<double>& omega_nm,
    ...
    */
    ...
    complex<double> Integrand_NE_C0(double tp,
                                    double tau,
                                    double omega_DA,
                                    double omega,
                                    double req,
...
*/
      ...
      re =
          1.0 / (beta * omega * omega) + 0.5 * shift * shift * (1.0 + cos(2.0 * wtp)) - argg * argg;
      im = -2.0 * argg * shift * cos(wtp);

      return gamma * gamma * complex<double>(re, im);
    }

    complex<double> ACF_NE_C0(double tp,
                              double tau,
                              double omega_DA,
                              double V,
                              vector<double>& omega_nm,
...
*/
      ...
      for (int n = 0; n < N; n++) {  //tau index
        ...
        if (tau <= tp) {
          switch (method) {
            case 0: {
              C = ACF_NE_exact(
                  tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);
            } break;
            case 1: {
              C = ACF_NE_LSC(
                  tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);
            ...
            case 3: {
              C = ACF_NE_CD(tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);
            } break;
            case 4: {
              C = ACF_NE_W0(tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);
            } break;
            case 5: {
              C = ACF_NE_C0(tp, tau, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, beta, type);
            } break;
            default: {
              cout << "Method " << method
                   << " is not available. Please choose from the following options:\n";
              cout << "0 - Exact\n";
              cout << "1 - LSC\n";
              cout << "2 - CAV\n";
              cout << "3 - CD\n";
    ...
    MATRIX NEFGRL_population(double omega_DA,
                             double V,
                             vector<double>& omega_nm,
                             vector<double>& gamma_nm,
                             vector<double>& req_nm,
...
*/
      ...
      for (int step = 0; step < nsteps; step++) {  // t'
        ...
        // k = k(t')
        double k = NEFGRL_rate(
            step * dt, omega_DA, V, omega_nm, gamma_nm, req_nm, shift_NE, method, beta, type, dtau);

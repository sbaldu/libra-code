...
*/
      ...
      if (req_nm.size() != sz) {
        cout << "ERROR in reorganization_energy: the dimensions of the input arguments do not "
                "match\n";
...
*/
      ...
      tmp.set(0, 0, 1.0);
      for (i = 1; i < ndof; i++) {
        tmp.set(i, 0, -coeff[i] / (omega[i] * omega[i]));
      }
      tmp = 2.0 * y0 * T.T() * tmp;
...
*/
      ...
      vector<double> req(ndof, 0.0);
      for (int i = 0; i < ndof; i++) {
        req[i] = (d1[i] - d2[i]) / (omega_nm[i] * omega_nm[i]);
      }

...
****************************************************************************/
    ...
    double gaussian_overlap(int nxa,
                            double alp_a,
                            double Xa,
                            int nxb,
                            double alp_b,
...
****************************************************************************/
      ...
      dfdXpb = aux[2];
      double* g;
      g = aux[3];  // will containg gaussian integral

      // Compute binomial expansion and derivatives, if necessary
      binomial_expansion(nxa,
                         nxb,
                         Xpa,
                         Xpb,
                         f,
                         dfdXpa,
                         dfdXpb,
                         is_derivs);  // (x+x1)^n1 * (x+x2)^n2 = summ_i { x^i * f_i (x1,x2,n1,n2) }
    ...
    boost::python::list gaussian_overlap(int nxa,
                                         double alp_a,
                                         double Xa,
                                         int nxb,
                                         double alp_b,
      ...
      double I =
          gaussian_overlap(nxa, alp_a, Xa, nxb, alp_b, Xb, is_normalize, is_derivs, dI_dXa, dI_dXb);
    ...
    double gaussian_overlap(
      ...
      double dI_dxa, dI_dxb;
      double res =
          gaussian_overlap(nxa, alp_a, Xa, nxb, alp_b, Xb, is_normalize, 0, dI_dxa, dI_dxb);
      return res;
    ...
    double gaussian_overlap(int nxa,
      ...
      // Do computations
      double res = gaussian_overlap(nxa,
                                    nya,
                                    nza,
                                    alp_a,
                                    Ra,
...
*/
      ...
      double res = sqrt(2.0 * alp / FACTORIAL(2 * n)) *
                   FAST_POW(2.0 * alp, n);  // this is normalization factor

      return res;
    }

    double sto_overlap(int na,
                       int la,
                       int ma,
                       double alp_a,
                       int nb,
      ...
      if (ma == mb) {
        ...
        if (R < R_cutoff) {
          ...
          double* f;
          f = new double[na + nb + 1];
          double* g;
          g = new double[na + nb + 1];
    ...
    double sto_overlap_fast(int na,
                            int la,
                            int ma,
                            double alp_a,
                            int nb,

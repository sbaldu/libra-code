...
namespace liblibra {
  ...
  namespace libmolint {
    ...
    double K2aa(int m, int n, double alpha, double betha, double R) {
      ...
      if (R != 0.0) {
        K1 = (1.0 / R);
        sum = (2.0 * alpha * R - 2.0 * m) *
              (A_plus_1((2 * m - 1), (2.0 * alpha * R)) - exp(-2.0 * alpha * R));
        K1 += sum * (t * r / (f * R));
        ...
        for (float v = 0; v <= (2 * n - 1); v++) {
          ...
          for (int p = 0; (2.0 * p <= (2.0 * m + v - 1)); p++) {
            sum += (1.0 / (2.0 * p + 1.0)) * D((2 * m - 1), v, 2 * p) *
                   A_plus_1((2 * m - 1 + v - 2 * p), 2.0 * R * alpha);
          }
    ...
    double K2ab(int m, int n, double alpha, double betha, double R) {
      ...
      for (float v = 0; v <= 2 * n; v++) {
        ...
        for (int p = 0; p <= (2 * m - 1 + v); p++) {
          sum += D((2 * m - 1), v, p) * B(p, R * (alpha - betha)) *
                 A_plus_1((2 * m - 1 + v - p), R * (alpha + betha));
        }
    ...
    double dJab_dqa(int m, int n, double alpha, double betha, double R) {
      ...
      for (float i = 0; i <= 2 * n; i++) {
        sum = 0.0;
        for (int j = 0; j <= (1 + i); j++) {
          sum1 = (-i * alpha * alpha + (i - 2.0 * j + 1.0) * alpha * betha - 3.0 * betha * betha) /
                 (alpha * alpha - betha * betha);
          sum1 = sum1 * B(j, R * (alpha - betha)) * A_plus_1(1 + i - j, R * (alpha + betha));

          sum1 += (alpha / (alpha - betha)) * A_plus_1(1 + i - j, R * (alpha + betha)) *
                  exp(-R * (alpha - betha));
    ...
    double Coulomb_Integral(double R,
                            int n_i,
                            double Jii,
                            double ksi_i,
                            std::string type_i,
...
*/
      ...
      switch (mode) {
        case 0: {  //--------- Calculate (charge-dependent idempotential)----------------
          ...
          if ((type_i == "H") && (type_j == "H")) {
            Jij =
                ...
                (1.0 +
                 q_i /
                     ksi_i);  // Make only H idempotentials charge dependent,[ what is q_i != q_j ?? ]
          }
        } break;
        ...
        default: {
          std::cout << "Warning: To calculate two-center Coulomb integral there are options:"
                    << std::endl;

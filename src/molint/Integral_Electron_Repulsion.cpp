  ...
  namespace libmolint {
    ...
    void eri_aux1(int l1,
      ...
      for (i = 0; i <= maxL; i++) {
        for (r = 0; r <= (i / 2); r++) {  //!!! Integer division
          L = i - 2 * r;
          if (L >= 0) {
            double prefact = ((FACTORIAL(i) / (1.0 * FACTORIAL(r) * FACTORIAL(L)))) *
                             FAST_POW((0.25 / gamma), (i - r));
            H[L] += prefact * f[i];
      ...
      for (L = 0; L <= maxL; L++) {
        for (M = 0; M <= maxM; M++) {
          ...
          for (u = 0; u <= maxU; u++) {
            ...
            if (I >= 0) {
              double prefac =
                  FAST_POW(-1.0, (M + u)) * FACTORIAL(L + M) /
                  (1.0 * FACTORIAL(u) * FACTORIAL(L + M - 2 * u) * FAST_POW(d, (L + M - u)));
              ...
              if ((L + M - 2 * u) > 0) {  // to avoid singularity when p = 0
                dGdp[I] +=
                    (L + M - 2 * u) * prefac * HL[L] * HM[M] * FAST_POW(p, (L + M - 2 * u - 1));
              }
            }
          }  // for u
        }  // for M
    ...
    double electron_repulsion_integral(int nxa,
        ...
        for (int J = 0; J <= maxJ; J++) {
          for (int K = 0; K <= maxK; K++) {
            ...
            if (is_derivs) {
              ...
              dCdA_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dA + dGI_dPB[I] * dPB_dA + dGI_dQC[I] * dQC_dA +
                   dGI_dQD[I] * dQD_dA + dGI_dPQ[I] * dPQ_dA) *
                  GJ[J] * GK[K];
              dCdA_nu[I + J + K].y +=
                  GI[I] *
                  (dGJ_dPA[J] * dPA_dA + dGJ_dPB[J] * dPB_dA + dGJ_dQC[J] * dQC_dA +
                   dGJ_dQD[J] * dQD_dA + dGJ_dPQ[J] * dPQ_dA) *
                  GK[K];
              dCdA_nu[I + J + K].z +=
                  GI[I] * GJ[J] *
                  (dGK_dPA[K] * dPA_dA + dGK_dPB[K] * dPB_dA + dGK_dQC[K] * dQC_dA +
                   dGK_dQD[K] * dQD_dA + dGK_dPQ[K] * dPQ_dA);

              // Derivatives with respect to B coordinates
              dCdB_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dB + dGI_dPB[I] * dPB_dB + dGI_dQC[I] * dQC_dB +
                   dGI_dQD[I] * dQD_dB + dGI_dPQ[I] * dPQ_dB) *
                  GJ[J] * GK[K];
              dCdB_nu[I + J + K].y +=
                  GI[I] *
                  (dGJ_dPA[J] * dPA_dB + dGJ_dPB[J] * dPB_dB + dGJ_dQC[J] * dQC_dB +
                   dGJ_dQD[J] * dQD_dB + dGJ_dPQ[J] * dPQ_dB) *
                  GK[K];
              dCdB_nu[I + J + K].z +=
                  GI[I] * GJ[J] *
                  (dGK_dPA[K] * dPA_dB + dGK_dPB[K] * dPB_dB + dGK_dQC[K] * dQC_dB +
                   dGK_dQD[K] * dQD_dB + dGK_dPQ[K] * dPQ_dB);

              // Derivatives with respect to C coordinates
              dCdC_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dC + dGI_dPB[I] * dPB_dC + dGI_dQC[I] * dQC_dC +
                   dGI_dQD[I] * dQD_dC + dGI_dPQ[I] * dPQ_dC) *
                  GJ[J] * GK[K];
              dCdC_nu[I + J + K].y +=
                  GI[I] *
                  (dGJ_dPA[J] * dPA_dC + dGJ_dPB[J] * dPB_dC + dGJ_dQC[J] * dQC_dC +
                   dGJ_dQD[J] * dQD_dC + dGJ_dPQ[J] * dPQ_dC) *
                  GK[K];
              dCdC_nu[I + J + K].z +=
                  GI[I] * GJ[J] *
                  (dGK_dPA[K] * dPA_dC + dGK_dPB[K] * dPB_dC + dGK_dQC[K] * dQC_dC +
                   dGK_dQD[K] * dQD_dC + dGK_dPQ[K] * dPQ_dC);

              // Derivatives with respect to D coordinates
              dCdD_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dD + dGI_dPB[I] * dPB_dD + dGI_dQC[I] * dQC_dD +
                   dGI_dQD[I] * dQD_dD + dGI_dPQ[I] * dPQ_dD) *
                  GJ[J] * GK[K];
              dCdD_nu[I + J + K].y +=
                  GI[I] *
                  (dGJ_dPA[J] * dPA_dD + dGJ_dPB[J] * dPB_dD + dGJ_dQC[J] * dQC_dD +
                   dGJ_dQD[J] * dQD_dD + dGJ_dPQ[J] * dPQ_dD) *
                  GK[K];
              dCdD_nu[I + J + K].z +=
                  GI[I] * GJ[J] *
                  (dGK_dPA[K] * dPA_dD + dGK_dPB[K] * dPB_dD + dGK_dQC[K] * dQC_dD +
                   dGK_dQD[K] * dQD_dD + dGK_dPQ[K] * dPQ_dD);
      ...
      for (int nu = 0; nu <= (maxI + maxJ + maxK); nu++) {
        ...
        if (is_derivs) {
          dERI_dA +=
              (dCdA_nu[nu] * F_nu[nu] + C_nu[nu] * (-(2.0 / d4) * F_nu[nu + 1] * PQ * dPQ_dA));
          dERI_dB +=
              (dCdB_nu[nu] * F_nu[nu] + C_nu[nu] * (-(2.0 / d4) * F_nu[nu + 1] * PQ * dPQ_dB));
          dERI_dC +=
              (dCdC_nu[nu] * F_nu[nu] + C_nu[nu] * (-(2.0 / d4) * F_nu[nu + 1] * PQ * dPQ_dC));
          dERI_dD +=
              (dCdD_nu[nu] * F_nu[nu] + C_nu[nu] * (-(2.0 / d4) * F_nu[nu + 1] * PQ * dPQ_dD));
        }
      ...
      if (is_normalize) {
        ...
        N *=
            (gaussian_normalization_factor(nxa, alp_c) * gaussian_normalization_factor(nxb, alp_d));
        N *=
            (gaussian_normalization_factor(nya, alp_c) * gaussian_normalization_factor(nyb, alp_d));
        N *=
            (gaussian_normalization_factor(nza, alp_c) * gaussian_normalization_factor(nzb, alp_d));
      }
    ...
    double electron_repulsion_integral(int nxa,
      ...
      // Do computations
      double res = electron_repulsion_integral(nxa,
                                               nya,
                                               nza,
                                               alp_a,
                                               Ra,
    ...
    boost::python::list electron_repulsion_integral(int nxa,
      ...
      double I = electron_repulsion_integral(nxa,
                                             nya,
                                             nza,
                                             alp_a,
                                             Ra,

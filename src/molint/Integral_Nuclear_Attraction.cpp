...
namespace liblibra {
  ...
  namespace libmolint {
    ...
    void Aux_Function4(int n1,
      ...
      int n = n1 + n2;
      double p =
          PC;  // But keep in mind: that Taketa uses p = CP = -PC, so define PC outside this function accordingly
      ...
      if (n <= 4) {  // Explicit formula
        if (n == 0) {
          ...
          G[0] = f[0] + scl * f[2];
          dGdA[0] = dfda[0] + scl * dfda[2];
          dGdB[0] = dfdb[0] + scl * dfdb[2];
          dGdC[0] = 0.0;
          G[1] = -f[1] * p - scl * f[2];
          dGdA[1] = -dfda[1] * p - scl * dfda[2];
          dGdB[1] = -dfdb[1] * p - scl * dfdb[2];
          dGdC[1] = -f[1];
          G[2] = p * p;
          dGdA[2] = 0.0;
          dGdB[2] = 0.0;
          ...
          G[0] = f[0] + scl * f[2] + scl2;
          dGdA[0] = dfda[0] + scl * dfda[2];
          dGdB[0] = dfdb[0] + scl * dfdb[2];
          dGdC[0] = 0.0;
          G[1] = -f[1] * p - scl * f[2] - scl1 * f[3] - 2.0 * scl2;
          dGdA[1] = -dfda[1] * p - scl * dfda[2] - scl1 * dfda[3];
          dGdB[1] = -dfdb[1] * p - scl * dfdb[2] - scl1 * dfdb[3];
          dGdC[1] = -f[1] - f[3] * 3.0 * scl;
          G[2] = f[2] * p2 + scl1 * f[3] + scl2 + 6.0 * scl * p2;
          dGdA[2] = dfda[2] * p2 + scl1 * dfda[3];
          dGdB[2] = dfdb[2] * p2 + scl1 * dfdb[3];
          dGdC[2] = 2.0 * p * f[2] + 3.0 * scl * f[3] + 12.0 * scl * p;
          G[3] = -f[3] * p3 - 6.0 * scl * p2;
          dGdA[3] = -dfda[3] * p3;
          dGdB[3] = -dfdb[3] * p3;
          dGdC[3] = -3.0 * f[3] * p2 - 12.0 * scl * p;
          G[4] = p2 * p2;
          dGdA[4] = 0.0;
          dGdB[4] = 0.0;
          dGdC[4] = 4.0 * p3;
        ...
        for (int i = 0; i <= n; i++) {
          for (int r = 0; r <= (i / 2); r++) {  // use integer division! so the upper limit is [i/2]
            ...
            for (int u = 0; u <= ((i - 2 * r) / 2);
                 u++) {  // use integer division! so the upper limit is [(i-2r)/2]

              int I = (i - 2 * r - u);
              double den;
              den = FACTORIAL(r) * FACTORIAL(u) * FACTORIAL(i - 2 * r - 2 * u);
              double pref =
                  pow(-1.0, (i + u)) * (FACTORIAL(i) / den) * FAST_POW((0.25 / gamma), (r + u));
              double pw;
              ...
              if ((i - 2 * r - 2 * u) == 0) {
                ...
                pw = (i - 2 * r - 2 * u) * FAST_POW(p, (i - 2 * r - 2 * u - 1));
              }
              dGdC[I] += pref * pw * f[i];
...
***********************************************************************************/
  ...
  ***********************************/
        ...
        for (int J = 0; J <= (nya + nyb); J++) {
          for (int K = 0; K <= (nza + nzb); K++) {
            ...
            if (is_derivs) {
              ...
              dCdA_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dA + dGI_dPB[I] * dPB_dA + dGI_dPC[I] * dPC_dA) * GJ[J] * GK[K];
              dCdA_nu[I + J + K].y +=
                  (dGJ_dPA[J] * dPA_dA + dGJ_dPB[J] * dPB_dA + dGJ_dPC[J] * dPC_dA) * GI[I] * GK[K];
              dCdA_nu[I + J + K].z +=
                  (dGK_dPA[K] * dPA_dA + dGK_dPB[K] * dPB_dA + dGK_dPC[K] * dPC_dA) * GI[I] * GJ[J];

              // Derivatives with respect to B coordinates
              dCdB_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dB + dGI_dPB[I] * dPB_dB + dGI_dPC[I] * dPC_dB) * GJ[J] * GK[K];
              dCdB_nu[I + J + K].y +=
                  (dGJ_dPA[J] * dPA_dB + dGJ_dPB[J] * dPB_dB + dGJ_dPC[J] * dPC_dB) * GI[I] * GK[K];
              dCdB_nu[I + J + K].z +=
                  (dGK_dPA[K] * dPA_dB + dGK_dPB[K] * dPB_dB + dGK_dPC[K] * dPC_dB) * GI[I] * GJ[J];

              // Derivatives with respect to C coordinates
              dCdC_nu[I + J + K].x +=
                  (dGI_dPA[I] * dPA_dC + dGI_dPB[I] * dPB_dC + dGI_dPC[I] * dPC_dC) * GJ[J] * GK[K];
              dCdC_nu[I + J + K].y +=
                  (dGJ_dPA[J] * dPA_dC + dGJ_dPB[J] * dPB_dC + dGJ_dPC[J] * dPC_dC) * GI[I] * GK[K];
              dCdC_nu[I + J + K].z +=
                  (dGK_dPA[K] * dPA_dC + dGK_dPB[K] * dPB_dC + dGK_dPC[K] * dPC_dC) * GI[I] * GJ[J];
      ...
      for (int nu = 0; nu <= (max_exp + 1); nu++) {
        F_nu[nu] = Fn(nu, gamma * PC.length2());
      }
      ...
      for (int nu = 0; nu <= max_exp; nu++) {
        ...
        if (is_derivs) {
          dNAI_dA +=
              dCdA_nu[nu] * F_nu[nu] + C_nu[nu] * (-2.0 * gamma * F_nu[nu + 1] * PC * dPC_dA);
          dNAI_dB +=
              dCdB_nu[nu] * F_nu[nu] + C_nu[nu] * (-2.0 * gamma * F_nu[nu + 1] * PC * dPC_dB);
          dNAI_dC +=
              dCdC_nu[nu] * F_nu[nu] + C_nu[nu] * (-2.0 * gamma * F_nu[nu + 1] * PC * dPC_dC);
      ...
      NAI = pref *
            NAI;  // this must be done after derivatives, because we use NAI as temporary variable

      double N = 1.0;
      if (is_normalize) {
        N = (gaussian_normalization_factor(nxa, alp_a) * gaussian_normalization_factor(nxb, alp_b));
        N *=
            (gaussian_normalization_factor(nya, alp_a) * gaussian_normalization_factor(nyb, alp_b));
        N *=
            (gaussian_normalization_factor(nza, alp_a) * gaussian_normalization_factor(nzb, alp_b));
      }
    ...
    double nuclear_attraction_integral(int nxa,
      ...
      // Do computations
      double res = nuclear_attraction_integral(nxa,
                                               nya,
                                               nza,
                                               alp_a,
                                               Ra,
    ...
    boost::python::list nuclear_attraction_integral(int nxa,
      ...
      double I = nuclear_attraction_integral(nxa,
                                             nya,
                                             nza,
                                             alp_a,
                                             Ra,
    ...
    double nuclear_attraction_integral(int nxa,
                                       ...
                                       int is_normalize) {
      VECTOR dIdA, dIdB, dIdC;
      double res = nuclear_attraction_integral(
          nxa, nya, nza, alp_a, Ra, nxb, nyb, nzb, alp_b, Rb, Rc, is_normalize, 0, dIdA, dIdB, dIdC);
      return res;
    }
    ...
    double nuclear_attraction_integral(int nxa,
                                       ...
                                       VECTOR& Rc) {
      double res =
          nuclear_attraction_integral(nxa, nya, nza, alp_a, Ra, nxb, nyb, nzb, alp_b, Rb, Rc, 1);
      return res;
    }

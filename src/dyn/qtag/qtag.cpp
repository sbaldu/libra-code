...
*/
        ...
        for (int dof = 0; dof < ndof; dof++) {
          ...
          complex<double> Q2_i(
              1.0 / denom + X_ij * X_ij - P_ij * P_ij - 2.0 * X_ij * q1i + q1i * q1i,
              2.0 * P_ij * (X_ij - q1i));
          complex<double> Q2_j(
              1.0 / denom + X_ij * X_ij - P_ij * P_ij - 2.0 * X_ij * q1j + q1j * q1j,
              2.0 * P_ij * (X_ij - q1j));
...
*/
        ...
        if (n1 == n2) {  // LHA for single-surface elements
          ...
          v = LHA(ham.children[i]->ham_dia,
                  ham.children[j]->ham_dia,
                  ham.children[i]->d1ham_dia,
                  ham.children[j]->d1ham_dia,
                  ham.children[i]->d2ham_dia,
                  ham.children[j]->d2ham_dia,
                  q1,
                  p1,
                  s1,
                  alp1,
          ...
          for (int dof = 0; dof < ndof; dof++) {
            ...
            complex<double> expt(aCq1 * aCq1 + aCq2 * aCq2 - dp * dp + 2.0 * aCq1 * aCq2,
                                 2.0 * dp * (aCq1 + aCq2));

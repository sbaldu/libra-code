...
*/
        ...
        if (thermostat_type == "Nose-Hoover") {
          ...
          comp = (0.5 * Ps * Ps / Q) +
                 (Nf_t + Nf_r) * (boltzmann / hartree) * Temperature * log(s_var);
        }
...
*/
        if (thermostat_type == "Nose-Hoover") {
          ...
          for (int i = 0; i < NHC_size; i++) {
            if (i == 0) {
              if (Nf_t > 0) {
                G_t[0] = (2.0 * ekin_tr - Nf_t * kT) / Q_t[i];
              }  //else{ G_t[0] = 0.0; }
              if (Nf_r > 0) {
                G_r[0] = (2.0 * ekin_rot - Nf_r * kT) / Q_r[i];
              }  //else{ G_r[0] = 0.0; }
              if (Nf_b > 0) {
                G_b[0] = (2.0 * ekin_baro - Nf_b * kT) / Q_b[0];
              }  //else{ G_b[0] = 0.0; }
            } else {
              if (Nf_t > 0) {
                G_t[i] = (Q_t[i - 1] * ksi_t[i - 1] * ksi_t[i - 1] - kT) / Q_t[i];
              }  //else{ G_t[i] = 0.0; }
              if (Nf_r > 0) {
                G_r[i] = (Q_r[i - 1] * ksi_r[i - 1] * ksi_r[i - 1] - kT) / Q_r[i];
              }  //else{ G_r[i] = 0.0; }
              if (Nf_b > 0) {
                G_b[i] = (Q_b[i - 1] * ksi_b[i - 1] * ksi_b[i - 1] - kT) / Q_b[i];
              }  //else{ G_b[i] = 0.0; }
            }
          }  // for i
        }  // if Nose-Hoover
      ...
      void Thermostat::update_thermostat_forces(double ekin_tr,
                                                double ekin_rot,
                                                double ekin_baro,
                                                int i) {
        /**
...
*/
        ...
        if (thermostat_type == "Nose-Hoover") {
          ...
          if (i == 0) {
            if (Nf_t > 0) {
              G_t[i] = (2.0 * ekin_tr - Nf_t * kT) / Q_t[i];
            }  //else{ G_t[0] = 0.0; }
            if (Nf_r > 0) {
              G_r[i] = (2.0 * ekin_rot - Nf_r * kT) / Q_r[i];
            }  //else{ G_r[0] = 0.0; }
            if (Nf_b > 0) {
              G_b[i] = (2.0 * ekin_baro - Nf_b * kT) / Q_b[i];
            }  //else{ G_b[0] = 0.0; }
          } else {
            if (Nf_t > 0) {
              G_t[i] = (Q_t[i - 1] * ksi_t[i - 1] * ksi_t[i - 1] - kT) / Q_t[i];
...
*******************************************************************/
        ...
        if (thermostat_type == "Nose-Hoover") {
          ...
          for (k = 0; k <= M; k++) {
            if (Nf_t > 0) {
              s_t[k] = s_t[k] + dt * ksi_t[k];
            }
            if (Nf_r > 0) {
              s_r[k] = s_r[k] + dt * ksi_r[k];
            }
            if (Nf_b > 0) {
              s_b[k] = s_b[k] + dt * ksi_b[k];
            }
          }

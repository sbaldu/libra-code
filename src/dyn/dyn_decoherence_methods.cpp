  ...
  namespace libdyn {
    ...
    */
      ...
      if (p_aa_old > 1.0 + tol) {
        ...
        // for some algorithms
        cout << "=== Place 1 =====\n";
        cout << "WARNING in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, MATRIX& "
                "decoh_rates):\n";
        cout << "The population of the active state is larger than 1: p_aa_old = " << p_aa_old
             << endl;
        cout << "C = \n";
        C.show_matrix();
      ...
      if (p_aa_old > 0.0) {
        ...
        if (inact_st_pop > 1.0) {
          cout << "=== Place 2 =====\n";
          cout << "Error in CMATRIX sdm(CMATRIX& Coeff, double dt, int act_st, MATRIX& "
                  "decoh_rates):\n";
          cout << "Total population of inactive states after rescaling is larger than 1: "
                  "inact_st_pop = "
               << inact_st_pop << endl;
          cout << "C = \n";
          C.show_matrix();
...
*/
      ...
      if (accepted_states.size() != ntraj) {
        cout << "ERROR in ids: the sizes of the input variables Coeff and accepted_states are "
                "inconsistent\n";
      ...
      if (proposed_states.size() != ntraj) {
        cout << "ERROR in ids: the sizes of the input variables Coeff and proposed_states are "
                "inconsistent\n";
        cout << "Coeff.num_of_cols = = " << ntraj << "\n";
      ...
      if (initial_states.size() != ntraj) {
        cout << "ERROR in ids: the sizes of the input variables Coeff and initial_states are "
                "inconsistent\n";
        cout << "Coeff.num_of_cols = = " << ntraj << "\n";
          ...
          for (i = 0; i < nadi; i++) {
            ...
            for (idof = 0; idof < ndof; idof++) {
              if (i != decohered_state) {
                nac_eff.add(
                    idof, 0, P_i * ham.get_dc1_adi(idof, _id).get(i, decohered_state).real());
              }
            }  // idof
          }  // i
          ...
          if (is_mixed[i] == 1) {
            if (a_ii > upper_lim) {
              ...
              if (alpha > 0.0) {
                ...
                alpha = 0.0;
                cout << "Total energy is drifted due to adiabatic recovery to a classically "
                        "forbidden state!"
                     << endl;
              }
      ...
      for (int traj = 0; traj < ntraj; traj++) {
        ...
        for (int i = 0; i < nadi; i++) {
          if (is_mixed[i] == 1) {
            ...
            if (i == a) {
              ...
              if (is_first[i] == 1) {
                alpha = compute_kinetic_energy(p_real, invM) + ham_adi.get(a, a).real() -
                        ham_adi.get(i, i).real();
              } else {
                p_aux_temp = p_aux_old.row(i).T();
                alpha = compute_kinetic_energy(p_aux_temp, invM) + ham_adi_prev.get(i, i).real() -
                        ham_adi.get(i, i).real();
              }
            }
            ...
            if (alpha < 0.0) {
              ...
              if (prms.project_out_aux == 1) {
                ...
                xf_init_AT(dyn_var, traj, -1);
                cout << "Project out a classically forbidden state " << i << " on traj " << traj
                     << endl;
                break;
              }
            }
        ...
        if (is_tp == 1) {
          if (prms.tp_algo == 1) {
            ...
            xf_init_AT(dyn_var, traj, -1);
            cout << "Collapse to the active state " << a << " at a classical turning point on traj "
                 << traj << endl;
            ...
            if (is_first[i] == 1) {
              ...
              for (int idof = 0; idof < ndof; idof++) {
                nab_phase.set(
                    i,
                    idof,
                    nab_phase_old.get(i, idof) + p_aux.get(i, idof) - p_aux_old.get(i, idof));
              }  //idof
            }
          }
        }  //i
        ...
        if (is_tp == 1) {
          if (prms.tp_algo == 1) {
            ...
            if (alpha > 0.0) {
              ...
              alpha = 0.0;
              cout << "Total energy is drifted due to the dynamics initialization at a classical "
                      "turning point"
                   << endl;
            }
          ...
          if (is_mixed[i] == 1) {
            for (int idof = 0; idof < ndof; idof++) {
              nab_phase.set(
                  i,
                  idof,
                  -0.5 * E.get(i, i).real() * dyn_var.p->get(idof, traj) / (Ekin + prms.e_mask));
            }  //idof
          }
        }  //i
      }  // traj
    ...
    void update_ham_xf(dyn_variables& dyn_var) {
      ...
      for (int traj = 0; traj < ntraj; traj++) {
          ...
          if (is_mixed[i] == 1) {
            for (int idof = 0; idof < ndof; idof++) {
              dyn_var.p_quant->add(
                  idof,
                  traj,
                  0.5 / pow(dyn_var.wp_width->get(idof, traj), 2.0) * RHO.get(i, i).real() *
        ...
        for (int idof = 0; idof < ndof; idof++) {
          ...
          Ham_xf +=
              -invM.get(idof, 0) * dyn_var.p_quant->get(idof, traj) * (RHO * iphi - iphi * RHO);
        }

...
*/
        ...
        if (is_Wg) {
          std::cout << "Wg  = " << Wg << " some units" << std::endl;
        }
        if (is_Nf_t) {
        ...
        if (is_nu_baro) {
          std::cout << "nu_baro = " << nu_baro << " tau" << std::endl;
        }
        if (is_Pressure) {
          std::cout << "Pressure = " << (Pressure * (hartree * Angst * Angst * Angst) / atm_to_int)
                    << " atmospheres " << Pressure << " [Ha/Bohr^3]" << std::endl;
        }
        if (is_barostat_type) {
          std::cout << "barostat_type = " << barostat_type << std::endl;
        }
        std::cout << std::endl;
      }
...
*/
        ...
        is_Nf_r = 1;
        Nf_b = 1;
        is_Nf_b = 1;  // This defines isotropic case (uniform dilation)
        nu_baro = 1.0;
        is_nu_baro = 1;
        Pressure = 1.0 * atm_to_int;
        is_Pressure = 1;  // Default: 1 atm
        barostat_type = "None";
        is_barostat_type = 1;
      }
...
*/
        ...
        if (bar.is_ksi_eps_iso) {
          ksi_eps_iso = bar.ksi_eps_iso;
          is_ksi_eps_iso = 1;
        }
        if (bar.is_G_eps_iso) {
          G_eps_iso = bar.G_eps_iso;
          is_G_eps_iso = 1;
        }
        if (bar.is_Wg) {
          Wg = bar.Wg;
        ...
        if (bar.is_barostat_type) {
          barostat_type = bar.barostat_type;
          is_barostat_type = 1;
        }
      }
...
*/
        ...
        for (int i = 0; i < len(d.values()); i++) {
          ...
          if (key == "Wg") {
            Wg = extract<double>(d.values()[i]);
            is_Wg = 1;
          } else if (key == "nu_baro") {
            nu_baro = extract<double>(d.values()[i]);
            is_nu_baro = 1;
          } else if (key == "Pressure") {
            Pressure = extract<double>(d.values()[i]);
            ...
            is_Pressure = 1;
          } else if (key == "barostat_type") {
            barostat_type = extract<std::string>(d.values()[i]);
            is_barostat_type = 1;
          }
        }
      }
...
*/
        ...
        if (is_ksi_eps) {
          liblinalg::save(pt, path + ".ksi_eps", ksi_eps);
        }
        if (is_G_eps) {
          liblinalg::save(pt, path + ".G_eps", G_eps);
        }
        if (is_eps_iso) {
          libio::save(pt, path + ".eps_iso", eps_iso);
        }
        if (is_ksi_eps_iso) {
          libio::save(pt, path + ".ksi_eps_iso", ksi_eps_iso);
        }
        if (is_G_eps_iso) {
          libio::save(pt, path + ".G_eps_iso", G_eps_iso);
        }
        if (is_Wg) {
          libio::save(pt, path + ".Wg", Wg);
        }
        if (is_nu_baro) {
          libio::save(pt, path + ".nu_baro", nu_baro);
        }
        if (is_Pressure) {
          libio::save(pt, path + ".Pressure", Pressure);
        }
        if (is_barostat_type) {
          libio::save(pt, path + ".barostat_type", barostat_type);
        }
      }
...
*/
        ...
        liblinalg::load(pt, path + ".ksi_eps", ksi_eps, is_ksi_eps);
        if (is_ksi_eps == 1) {
          status = 1;
        }
        liblinalg::load(pt, path + ".G_eps", G_eps, is_G_eps);
        if (is_G_eps == 1) {
          status = 1;
        }
        libio::load(pt, path + ".eps_iso", eps_iso, is_eps_iso);
        if (is_eps_iso == 1) {
          status = 1;
        }
        libio::load(pt, path + ".ksi_eps_iso", ksi_eps_iso, is_ksi_eps_iso);
        if (is_ksi_eps_iso == 1) {
          status = 1;
        }
        libio::load(pt, path + ".G_eps_iso", G_eps_iso, is_G_eps_iso);
        if (is_G_eps_iso == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Wg", Wg, is_Wg);
        if (is_Wg == 1) {
          status = 1;
        }
        libio::load(pt, path + ".nu_baro", nu_baro, is_nu_baro);
        if (is_nu_baro == 1) {
          status = 1;
        }
        libio::load(pt, path + ".Pressure", Pressure, is_Pressure);
        if (is_Pressure == 1) {
          status = 1;
        }
        libio::load(pt, path + ".barostat_type", barostat_type, is_barostat_type);
        if (is_barostat_type == 1) {
          status = 1;
        }
      }

...
*/
        ...
        double res = 0.0;
        if (Nf_b == 9) {
          res = 0.5 * Wg * (ksi_eps.T() * ksi_eps).tr();
        } else if (Nf_b == 1) {
          res = 0.5 * Wg * ksi_eps_iso * ksi_eps_iso;
        }
        return res;
      }
...
*/
        ...
        if (Nf_b == 9) {
          ksi_eps = ksi_eps + t * G_eps;
        } else if (Nf_b == 1) {
          ksi_eps_iso += t * G_eps_iso;
        }
      }
...
*/
        if (Nf_b == 1) {
          ksi_eps_iso = exp(-dt * ksi_b) * ksi_eps_iso +
                        dt * exp(-0.5 * dt * ksi_b) * sinh_(0.5 * dt * ksi_b) * G_eps_iso;
        } else if (Nf_b == 9) {
          ksi_eps = exp(-dt * ksi_b) * ksi_eps +
                    dt * exp(-0.5 * dt * ksi_b) * sinh_(0.5 * dt * ksi_b) * G_eps;
        }
      }

      void Barostat::update_barostat_forces(double ekin_tr,
                                            double ekin_rot,
                                            double curr_volume,
                                            double curr_pressure) {
        /**
...
*/
        ...
        G_eps_iso = (3.0 / Wg) * (curr_volume * (curr_pressure - Pressure) +
                                  (ekin_tr / Nf_t) /*((ekin_tr + ekin_rot)/(Nf_t + Nf_r))*/);
      }

      void Barostat::update_barostat_forces(double ekin_tr,
                                            double ekin_rot,
                                            double curr_volume,
                                            MATRIX3x3& curr_pressure_tensor) {
        /**
...
*/
        ...
        G_eps = (curr_volume * (curr_pressure_tensor - Pressure * I) +
                 /*((ekin_tr + ekin_rot)/(Nf_t + Nf_r))*/ (ekin_tr / Nf_t) * I) /
                Wg;
        G_eps_iso = (3.0 / Wg) * (curr_volume * (curr_pressure - Pressure) +
                                  (ekin_tr / Nf_t) /*((ekin_tr + ekin_rot)/(Nf_t + Nf_r))*/);
      }
...
*/
        ...
        res.identity();
        if (Nf_b == 9) {
          res = (exp1_(ksi_eps, dt));
        } else if (Nf_b == 1) {
          res = res * exp(0.5 * dt * ksi_eps_iso) *
                sinh_(0.5 * dt * ksi_eps_iso); /*exp1_(ksi_eps,dt)*/
          ;
        }
        return res;
      }
...
*/
        ...
        I.identity();
        if (Nf_b == 9) {
          res = exp(-dt * (ksi_r /*+(ksi_eps.tr()/Nf)*/)) * I;
        } else if (Nf_b == 1) {
          res = exp(-dt * (ksi_r /*+ (3.0/Nf)*ksi_eps_iso*/)) * I;
        }
        return res;
      }

...
*/
    ...
    vector<CMATRIX> compute_St(nHamiltonian& ham, int isNBRA) { return compute_St(&ham, isNBRA); }
    ...
    void apply_afssh(dyn_variables& dyn_var,
                     CMATRIX& C,
      ...
      for (traj = 0; traj < ntraj; traj++) {
        ...
        for (idof = 0; idof < ndof; idof++) {
          ...
          integrate_afssh_moments(dR_afssh,
                                  dP_afssh,
                                  hvib_curr,
                                  force_diag,
                                  c_traj,
...
*/
    ...
    void apply_thermal_correction(dyn_variables& dyn_var,
                                  nHamiltonian& ham,
                                  nHamiltonian& ham_old,
...
*/
      ...
      for (int itraj = 0; itraj < ntraj; itraj++) {
        ...
        if (fabs(dyn_var.tcnbra_ekin[itraj] - (-1000.0)) < 1e-4) {
          ...
          dyn_var.tcnbra_thermostats[itraj].Nf_t =
              dyn_var.ndof -
              prms.constrained_dofs.size();  //For now, we assume  prms.quantum_dofs.size();
    ...
    void remove_thermal_correction(dyn_variables& dyn_var,
                                   nHamiltonian& ham,
                                   dyn_control_params& prms) {
      int ntraj = dyn_var.ntraj;
      int nadi = dyn_var.nadi;
            ...
            for (int j = 0; j < nadi; j++) {
              ...
              if (is_first[i] == 1 or is_first[j] == 1) {
                w2_temp.set(
                    -1,
                    0,
                    1.0e+10);  // At initial, an auxiliary pair does not contribute to wp_width
              } else {
                for (int idof = 0; idof < ndof; idof++) {
                  w2_temp.set(
                      idof,
                      0,
                      fabs(dyn_var.q_aux[traj]->get(i, idof) - dyn_var.q_aux[traj]->get(j, idof)) /
...
*/
    ...
    */
    ...
    void propagate_electronic_kcrpmd(dyn_variables& dyn_var,
      ...
      if (y_aux_var[0] >= -0.5 && y_aux_var[0] <= 0.5) {
        if (gamKP == 0.0) {
          ...
          f_aux_var[0] =
              (Ham.kcrpmd_effective_auxiliary_force(y_aux_var, beta, eta, a, b, c, d))[0];
          p_aux_var[0] += 0.5 * f_aux_var[0] * dt;
        } else {
          ...
          y_aux_var[0] +=
              p_aux_var[0] / m_aux_var[0] * dt + 0.5 * theta / sqrt(3.0) * sigma * sqrt(pow(dt, 3));
          f_aux_var[0] =
              (Ham.kcrpmd_effective_auxiliary_force(y_aux_var, beta, eta, a, b, c, d))[0];
          p_aux_var[0] += 0.5 * xi * sigma * m_aux_var[0] * sqrt(dt) +
                          0.5 * (f_aux_var[0] - gamKP * p_aux_var[0]) * dt -
    ...
    void renormalize_hopping_probabilities(vector<vector<double> >& g,
...
*/
      ...
      if (prms.ensemble == 1) {
        ...
        if (n_therm_dofs != prms.thermostat_dofs.size()) {
          cout << "Error in compute_dynamics: The number of thermostat DOFs ( currently "
      ...
      if (prms.ensemble == 1) {
        if (therm[0].thermostat_type == "Nose-Hoover") {
          ...
          for (idof = 0; idof < n_therm_dofs; idof++) {
            ...
            for (traj = 0; traj < ntraj; traj++) {
              sigma_therm.set(
                  dof, traj, sqrt(2 * invM.get(dof, traj) * therm[traj].nu_therm / beta));
              xi_therm.set(dof, traj, rnd.normal());
              theta_therm.set(dof, traj, rnd.normal());
              extra_force.set(
                  dof,
                  traj,
                  0.5 * xi_therm.get(dof, traj) * sigma_therm.get(dof, traj) / invM.get(dof, traj) *
                          sqrt(prms.dt) -
                      0.5 * therm[traj].nu_therm * dyn_var.p->get(dof, traj) * prms.dt -
                      0.25 *
                          (xi_therm.get(dof, traj) / 2.0 + theta_therm.get(dof, traj) / sqrt(3.0)) *
                          therm[traj].nu_therm * sigma_therm.get(dof, traj) / invM.get(dof, traj) *
                          sqrt(pow(prms.dt, 3)) -
                      0.125 * therm[traj].nu_therm *
                          (dyn_var.f->get(dof, traj) -
                           therm[traj].nu_therm * dyn_var.p->get(dof, traj)) *
                          pow(prms.dt, 2));
              extra_momentum.set(dof,
                                 traj,
                                 theta_therm.get(dof, traj) * sigma_therm.get(dof, traj) /
      ...
      *dyn_var.p = *dyn_var.p + 0.5 * prms.dt * (*dyn_var.f) + extra_force;
      ...
      /// Compute the dephasing rates according the original energy-based formalism
        ...
        decoherence_rates = edc_rates(
            Eadi, Ekin, prms.decoherence_C_param, prms.decoherence_eps_param, prms.isNBRA);
      }

      else if (prms.decoherence_times_type == 2) {
        decoherence_rates =
            schwartz_1(prms, *dyn_var.ampl_adi, ham, *prms.schwartz_decoherence_inv_alpha);
      }
        ...
        decoherence_rates =
            schwartz_1(prms, *dyn_var.ampl_adi, *dyn_var.p, ham, *prms.schwartz_interaction_width);
      }
      ...
      if (prms.decoherence_algo == 0) {
        if (prms.rep_tdse == 1) {
          *dyn_var.ampl_adi = sdm(*dyn_var.ampl_adi,
                                  prms.dt,
                                  dyn_var.act_states,
                                  decoherence_rates,
                                  prms.sdm_norm_tolerance,
                                  prms.isNBRA);
        } else {
          ...
          exit(0);
                 ...
                 1) { /* ID-A. Nothing to do here, this is done in the surface hopping section */
      } else if (prms.decoherence_algo ==
                 2) { /* A-FSSH. Nothing to do here, this is done in the surface hopping section */
      }
      // BCSH
        ...
        if (prms.rep_tdse == 1) {
          ...
          *dyn_var.ampl_adi =
              bcsh(*dyn_var.ampl_adi, prms.dt, dyn_var.act_states, *dyn_var.reversal_events);
        } else {
      ...
      // FSSH, GFSH, MSSH, LZ, ZN, DISH, MASH, FSSH2, FSSH3
               ...
               prms.tsh_method == 3 || prms.tsh_method == 4 || prms.tsh_method == 5 ||
               prms.tsh_method == 6 || prms.tsh_method == 7 || prms.tsh_method == 8 ||
               prms.tsh_method == 9) {
        vector<int> old_states(dyn_var.act_states);
        ...
        if (prms.tsh_method == 0 || prms.tsh_method == 1 || prms.tsh_method == 2 ||
          ...
          if (prms.rep_sh == 1) {
            ...
            act_states_dia =
                accept_hops(dyn_var, ham, prop_states, dyn_var.act_states_dia, prms, rnd);
          }
          ...
          if (prms.decoherence_algo == 1) {
            if (prms.rep_tdse == 1) {
              instantaneous_decoherence(*dyn_var.ampl_adi,
                                        act_states,
                                        prop_states,
                                        old_states,
                                        prms.instantaneous_decoherence_variant,
                                        prms.collapse_option);
            } else {
              cout << "ERROR: Instantaneous Decoherence requires rep_tdse = 1\nExiting now...\n";
              exit(0);
            }
            ...
            if (prms.rep_sh == 1) {
              ...
              cout << "ERROR: Independent-trajectory XF methods require rep_sh = 1\nExiting "
                      "now...\n";
              exit(0);
            }
          }  // SHXF or MQCXF
          ...
          // Experimental: instantaneous decoherence in diabatic basis | TODO: check the compatibility with rep_sh==0
            ...
            if (prms.rep_tdse == 1) {
              instantaneous_decoherence_dia(*dyn_var.ampl_adi,
                                            ham,
                                            act_states,
                                            prop_states,
                                            old_states,
        ...
        // DISH - the old one
          ...
          if (prms.decoherence_algo == -1) {
            ...
            cout << "ERROR: DISH method should be used only with decoherence_algo = -1\nExiting "
                    "now...\n";
            exit(0);
          }
        ...
        cout << "tsh_method == " << prms.tsh_method << " is undefined.\nExiting...\n";
        exit(0);
      }

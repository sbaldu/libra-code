...
*/
  ...
  */
        ...
        // Gaussian wavepacket
        void (Wfcgrid2::*expt_add_wfc_Gau_v1)(vector<double>& x0,
                                              vector<double>& px0,
                                              vector<double>& dx0,
                                              int init_state,
                                              complex<double> weight,
                                              int rep) = &Wfcgrid2::add_wfc_Gau;

        // Harmonic oscillator wavepacket
        void (Wfcgrid2::*expt_add_wfc_HO_v1)(vector<double>& x0,
                                             vector<double>& px0,
                                             vector<double>& alpha,
                                             int init_state,
                                             vector<int>& nu,
                                             complex<double> weight,
                                             int rep) = &Wfcgrid2::add_wfc_HO;

        // Arbitrary wavefunction
        void (Wfcgrid2::*expt_add_wfc_ARB_v1)(bp::object py_funct, bp::object params, int rep) =
            &Wfcgrid2::add_wfc_ARB;
        ...
        // Auxiliary functions
        int (*expt_points_on_same_line_v1)(int idof, vector<int>& pt1, vector<int>& pt2) =
            &points_on_same_line;

        def("points_on_same_line", expt_points_on_same_line_v1);

        void (*expt_dvr0_v1)(MATRIX& T, int npts, double rmin, double rmax, double mass) = &dvr0;
        def("dvr0", expt_dvr0_v1);

        void (*expt_dvr1_v1)(MATRIX& T, int npts, double dr, double mass) = &dvr1;
        def("dvr1", expt_dvr1_v1);

        void (*expt_dvr2_v1)(MATRIX& T, int npts, double dr, double mass) = &dvr2;
...
*/
        ...
        class_<Wfcgrid2>(
            "Wfcgrid2",
            init<const vector<double>&, const vector<double>&, const vector<double>&, int>())

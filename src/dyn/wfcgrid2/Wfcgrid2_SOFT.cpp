...
*/
        ...
        for (int npt1 = 0; npt1 < Npts; npt1++) {
          for (int nst = 0; nst < nstates; nst++) {
            cs = std::cos(dt *
                          (Hadi[npt1].get(nst, nst).real() + Vcomplex[npt1].get(nst, nst).real()));
            si = std::sin(dt *
                          (Hadi[npt1].get(nst, nst).real() + Vcomplex[npt1].get(nst, nst).real()));
            scl = std::exp(dt *
                           (Hadi[npt1].get(nst, nst).imag() + Vcomplex[npt1].get(nst, nst).imag()));
...
*/
        ...
        for (int npt1 = 0; npt1 < Npts; npt1++) {
          ...
          for (int nst = 0; nst < nstates; nst++) {
            expK[npt1].set(nst, nst, complex<double>(std::cos(kfactor), std::sin(kfactor)));
          }  //for nst

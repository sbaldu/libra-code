...
*/
        ...
        ///=============== In the Wfcgrid2.cpp ====================
        void init_numbers(const vector<double>& rmin_,
                          const vector<double>& rmax_,
                          const vector<double>& dr_,
                          int nstates_);
        void allocate();
        void init_grids();
        ...
        vector<CMATRIX>
            PSI_dia_past;  ///< wavefunction at time t-dt:    list of Npts matrices  nstates x 1
        vector<CMATRIX>
            PSI_adi_past;  ///< wavefunction at time t-dt:    list of Npts matrices  nstates x 1
      ...
      public:
        ...
        vector<MATRIX*>
            rgrid;  ///< rgrid[dof].get(ipt) - coordinate of the real-space point ipt along the dof dimension
        vector<MATRIX*>
            kgrid;  ///< kgrid[dof].get(ipt) - coordinate of the reciptocal-space point ipt along the dof dimension
        /**
  ...
  */
        ...
        /// Linearized version:
        CMATRIX*
            lin_PSI_dia;  ///< wavefuncton: nstates blocks of Npts points in each, so it is an (nstates * Npts) x 1 matrix
        ...
        /// Linearized version:
        CMATRIX*
            lin_PSI_adi;  ///< wavefuncton: nstates blocks of Npts points in each, so it is an (nstates * Npts) x 1 matrix

        /// Wavefunction spatial derivatives
        vector<vector<CMATRIX> >
            nabla_PSI_dia;  /// d/dR PSI_dia in real space; nablaPSI_dia[dof][ipt].get(istate, 0)   dPSI_dia/dR_dof ( r[ipt]) - nstates x 1 matrix
        vector<vector<CMATRIX> >
            nabla_PSI_adi;  /// d/dR PSI_adi in real space; nablaPSI_adi[dof][ipt].get(istate, 0)   dPSI_adi/dR_dof ( r[ipt]) - nstates x 1 matrix
        vector<vector<CMATRIX> >
            nabla_reciPSI_dia;  /// d/dR PSI_dia in k-space - same structure as nabla_PSI_dia;
        vector<vector<CMATRIX> >
            nabla_reciPSI_adi;  /// d/dR PSI_adi in k-space - same structure as nabla_PSI_adi;
...
*/
        ...
        CMATRIX*
            lin_Hdia;  ///<  linearized diabatic Hamiltonian - a block diagonal matrix (nstates * Npts) x (nstates * Npts)
        vector<CMATRIX> Hadi;  ///<  adiabatic Hamiltoninans for all the Npts points
        CMATRIX*
            lin_Hadi;  ///<  linearized adiabatic Hamiltonian - a block diagonal matrix (nstates * Npts) x (nstates * Npts)
        vector<CMATRIX>
            Vcomplex;  ///<  complex absorbing potential that will be added to the real-space propagator
        vector<vector<CMATRIX> >
            NAC1;  ///<  1-st order NACS: NAC1[ipt][alpha].get(i,j) = <psi_i| nabla_alpha | psi_j>
        vector<vector<CMATRIX> >
            NAC2;  ///<  2-nd order NACS: NAC2[ipt][alpha].get(i,j) = <psi_i| nabla_alpha^2 | psi_j>
        vector<CMATRIX>
            U;  ///<  |adi> = |dia> * U : diabatic-to-adiabatic transformation for all the Npts points
        ...
        CMATRIX*
            lin_expH;  ///<  linearized form of the exponent of the diabatic Hamiltonian - a (nstates * Npts) x (nstates * Npts) matrix
        vector<CMATRIX> expK;  ///<  exponent of the kinetik energy propagator for all the Npts points

        ///=============== In the Wfcgrid2.cpp ====================
        //< Auxiliary converter functions:
        void convert_PSI(int _rep,
                         int _dir);  // converts  PSI_dia (PSI_adi) <-> lin_PSI_dia (lin_PSI_adi)
        void convert_Ham(int _rep, int _dir);  // converts  Hdia (Hadi) <-> lin_Hdia (lin_Hadi)

        ///=============== In the Wfcgrid2.cpp ====================
        ///< Grid constructor
        Wfcgrid2(const vector<double>& rmin_,
                 const vector<double>& rmax_,
                 const vector<double>& dr_,
                 int nstates_);  ///< constructor for n-D wavefunction
        ...
        ///=============== In the Wfcgrid2_ColbertMiller.cpp ===============
        vector<CMATRIX> T_PSI(vector<CMATRIX>& inp_psi,
                              vector<int>& bc_type,
                              vector<double>& mass,
                              complex<double> scaling);

        vector<CMATRIX> T_PSI_adi(vector<int>& bc_type,
                                  vector<double>& mass,
                                  complex<double> scaling);
        vector<CMATRIX> T_PSI_dia(vector<int>& bc_type,
                                  vector<double>& mass,
                                  complex<double> scaling);
        ...
        vector<CMATRIX> V_PSI(vector<CMATRIX>& V,
                              vector<CMATRIX>& inp_psi,
                              complex<double> scaling);
        ...
        void add_wfc_Gau(vector<double>& x0,
                         vector<double>& px0,
                         vector<double>& dx0,
                         int init_state,
                         complex<double> weight,
                         int rep);
        void add_wfc_HO(vector<double>& x0,
                        vector<double>& px0,
                        vector<double>& alpha,
                        int init_state,
                        vector<int>& nu,
        ...
        void reshape_wfc_1D(
            ...
            int _r_or_k,
            int _dir,
            vector<CMATRIX>& _tmp);  // reshape wfc into/from the nstates x CMATRIX(Nx, 1) format
        void reshape_wfc_2D(
            int _rep,
            int _r_or_k,
            int _dir,
            vector<CMATRIX>& _tmp);  // reshape wfc into/from the nstates x CMATRIX(Nx, Ny) format
        ...
        void print_wfc_1D(std::string prefix,
                          int rep,
                          vector<int>& states,
                          int do_real,
                          int do_imag,
                          int do_dens);
        void print_reci_wfc_1D(std::string prefix,
                               int rep,
                               vector<int>& states,
                               int do_real,
                               int do_imag,

...
namespace liblibra {
  ...
  namespace libdyn {
    ...
    namespace libwfcgrid2 {
      ...
      void Wfcgrid2::init_numbers(const vector<double>& rmin_,
...
*/
        ...
        reciPSI_dia = vector<CMATRIX>(
            Npts,
            CMATRIX(nstates, 1));  ///< same as PSI but in Fourier (reciprocal) space with 1.0*kmin
                                   //  lin_PSI_dia = new CMATRIX(dim, 1);

        PSI_adi = vector<CMATRIX>(Npts, CMATRIX(nstates, 1));  ///< wavefunction  Npts x nstates x 1
        reciPSI_adi = vector<CMATRIX>(
            Npts,
            CMATRIX(nstates, 1));  ///< same as PSI but in Fourier (reciprocal) space with 1.0*kmin
                                   //  lin_PSI_adi = new CMATRIX(dim, 1);

        nabla_PSI_dia = vector<vector<CMATRIX> >(
            ndof,
            vector<CMATRIX>(
                Npts, CMATRIX(nstates, 1)));  ///   dPSI_dia/dR_dof ( r[ipt]) - nstates x 1 matrix
        nabla_PSI_adi = vector<vector<CMATRIX> >(
            ndof,
            vector<CMATRIX>(
                Npts, CMATRIX(nstates, 1)));  ///   dPSI_adi/dR_dof ( r[ipt]) - nstates x 1 matrix
        nabla_reciPSI_dia = vector<vector<CMATRIX> >(
            ndof,
            vector<CMATRIX>(
                Npts,
                CMATRIX(nstates,
                        1)));  ///   dPSI_dia/dR_dof ( r[ipt]) - nstates x 1 matrix in k-space
        nabla_reciPSI_adi = vector<vector<CMATRIX> >(
            ndof,
            vector<CMATRIX>(
                Npts,
                CMATRIX(nstates,
                        1)));  ///   dPSI_adi/dR_dof ( r[ipt]) - nstates x 1 matrix in k-space

        Hdia = vector<CMATRIX>(
            Npts, CMATRIX(nstates, nstates));  ///<  diabatic Hamiltoninans for all the Npts points
                                               //  lin_Hdia = new CMATRIX(dim, dim);
        Hadi = vector<CMATRIX>(
            Npts, CMATRIX(nstates, nstates));  ///<  adiabatic Hamiltoninans for all the Npts points
                                               //  lin_Hadi = new CMATRIX(dim, dim);
        Vcomplex =
            vector<CMATRIX>(Npts, CMATRIX(nstates, nstates));  ///<  complex absorbing potential
        U = vector<CMATRIX>(
            Npts,
            CMATRIX(
                nstates,
                nstates));  ///<  |adi> = |dia> * U : diabatic-to-adiabatic transformation for all the Npts points
                            //  lin_U = new CMATRIX(dim, dim);
        expH = vector<CMATRIX>(
            Npts,
            CMATRIX(nstates,
                    nstates));  ///<  exponent of the diabatic Hamiltoninans for all the Npts points
                                //  lin_expH = new CMATRIX(dim, dim);
        expK = vector<CMATRIX>(
            Npts,
            CMATRIX(
                nstates,
                nstates));  ///<  exponent of the kinetic energy propagator for all the Npts points
...
*/
        ...
        for (dof = 0; dof < ndof; dof++) {
          ...
          *rgrid[dof] =
              libdyn::libwfcgrid::init_grid(rmin[dof], rmax[dof], dr[dof]).real().T();  // real space
          ...
          cout << "For idof = " << dof << "\n";
          cout << "Limits of the r-grid: [" << rgrid[dof]->M[0] << " , "
               << rgrid[dof]->M[npts[dof] - 1] << " ]\n";
          cout << "Limits of the k-grid: [" << kgrid[dof]->M[0] << " , "
               << kgrid[dof]->M[npts[dof] - 1] << " ]\n";
        }
      ...
      // Constructor
      Wfcgrid2::Wfcgrid2(const vector<double>& rmin_,
                         const vector<double>& rmax_,
                         const vector<double>& dr_,
                         int nstates_) {
        /**
...
*/
        ...
        if (_rep == 0 && _dir == 1) {
          for (i = 0; i < nstates; i++) {
            for (ipt = 0; ipt < Npts; ipt++) {
              lin_PSI_dia->set(i * Npts + ipt, 0, PSI_dia[ipt].get(i, 0));
            }
          }
        }  // diabtic, direct-> lin
        ...
        if (_rep == 1 && _dir == 1) {
          for (i = 0; i < nstates; i++) {
            for (ipt = 0; ipt < Npts; ipt++) {
              lin_PSI_adi->set(i * Npts + ipt, 0, PSI_adi[ipt].get(i, 0));
            }
          }
        }  // adiabtic, direct-> lin
        ...
        if (_rep == 0 && _dir == -1) {
          for (i = 0; i < nstates; i++) {
            for (ipt = 0; ipt < Npts; ipt++) {
              PSI_dia[ipt].set(i, 0, lin_PSI_dia->get(i * Npts + ipt, 0));
            }
          }
        }  // diabtic, lin -> direct
        ...
        if (_rep == 1 && _dir == -1) {
          for (i = 0; i < nstates; i++) {
            for (ipt = 0; ipt < Npts; ipt++) {
              PSI_adi[ipt].set(i, 0, lin_PSI_adi->get(i * Npts + ipt, 0));
            }
          }
        }  // adiabtic, lin -> direct
        ...
        for (int i = 0; i < nstates; i++) {
          for (int j = 0; j < nstates; j++) {
            for (int ipt = 0; ipt < Npts; ipt++) {
              if (_rep == 0) {
                if (_dir == 1) {
                  lin_Hdia->set(i * Npts + ipt, j * Npts + ipt, Hdia[ipt].get(i, j));
                } else if (_dir == -1) {
                  Hdia[ipt].set(i, j, lin_Hdia->get(i * Npts + ipt, j * Npts + ipt));
                }
              }

              else if (_rep == 1) {
                if (_dir == 1) {
                  lin_Hadi->set(i * Npts + ipt, j * Npts + ipt, Hadi[ipt].get(i, j));
                } else if (_dir == -1) {
                  Hadi[ipt].set(i, j, lin_Hadi->get(i * Npts + ipt, j * Npts + ipt));
                }
              }

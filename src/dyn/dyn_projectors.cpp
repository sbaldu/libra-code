        ...
        if (!visited_r[v] && cost_matrix[u][v] == 0) {
          ...
          if (match_r[v] == -1 ||
              find_augmenting_path(
                  match_r[v], cost_matrix, match_l, match_r, visited_l, visited_r)) {
            match_l[u] = v;
            match_r[v] = u;
            return true;
          }
...
*/
      ...
      for (i = 0; i < nc; i++) {
        ...
        double af = sqrt(std::norm(
            f));  // std::norm(val) returns the norm squared of complex number val. So we use sqrt()
    ...
    CMATRIX compute_phase_corrections(CMATRIX& S) { return compute_phase_corrections(S, 1e-3); }
    ...
    */
    ...
    vector<int> Munkres_Kuhn(CMATRIX& orb_mat_inp,
                             CMATRIX& en_mat_inp,
                             double alpha,
                             int verbosity) {
      MATRIX cost_mat = make_cost_mat(orb_mat_inp, en_mat_inp, alpha);
      ...
      for (i = 0; i < sz; i++) {  // all starting states (at time t)
        ...
        if (act_st_sz > 1) {
          ...
          if (target > act_st_sz - 1) {
            cout << "ERROR: target = " << target
                 << " is larger than max possible value = " << act_st_sz - 1 << endl;
            exit(0);
          }
    ...
    */
      ...
      for (i = 0; i < number_states; i++) {
        identity_permutation.push_back(i);
      }
      ...
      for (iperm = 0; iperm < num_permutations; iperm++) {
        ...
        for (i = 0; i < number_states; i++) {
          permutation_probability *=
              real(conj(S.get(i, permutation[i])) * S.get(i, permutation[i]));
        }  // for i
    ...
    */
      ...
      while (!is_accepted && (number_of_attempts < max_number_of_attempts)) {
        if (verbosity_level > 0) {
          std::cout << "Attempt # " << number_of_attempts << endl;
        }
        //======= propose hop ========
        ...
        if (verbosity_level > 0) {
          std::cout << " Is accepted: " << is_accepted << endl;
        }
      ...
      if (!is_accepted && !convergence) {
        ...
        std::cout << "Stochastic reordering did not converge in " << max_number_of_attempts
                  << " attempts\nChoosing identity...\n";
      }
      if (!is_accepted && convergence) {
        std::cout << "Stochastic reordering did not converge in " << max_number_of_attempts
    ...
    vector<int> get_stochastic_reordering3(CMATRIX& time_overlap,
      ...
      return get_stochastic_reordering3(
          time_overlap, rnd, convergence, max_number_of_attempts, filter_tol, verbosity_level);
    }
    ...
    void update_projectors(dyn_control_params& prms,
      ...
      for (int traj = 0; traj < ntraj; traj++) {
        ...
        if (prms.state_tracking_algo == 33) {
          perm_t = get_stochastic_reordering3(st,
                                              rnd,
                                              prms.convergence,
                                              prms.max_number_attempts,
                                              prms.min_probability_reordering,
                                              0);
        }
        if (prms.state_tracking_algo ==
            4) {  // same as option 2, but the input  for St argument will be different
          perm_t = Munkres_Kuhn(st, Eadi[traj], prms.MK_alpha, prms.MK_verbosity);
        }
...
*/
      ...
      if (prms.state_tracking_algo == 1) {
        perm_t = get_reordering(st);
      } else if (prms.state_tracking_algo == 2 || prms.state_tracking_algo == 4) {
        perm_t = Munkres_Kuhn(st, Eadi, prms.MK_alpha, prms.MK_verbosity);
      } else if (prms.state_tracking_algo == 21) {
        perm_t = hungarian_algorithm(st, Eadi, prms.MK_alpha);
      }
...
*/
      ...
      if (prms.do_phase_correction) {
        ...
        // ### Scale projections' components by the phases ###
        for (int a = 0; a < nst; a++) {
          res.scale(-1, a, std::conj(phase_i.get(a)));
        }
      }
...
*/
    ...
    CMATRIX compute_F_cost_matrix2(CMATRIX& F_curr,
                                   CMATRIX& F_prev,
                                   MATRIX& e_curr,
                                   MATRIX& e_prev,
                                   MATRIX& _p,

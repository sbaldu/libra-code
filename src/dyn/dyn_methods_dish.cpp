...
*/
      ...
      for (traj = 0; traj < ntraj; traj++) {
        ...
        for (i = 0; i < nst; i++) {
          ...
          double tau = 0.0;
          if (decoherence_event_option == 0) {
            tau = coherence_interval.get(i, traj);
          } else if (decoherence_event_option == 1) {
            tau = rnd.exponential(1.0 / coherence_interval.get(i, traj));
          }

          if (coherence_time.get(i, traj) >= tau) {
    ...
    vector<int> dish(dyn_variables& dyn_var,
      ...
      vector<int> decohered_states(decoherence_event(
          coherence_time, coherence_interval, prms.dish_decoherence_event_option, rnd));
      ...
      for (traj = 0; traj < ntraj; traj++) {
        ...
        if (istate > -1) {
          ...
          /// Situation 2: Decohered state is an inactive state
            ...
            if (ksi <=
                prob) {  // Try to collapse on the decohered state with the |C_istate|^2 probability
              ...
              new_states = accept_hops(
                  dyn_var, ham, proposed_states, old_states, prms, rnd, which_trajectories);
    ...
    void dish_rev2023(dyn_variables& dyn_var,
      ...
      if (prms.decoherence_times_type == 2 || prms.decoherence_times_type == 4) {
        for (traj = 0; traj < ntraj; traj++) {
          for (int i = 0; i < nst; i++) {
            double tt = 1e+10;
            if (fabs(decoherence_rates[traj].get(i, i)) > 0.0) {
              tt = 1.0 / decoherence_rates[traj].get(i, i);
            }
      ...
      for (traj = 0; traj < ntraj; traj++) {
        ...
        if (istate > -1) {
          ...
          if (ksi <= prob) {
            collapse(Coeff, traj, istate, collapse_option);
          } else {
            project_out(Coeff, traj, istate);
          }

...
*/
    ...
    */
      ...
      vector<int> filter(vector<CMATRIX>& rho,
                         vector<int>& adm_list,
                         double tolerance,
                         int do_zeroing) {
        /**
...
*/
        ...
        if (nonzero[n] == 1) {
          ...
          //============= Truncation ======================
          if (truncation_scheme == 1 || truncation_scheme == 2 || truncation_scheme == 3 ||
              truncation_scheme == 4) {
        ...
        for (m = 0; m < nquant; m++) {
            ...
            if (nplus != -1) {
              if (nonzero[nplus] == 1) {
                ...
                if (do_scale == 1) {  // Scaled HEOM approach from JCP 130, 084105 (2009)
                  scaling_factor =
                      sqrt((nvectors[n][m * (KK + 1) + k] + 1.0) * std::abs(c_matsubara[k]));
                }
          ...
          drho_n_dt -=
              iota * (el_phon_coupl[m] * sum_rho_over_k - sum_rho_over_k * el_phon_coupl[m]);
        ...
        for (m = 0; m < nquant; m++) {
          ...
          drho_n_dt -=
              iota * (el_phon_coupl[m] * sum1_c_rho_over_k - sum2_c_rho_over_k * el_phon_coupl[m]);
    ...
    */
        ...
        if (truncation_scheme == 3 || truncation_scheme == 4) {
          ...
          sum_KK_ext = compute_matsubara_sum(gamma_matsubara_ext, c_matsubara_ext, KK + more);
...
#pragma omp for
          for (int n1 = 0; n1 < nn_nonzero; n1++) {
            drho_unpacked[adm_list[n1]] = compute_deriv_n(adm_list[n1],
                                                          rho_unpacked,
                                                          Ham,
                                                          el_phon_couplings,
                                                          eta,

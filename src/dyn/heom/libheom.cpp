...
*/
        ...
        vector<vector<int> > (*expt_gen_next_level_v1)(vector<vector<int> >& parents) =
            &gen_next_level;
        def("gen_next_level", expt_gen_next_level_v1);

        void (*expt_gen_hierarchy_v1)(int d,
                                      int max_tier,
                                      int verbosity,
        ...
        vector<int> (*expt_filter_v1)(
            vector<CMATRIX>& rho, vector<int>& adm_list, double tolerance, int do_zeroing) =
            &filter;
        ...
        vector<CMATRIX> (*expt_initialize_el_phonon_couplings_v1)(int nquant) =
            &initialize_el_phonon_couplings;
        def("initialize_el_phonon_couplings", expt_initialize_el_phonon_couplings_v1);

        complex<double> (*expt_compute_matsubara_sum_v1)(
            vector<double>& gamma_matsubara, vector<complex<double> >& c_matsubara, int KK) =
            &compute_matsubara_sum;
        def("compute_matsubara_sum", expt_compute_matsubara_sum_v1);

        void (*expt_setup_bath_v1)(int KK,
                                   double eta,
                                   double gamma,
                                   double temperature,
                                   vector<double>& gamma_matsubara,
                                   vector<complex<double> >& c_matsubara) = &setup_bath;
        def("setup_bath", expt_setup_bath_v1);

        void (*expt_unpack_mtx_v1)(vector<CMATRIX>& rho_unpacked, CMATRIX& RHO) = &unpack_mtx;
        def("unpack_mtx", expt_unpack_mtx_v1);

        void (*expt_pack_mtx_v1)(vector<CMATRIX>& rho_unpacked, CMATRIX& RHO) = &pack_mtx;

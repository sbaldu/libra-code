...
*/
      ...
      if (update_type == 0 || update_type == 1) {
        ...
        if (prms.do_nac_phase_correction == 2) {  // Experimental option to fix the phase of NACVs
          ...
          for (int traj = 0; traj < ntraj; traj++) {
            ...
            MATRIX T_new(
                compute_F_cost_matrix(f_curr, f_prev, e_curr, e_prev, pp, iM, dt, act_state).real());
    ...
    void update_Hamiltonian_variables(bp::dict prms,
      ...
      update_Hamiltonian_variables(
          _prms, dyn_var, ham, ham_prev, py_funct, model_params, update_type);
    }
      ...
      for (int itraj = 0; itraj < ntraj; itraj++) {
        ...
        if (prms.state_tracking_algo == -1) {  // This is LD
          ...
          T_new = orthogonalized_T(T_new);
                   ...
                   prms.state_tracking_algo == 21 || prms.state_tracking_algo == 3 ||
          ...
          T_new = compute_projector(
              prms,
              Eadi,
              T_new);  // CMATRIX compute_projector(dyn_control_params& prms, CMATRIX& Eadi, CMATRIX& St){
          //  T_new = orthogonalized_T( T_new );
        }

        ...
        for (i = 0; i < ntraj1; i++) {
          ...
          for (j = 0; j < ntraj2; j++) {
            ...
            if (state1[i] == state2[j]) {
              sij = gwp_overlap(q1_i, p1_i, g1_i, a1_i, q2_j, p2_j, g2_j, a2_j);
            }
...
*/
        ...
        double re = -0.5 * alp * ((R2 - R1).T() * (R2 - R1)).M[0] -
                    (0.125 / (alp * hbar * hbar)) * ((P2 - P1).T() * (P2 - P1)).M[0];
...
*/
        ...
        double scl =
            gwp_product_decomposition(q1, p1, gamma1, alp1, q2, p2, gamma2, alp2, q, p, gamma, alp);

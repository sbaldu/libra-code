...
*/
        ...
        return -0.5 *
               complex<double>(-pref1 + pref1 * pref1 * dq * dq - p_weighted * p_weighted,
                               2.0 * pref1 * p_weighted * dq) *
               ovlp;
      }
...
*/
        ...
        for (int i = 0; i < Ndof; i++) {
          ...
          res += complex<double>(-pref1 + pref1 * pref1 * dq * dq - p_weighted * p_weighted,
                                 2.0 * pref1 * p_weighted * dq);
        }
...
*/
        ...
        // Overlap part
        double re = -alp + alp * alp * ((R2 - R1).T() * (R2 - R1)).M[0] -
                    0.25 * ((P2 + P1).T() * (P2 + P1)).M[0] / (hbar * hbar);

...
*/
      ...
      if (det < 0.0) {  // Frustrated hops!
        ...
        if (fabs(a_ij) > 1e-100) {  // only consider reversals, if the couplings are sizable
          if (b_ij < 0) {
            gamma_ij = 0.5 * (b_ij + sqrt(det)) / a_ij;
          } else {
            gamma_ij = 0.5 * (b_ij - sqrt(det)) / a_ij;
          }
        } else {
...
*/
      ...
      if (prms.hop_acceptance_algo == 0) {  // Just accept all the proposed hops
        ...
        for (itraj = 0; itraj < ntraj_active; itraj++) {
          ...
          if (old_st != new_st) {
            p_tr = p.col(traj);
            double T_i = compute_kinetic_energy(p_tr, invM, which_dofs);  // initial kinetic energy
            double E_i = ham.children[traj]
                             ->get_ham_dia()
                             .get(old_st, old_st)
                             .real();  // initial potential energy
            double E_f = ham.children[traj]
                             ->get_ham_dia()
                             .get(new_st, new_st)
                             .real();      // final potential energy
            double T_f = T_i + E_i - E_f;  // predicted final kinetic energy
      ...
      else if (prms.hop_acceptance_algo ==
               20) {  // if rescaling momenta along the derivative coupling vector
        ...
        for (itraj = 0; itraj < ntraj_active; itraj++) {
          ...
          if (old_st != new_st) {
            ...
            fstates[traj] = initial_states[traj];
               ...
               21) {  // if rescaling momenta along the difference in forces
        ...
        for (itraj = 0; itraj < ntraj_active; itraj++) {
          ...
          if (old_st != new_st) {
            ...
            fstates[traj] = initial_states[traj];
               ...
               31) {  // stochastic decision based on quantum Boltzmann factors
        ...
        for (itraj = 0; itraj < ntraj_active; itraj++) {
          ...
          if (ksi < prob) {
            ...
            fstates[traj] = initial_states[traj];
               ...
               32) {  // stochastic decision based on classical Maxwell-Boltzmann factors
        ...
        for (itraj = 0; itraj < ntraj_active; itraj++) {
          ...
          if (ksi < prob) {
            ...
            fstates[traj] = initial_states[traj];
               ...
               33) {  // stochastic decision based on quantum probabilities
...
*/
    vector<int> accept_hops(dyn_variables& dyn_var,
      ...
      for (int i = 0; i < ntraj; i++) {
        which_trajectories[i] = i;
      }

      return accept_hops(
          dyn_var, ham, proposed_states, initial_states, prms, rnd, which_trajectories);
...
*/
    vector<int> where_can_we_hop(int traj,
...
*/
      ...
      for (int istate = 0; istate < nstates; istate++) {
        if (istate != act_states[traj]) {
          ...
          new_states =
              accept_hops(dyn_var, ham, proposed_states, act_states, prms, rnd, which_trajectories);
...
*/
      ...
      if (prms.momenta_rescaling_algo == 0) {  // Don't rescale at all
      ...
      else if (prms.momenta_rescaling_algo == 100 ||
               prms.momenta_rescaling_algo ==
                   101) {  // rescale momenta uniformly based on adiabatic energies
        ...
        for (traj = 0; traj < ntraj; traj++) {
          ...
          if (old_st != new_st) {
            ...
            if (T_f >= 0.0) {
              scl_fac = std::sqrt(T_f / T_i);
            } else {
              if (prms.momenta_rescaling_algo == 100) {
                scl_fac = 1.0;
              } else if (prms.momenta_rescaling_algo == 101) {
                scl_fac = -1.0;
              }
            }
      ...
      else if (prms.momenta_rescaling_algo == 110 ||
               prms.momenta_rescaling_algo ==
                   111) {  // rescale momenta uniformly based on diabatic energies
        ...
        for (traj = 0; traj < ntraj; traj++) {
          ...
          if (old_st != new_st) {
            ...
            if (T_f >= 0.0) {
              ...
              if (prms.momenta_rescaling_algo == 110) {
                scl_fac = 1.0;
              } else if (prms.momenta_rescaling_algo == 111) {
                scl_fac = -1.0;
              }
            }
      ...
      else if (prms.momenta_rescaling_algo == 200 ||
               prms.momenta_rescaling_algo ==
                   201) {  // rescale momenta along the derivative coupling vector
        ...
        if (prms.momenta_rescaling_algo == 200) {
          do_reverse = 0;
        } else if (prms.momenta_rescaling_algo == 201) {
          do_reverse = 1;
        }
        ...
        for (traj = 0; traj < ntraj; traj++) {
          ...
          if (old_st != new_st) {
            ...
            p_tr = p.col(traj);
            for (dof = 0; dof < ndof; dof++) {
              vel.set(dof, 0, p_tr.get(dof, 0) * invM.get(dof, 0));
            }
      ...
      else if (prms.momenta_rescaling_algo == 210 ||
               prms.momenta_rescaling_algo ==
                   211) {  // rescale momenta along the difference in forces
...
*/
        ...
        if (prms.momenta_rescaling_algo == 210) {
          do_reverse = 0;
        } else if (prms.momenta_rescaling_algo == 211) {
          do_reverse = 1;
        }

...
*/
        ...
        MATRIX q(1, 1);
        CMATRIX* ham_dia;
        ham_dia = new CMATRIX(nstates, nstates);
...
*/
        ...
        MATRIX* diaH;
        diaH = new MATRIX(nstates, nstates);
        MATRIX* adiH;
        adiH = new MATRIX(nstates, nstates);
        MATRIX* S;
        S = new MATRIX(nstates, nstates);
        S->Init_Unit_Matrix(1.0);
        MATRIX* C;
        C = new MATRIX(nstates, nstates);
        *C = 0.0;
        MATRIX* si;
        si = new MATRIX(nstates, nstates);
        *si = 0.0;  // cos(-dt*E), where E is adiabatic Ham.
        MATRIX* cs;
        cs = new MATRIX(nstates, nstates);
        *cs = 0.0;  // sin(-dt*E), where E is adiabatic Ham.
        ...
        for (int nx = 0; nx < Nx; nx++) {
          ...
          *si = (*C) * (*si) * ((*C).T());
          ...
          for (nst = 0; nst < nstates; nst++) {
            for (nst1 = 0; nst1 < nstates; nst1++) {
              expH[nst][nst1].M[nx] = complex<double>(cs->M[nst * nstates + nst1],
                                                      si->M[nst * nstates + nst1]);  // exp(-i*H*dt)
            }
          }  //for nst
        ...
        for (int nx = 0; nx < Nx; nx++) {
          for (int ny = 0; ny < Ny; ny++) {
            ...
            *si = (*C) * (*si) * ((*C).T());
            ...
            for (nst = 0; nst < nstates; nst++) {
              for (nst1 = 0; nst1 < nstates; nst1++) {
                expH[nst][nst1].M[nx * Ny + ny] = complex<double>(
                    cs->M[nst * nstates + nst1], si->M[nst * nstates + nst1]);  // exp(-i*H*dt)
              }
            }  //for nst
...
*/
        ...
        CMATRIX psi1(Nx, 1);
        psi1 = 0.0;  // is a matrix placeholder
        CMATRIX psi2(Nx, 1);
        psi2 = 0.0;  // is a matrix placeholder
        vector<CMATRIX> newPSI;
        newPSI = PSI;
        ...
        for (nst = 0; nst < nstates; nst++) {
          psi.dot_product(reciPSI[nst], expK[nst]);
          reciPSI[nst] = psi;
        }  // for nst
...
*/
        ...
        if (Pops_l.size() < nstates) {
          Pops_l = vector<double>(nstates, 0.0);
        }  // Population in the left trapping region
        if (Pops_r.size() < nstates) {
          Pops_r = vector<double>(nstates, 0.0);
        }
        ...
        for (int nst = 0; nst < nstates; nst++) {
          ...
          for (i = 0; i <= nL; i++) {
            Pops_r[nst] +=
                dx * real(std::conj(PSI[nst].M[(Nx - 1 - i)]) * PSI[nst].M[(Nx - 1 - i)]);

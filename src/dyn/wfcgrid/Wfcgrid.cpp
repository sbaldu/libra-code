...
*/
      ...
      void Wfcgrid::init_numbers(double minx_,
                                 double maxx_,
                                 double dx_,
                                 double miny_,
                                 double maxy_,
...
*/
        ...
        reciPSI = vector<CMATRIX>(
            nstates, psi);  // same as PSI but in Fourier (reciprocal) space with 1.0*kmin
        DtreciPSI = vector<CMATRIX>(
            nstates, psi);  // d/dt * reciPSI - time derivative of PSI in reciprocal space
        DxPSI = vector<CMATRIX>(nstates, psi);  // d/dx * PSI in real space (up to factor)
        KxreciPSI =
            vector<CMATRIX>(nstates, psi);  // Kx * reciPSI  -  d/dx * PSI in k space (up to factor)
        ...
        Dx = vector<vector<CMATRIX> >(
            ...
            vector<CMATRIX>(
                nstates,
                psi));  // for each i,j pair - a separate x projection for all r-points of the grid
...
*/
        ...
        KxreciPSI =
            vector<CMATRIX>(nstates, psi);  // Kx * reciPSI  -  d/dx * PSI in k space (up to factor)
        KyreciPSI =
            vector<CMATRIX>(nstates, psi);  // Ky * reciPSI  -  d/dy * PSI in k space (up to factor)
        ...
        Dy = vector<vector<CMATRIX> >(
            ...
            vector<CMATRIX>(
                nstates,
                psi));  // for each i,j pair - a separate y projection for all r-points of the grid
        expH = vector<vector<CMATRIX> >(nstates, vector<CMATRIX>(nstates, psi));
        expK = vector<CMATRIX>(nstates, psi);
...
*/
      ...
      void Wfcgrid::init_wfc_1D(vector<double>& x0,
                                vector<double>& px0,
                                vector<double>& dx0,
...
*/
      ...
      void Wfcgrid::init_wfc_1D_HO(vector<int>& init_state,
                                   vector<int>& nu,
                                   vector<complex<double> >& weights,
                                   vector<double>& x0,
                                   vector<double>& px0,
...
*/
      ...
      void Wfcgrid::init_wfc_2D(
          double x0, double y0, double px0, double py0, double dx0, double dy0, int init_state) {
        /**
...
*/
        ...
        for (int nx = 0; nx < Nx; nx++) {
          out << real(X->M[nx]) << "  " << real(std::conj(PSI[state].M[nx]) * PSI[state].M[nx])
              << endl;
        ...
        for (int nx = 0; nx < Nx; nx++) {
          for (int ny = 0; ny < Ny; ny++) {
            out << real(X->M[nx]) << "  " << real(Y->M[ny]) << "  "
                << real(std::conj(PSI[state].M[nx * Ny + ny]) * PSI[state].M[nx * Ny + ny]) << endl;
...
*/
        ...
        for (int nx = 0; nx < Nx; nx++) {
          for (int ny = 0; ny < Ny; ny++) {
            out << real(2.0 * M_PI * Kx->M[nx]) << "  " << real(2.0 * M_PI * Ky->M[ny]) << "  "
                << real(std::conj(reciPSI[state].M[nx * Ny + ny]) * reciPSI[state].M[nx * Ny + ny])
                << endl;
...
*/
        ...
        for (int nx = 0; nx < Nx; nx++) {
          out << real(X->M[nx]) << "  " << real(2.0 * M_PI * Kx->M[nx]) << "  " << CM.M[nx].real()
              << "   " << CM.M[nx].imag() << endl;
        }  // for nx
          ...
          for (int nst1 = 0; nst1 < nstates; nst1++) {
            for (int nx = 0; nx < Nx; nx++) {
              for (int ny = 0; nx < Ny; ny++) {
                ...
                res += real(Kx->M[nx]) * real(std::conj(reciPSI[nst].M[nx * Ny + ny]) *
        ...
        for (int nst = 0; nst < nstates; nst++) {
          for (int nst1 = 0; nst1 < nstates; nst1++) {
            for (int nx = 0; nx < Nx; nx++) {
              for (int ny = 0; nx < Ny; ny++) {
                norm += real(std::conj(PSI[nst].M[nx * Ny + ny]) * PSI[nst1].M[nx * Ny + ny]);
                res += real(Ky->M[ny]) * real(std::conj(reciPSI[nst].M[nx * Ny + ny]) *
                                              reciPSI[nst1].M[nx * Ny + ny]);
        ...
        for (int nst = 0; nst < nstates; nst++) {
          for (int nst1 = 0; nst1 < nstates; nst1++) {
            for (int nx = 0; nx < Nx; nx++) {
              for (int ny = 0; ny < Ny; ny++) {
                norm += real(std::conj(PSI[nst].M[nx * Ny + ny]) * PSI[nst1].M[nx * Ny + ny]);
                res += real(std::conj(PSI[nst].M[nx * Ny + ny]) * H[nst][nst1].M[nx * Ny + ny] *
                            PSI[nst1].M[nx * Ny + ny]);

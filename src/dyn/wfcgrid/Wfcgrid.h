...
*/
        ...
        void init_numbers(double minx_,
                          double maxx_,
                          double dx_,
                          double miny_,
                          double maxy_,
  ...
  */
        ...
        vector<CMATRIX>
            reciPSI;  ///< same as PSI but in Fourier (reciprocal) space with 1.0*kmin - nstates x Nx x Ny
        vector<CMATRIX>
            DtreciPSI;  ///< d/dt * reciPSI - time derivative of PSI in reciprocal space  - nstates x Nx x Ny
        ...
        vector<CMATRIX>
            KxreciPSI;  ///< Kx * reciPSI  -  d/dx * PSI in k space (up to a factor) - nstates x Nx x Ny
        vector<CMATRIX>
            KyreciPSI;  ///< Ky * reciPSI  -  d/dy * PSI in k space (up to a factor) - nstates x Nx x Ny

        // Hamiltonian and Propagator
        vector<vector<CMATRIX> > H;  ///< PES - nstates x nstates x Nx x Ny - full Hamiltonian
        vector<vector<CMATRIX> >
            Dx;  ///< for each i,j pair - a separate x projection for all r-points of the grid - nstates x nstates x Nx x Ny
        vector<vector<CMATRIX> >
            Dy;  ///< for each i,j pair - a separate y projection for all r-points of the grid - nstates x nstates x Nx x Ny
        vector<vector<CMATRIX> > expH;  ///< nstates x nstates x Nx x Ny
        vector<CMATRIX> expK;           ///< nstates x Nx x Ny

        // Constructor : 1D and 2D
        Wfcgrid(double minx_,
                double maxx_,
                double dx_,
                int nstates_);  ///< constructor for 1D wavefunction
        Wfcgrid(double minx_,
                double maxx_,
                double dx_,
                double miny_,
                double maxy_,
                double dy_,
                int nstates_);  ///< constructor for 2D wavefunction

        // Populate wfc
        void init_wfc_1D(double x0,
                         double px0,
                         double dx,
                         int init_state);  ///< initialization of 1D wavefunction
        ///< initialization of 1D wavefunction
        void init_wfc_1D(vector<double>& x0,
                         vector<double>& px0,
                         vector<double>& dx,
                         vector<int>& init_state,
                         vector<complex<double> >& weights);

        void init_wfc_1D_HO(vector<int>& init_state,
                            vector<int>& nu,
                            vector<complex<double> >& weights,
                            vector<double>& x0,
                            vector<double>& px0,
        ...
        void init_wfc_2D(double x0,
                         double y0,
                         double px0,
                         double py0,
                         double dx,
                         double dy,
                         int init_state);  ///< initialization of 2D wavefunction

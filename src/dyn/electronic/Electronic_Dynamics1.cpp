...
*/
      ...
      void iL2_action(double dt, CMATRIX& Coeff, CMATRIX& Hvib, int i, int j) {
        ...
        exp_iL2.set(0, 0, cs * one);
        exp_iL2.set(0, 1, si * one);
        exp_iL2.set(1, 0, -si * one);
        exp_iL2.set(1, 1, cs * one);
      ...
      void iL3_action(double dt, CMATRIX& Coeff, CMATRIX& Hvib, int i, int j) {
        ...
        exp_iL3.set(0, 0, cs * one);
        exp_iL3.set(0, 1, -si * eye);
        exp_iL3.set(1, 0, -si * eye);
        exp_iL3.set(1, 1, cs * one);
...
*/
        ...
        libmeigen::solve_eigen(
            Hvib, I, Heig, C, 0);  // Hvib_eff * C = I * C * Heig  ==>  Hvib = C * Heig * C.H()
...
*/
        ...
        CMATRIX Heig(
            sz, sz);  // eigenvalues - same for Hvib and HvibH, but the eigenvectors are different
        CMATRIX HvibH(sz, sz);
        HvibH = Hvib.H();
        CMATRIX expH(sz, sz);
...
*/
        ...
        libmeigen::solve_eigen(
            S, I, Seig, C, 0);  // S * C = I * C * Seig  ==>  S = C * Seig * C.H()
        ...
        libmeigen::solve_eigen(
            Hvib_eff, I, Seig, C, 0);  // Hvib_eff * C = I * C * Seig  ==>  Hvib = C * Seig * C.H()
        ...
        coeff =
            S_i_half * coeff;  // convert the effective coefficients back to original representation

        for (i = 0; i < Coeff.n_elts; i++) {
          Coeff.set(i, 0, coeff.get(i, 0));
        }  // so we don't allocate new memory for Coeff!!!
...
*/
      ...
      void propagate_electronic_qtag2(
          double dt, CMATRIX& Coeff, CMATRIX& Hvib, CMATRIX& Hvib_old, CMATRIX& S, CMATRIX& S_old) {
        /**
  Solves the generalized time-dependent Schrodinger equation:
...
*/
        ...
        // Transformation to adiabatic basis
        libmeigen::solve_eigen(
            Hvib, S, E, Z, 0);  // Hvib(t+dt) * Z(t+dt) = S(t+dt) * Z(t+dt) * E(t+dt)
        libmeigen::solve_eigen(
            Hvib_old, S_old, E_old, Z_old, 0);  // Hvib(t) * Z(t) = S(t) * Z(t) * E(t)
...
*/
      ...
      CMATRIX vectorize_density_matrix(CMATRIX& rho) { return vectorize_density_matrix(&rho); }
      ...
      CMATRIX unvectorize_density_matrix(CMATRIX& rho_vec) {
        ...
        int n = int(n_fl);
        if (n * n != sz) {
          cout << "Error in unvectorize_density_matrix: The number of elements in the input vector "
                  "should be a full square of an integer\n";
